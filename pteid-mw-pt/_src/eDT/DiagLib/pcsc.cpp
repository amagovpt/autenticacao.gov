...
#define PTEID_FIELD_TAG_ID_FirstName_2			0x09
...
int ReadCard(Card_ID id, Card_INFO *pInfo, bool *success, int * const TransmitDelay)
{
	...
	if(DIAGLIB_OK == iReturnCode)
	{
		...
		{
			GetTlvValue(Content,PTEID_FIELD_TAG_ID_Surname,&info.LastName);
			std::wstring FirstName;
			GetTlvValue(Content,PTEID_FIELD_TAG_ID_FirstName_1,&FirstName);
			info.FirstName.assign(FirstName);
			GetTlvValue(Content,PTEID_FIELD_TAG_ID_FirstName_2,&FirstName);
		...
		{
			GetTlvValue(Content,PTEID_FIELD_TAG_ADDR_Street,&info.Street);
