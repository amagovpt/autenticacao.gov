...
int PDFSignature::signSingleFile(const char *location, const char *reason, const char *outfile_path, bool isCardSign) {
	...
	if (showNIC) {
		...
		doc->prepareSignature(m_incrementalMode, &sig_location, m_citizen_fullname, doc_nr.c_str(), location, reason,
							  m_page, m_sector, isLangPT, isCC(), showDate, m_small_signature);
	} else {
		doc->prepareSignature(m_incrementalMode, &sig_location, m_citizen_fullname, NULL, location, reason, m_page,
							  m_sector, isLangPT, isCC(), showDate, m_small_signature);
	}
...
void PDFSignature::save() {
	PDFWriteMode pdfWriteMode = m_incrementalMode ? writeForceIncremental : writeForceRewrite;
