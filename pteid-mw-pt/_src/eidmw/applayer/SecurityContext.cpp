 ...
 */
CByteArray retail_mac_des(CByteArray &key, CByteArray &mac_input, uint64_t ssc) {
...
bool SecurityContext::validateInternalAuth(CByteArray &resp) {
	...
	int ret = RSA_public_decrypt(signature.Size(), signature.GetBytes(), inputToSignature, rsa_key, RSA_NO_PADDING);
...
void SecurityContext::initMuthualAuthProcess() {
	...
	// DH_compute_key() computes the shared secret from the private DH value in dh and the other party's public value in
	// pub_key and stores it in key. key must point to DH_size(dh) bytes of memory.
