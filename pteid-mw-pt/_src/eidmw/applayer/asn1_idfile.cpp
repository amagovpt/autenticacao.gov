...
ASN1_SEQUENCE(ICC_PUBLICKEY) =
	{
		ASN1_SIMPLE(ICC_PUBLICKEY, curve_oid, ASN1_OBJECT),
		...
		ASN1_IMP(ICC_PUBLICKEY, pk_cofactor, ASN1_OCTET_STRING, 0x7),

} ASN1_SEQUENCE_END(ICC_PUBLICKEY)

		IMPLEMENT_ASN1_FUNCTIONS(ICC_PUBLICKEY)

			ASN1_SEQUENCE(IDFILE) = {ASN1_IMP_EX(IDFILE, issuing_entity, ASN1_UTF8STRING, 0x0, ASN1_TFLG_PRIVATE),
									 ...
									 ASN1_IMP_EX(IDFILE, possible_indications, ASN1_UTF8STRING, 0x18,
												 ASN1_TFLG_PRIVATE | ASN1_TFLG_OPTIONAL),
									 ASN1_EX_TYPE(ASN1_TFLG_IMPTAG | ASN1_TFLG_APPLICATION, 73, IDFILE, icc_public_key,
												  ICC_PUBLICKEY)
										...
										IMPLEMENT_ASN1_FUNCTIONS(IDFILE)
											...
											IDFILE
									* decode_id_data(CByteArray & data) {
