...
ProxyInfo::ProxyInfo() {
	...
	if (system_proxy && pacfile.length() > 0) {
		MWLOG(LEV_DEBUG, MOD_APL, "Proxy configured with pacfile in system: %s", pacfile.c_str());
		auto_configured = true;
		m_pac_url = pacfile;
	} else if (manual_configured) {
		if (system_proxy) {
			MWLOG(LEV_DEBUG, MOD_APL, "Proxy configured manually in system: host=%s, port=%lu", proxy_host.c_str(),
				  proxy_port);
		} else {
			MWLOG(LEV_DEBUG, MOD_APL, "Proxy configured manually in application: host=%s, port=%lu", proxy_host.c_str(),
				  proxy_port);
		}
	} else {
...
void ProxyInfo::getProxyForHost(std::string urlToFetch, std::string *proxy_host, long *proxy_port) {
	if (!auto_configured) {
		MWLOG(LEV_ERROR, MOD_APL, "getProxyForHost: Trying to get proxy from pacfile but it is not configured.");
		return;
	}
	...
	MWLOG(LEV_DEBUG, MOD_APL, "getProxyForHost: Obtained proxy for endpoint %s: proxy_host=%s proxy_port=%lu",
		  urlToFetch.c_str(), proxy_host->c_str(), *proxy_port);
}
...
std::string ProxyInfo::getProxyHost() {
	...
	if (!manual_configured) {
		MWLOG(LEV_ERROR, MOD_APL, "getProxyHost: Trying to get proxy host but there is not manual configuration.");
		return "";
	}
	return m_proxy_host;
}
...
std::string ProxyInfo::getProxyPort() {
	...
	if (!manual_configured) {
		MWLOG(LEV_ERROR, MOD_APL, "getProxyPort: Trying to get proxy port but there is not manual configuration.");
		return "";
	}
	return m_proxy_port;
}
