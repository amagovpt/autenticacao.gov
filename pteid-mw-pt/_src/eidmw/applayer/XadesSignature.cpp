...
static void appendCertRef(XERCES_CPP_NAMESPACE::DOMDocument *doc, APL_Certif *cert, DOMNode *parent) {
	...
	DOMNode *n_Cert = CREATE_DOM_NODE makeQName(str, prefix, "CertDigest");
	DOMNode *n_CertDigest = CREATE_DOM_NODE makeQName(str, prefix_ds, "DigestMethod");
	...
	((DOMElement *)n_CertDigestMeth)
		->setAttribute(XMLString::transcode("Algorithm"), XMLString::transcode(SHA256_ID.c_str()));
...
static void addSignature(unsigned char *signature, unsigned int siglen, XERCES_CPP_NAMESPACE::DOMDocument *doc) {
	...
	DOMNodeList *nodes =
		doc->getElementsByTagNameNS(XMLString::transcode(DSIG_NAMESPACE), XMLString::transcode("SignatureValue"));
...
static DOMNode *addSignatureProperties(DSIGSignature *sig, XMLCh *sig_id, APL_Certif *signerCert, bool do_timestamping,
									   bool do_long_term_validation) {
	...
	((DOMElement *)n1)
		->setAttributeNS(XMLString::transcode(XADES_NAMESPACE), XMLString::transcode("Target"), props_target.c_str());
	...
	DOMNode *n_signSigProps = CREATE_DOM_NODE makeQName(str, prefix, "SigningTime");
	DOMNode *n_signingTime = CREATE_DOM_NODE makeQName(str, prefix, "SigningCertificate");
...
static DOMNode *findDOMNodeHelper(DOMDocument *dom, const char *ns, const char *tagname) {
	DOMNode *node;
	DOMNodeList *nodes1 = dom->getElementsByTagNameNS(XMLString::transcode(ns), XMLString::transcode(tagname));
	...
	while (!certif->isRoot()) {
		...
		if ((issuer = certif->getIssuer()) == NULL) {
			MWLOG(LEV_ERROR, MOD_APL,
				  "XadesSignature: addCertificateChain() Couldn't find issuer "
				  "for cert: %s",
				  certif->getOwnerName());
			break;
		}
...
static bool addRevocationInfo(DOMDocument *dom, APL_Certif *signerCert) {
	...
	DOMElement *revocation_node = dom->createElementNS(XMLString::transcode(XADES_NAMESPACE), str.rawXMLChBuffer());

	makeQName(str, prefix_xades, "OCSPValues");
	DOMElement *ocsp_values_node = dom->createElementNS(XMLString::transcode(XADES_NAMESPACE), str.rawXMLChBuffer());

	makeQName(str, prefix_xades, "CRLValues");
	DOMElement *crl_values_node = dom->createElementNS(XMLString::transcode(XADES_NAMESPACE), str.rawXMLChBuffer());
...
static bool appendTimestamp(DOMDocument *dom, DOMNode *parent, const char *tag_name, std::string to_timestamp,
	...
	if (timestamp_data.Size() == 0) {
		MWLOG(LEV_ERROR, MOD_APL,
			  L"An error occurred in timestamp_data. "
		...
		DOMElement *ts_node = dom->createElementNS(XMLString::transcode(XADES_NAMESPACE), str.rawXMLChBuffer());

		// We need to specify this because otherwise the timestamp validation can go wrong with different namespace
		// handling
		makeQName(str, prefix_dsig, "CanonicalizationMethod");
		DOMElement *c14n_node = dom->createElementNS(XMLString::transcode(DSIG_NAMESPACE), str.rawXMLChBuffer());
		...
		DOMElement *ts_token_node = dom->createElementNS(XMLString::transcode(XADES_NAMESPACE), str.rawXMLChBuffer());
...
static void setReferenceHash(XMLByte *hash, unsigned int hash_len, int ref_index, DOMDocument *doc) {
	...
	DOMNodeList *nodes1 =
		doc->getElementsByTagNameNS(XMLString::transcode(DSIG_NAMESPACE), XMLString::transcode("DigestValue"));
	...
	try {
		...
		if (m_doTimestamp || m_doLTV) {
			try {
				...
				MWLOG(LEV_ERROR, MOD_APL,
					  L"XadesSignature::Sign: Failed to add timestamp. "
					  "Error code: %08x",
					  e.GetError());
		...
		if (m_doLTV && !m_throwTimestampException) {
			...
			try {
				...
				MWLOG(LEV_ERROR, MOD_APL,
					  L"XadesSignature::Sign: Failed to add archive timestamp. "
					  "Error code: %08x",
					  e.GetError());
