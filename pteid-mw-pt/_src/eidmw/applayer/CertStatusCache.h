...
/******************************************************************************/ /**
																				  * Class caching the status for the
																				  *latest certificate in a file
																				  *
																				  * - We keep the status by Certificate
																				  *UniqueId and Flags (validation type,
																				  *allow test Root...) This way we can
																				  *have different status depending the
																				  *parameters (for example one status if
																				  *CRL validation is wanted and another
																				  *for OCSP validation)
																				  * - Each line in the cache has a
																				  *validity timestamp (for example valid
																				  *for 15 minutes - see
																				  *CSC_MAX_VALIDITY_SECOND) After this
																				  *delay the line is not valid anymore
																				  *and the status must be check again
																				  * - The cache is multi-process safe.
																				  *   Each access to the file is
																				  *protected by Mutex. The file is
																				  *written back as soon as possible.
																				  * - If process has to calculate the
																				  *status, he writes the status WAIT in
																				  *the file , so another process has to
																				  *wait until the status is available
																				  *(or the validity is passed) The wait
																				  *validity delay is shorter than the
																				  *normal delay to avoid waiting to long
																				  *in case of crash of the calculating
																				  *process
																				  *
																				  * Usage
																				  *CSCache.getCertStatus(ulUniqueID,validationType,pStore,bAllowTestRoot)
																				  *********************************************************************************/
