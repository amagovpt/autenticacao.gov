		...
		unsigned char *sigret, unsigned int *siglen, const RSA * rsa)
{
	...
	MWLOG(LEV_DEBUG, MOD_APL, "SSLConnectionLegacy.rsa_sign() called with type=%d", type);
	...
	{
		MWLOG(LEV_ERROR, MOD_APL, L"SSLConnectionLegacy->rsa_sign(): Exception caught in card.Sign. Aborting connection");
...
unsigned long SSLConnectionLegacy::getKeyLength() {
	APL_Certif *auth_cert = m_certs->getCert(APL_CERTIF_TYPE_AUTHENTICATION);
	return auth_cert->getKeyLength();
}


APL_Certif * SSLConnectionLegacy::loadCertsFromCard(SSL_CTX *ctx)
{
...
void SSLConnectionLegacy::loadAllRootCerts(X509_STORE *store)
{
	X509 * pCert = NULL;
	for (int i=0; i != m_certs->countAll(); i++)
	{
		...
		if (cert != NULL && cert->isRoot())
		{
		    ...
		    if (pCert == NULL)
			{
				...
				MWLOG(LEV_ERROR, MOD_APL, "SSLConnectionLegacy::loadAllRootCerts: Error parsing certificate #%d. Details: %s",
					i, parsing_error);
			}
			else
			{
				if (X509_STORE_add_cert(store, pCert) == 0)
				{
					...
					MWLOG(LEV_DEBUG, MOD_APL, "SSLConnectionLegacy::loadAllRootCerts: error adding certificate #%d Details: %s",
					  i, loading_error);
				}
			}
			pCert = NULL;
...
void SSLConnectionLegacy::loadCertChain(X509_STORE *store, APL_Certif * authentication_cert)
{
	...
	while (!certif->isRoot())
	{
		...
		{
			char *parsing_error = ERR_error_string(ERR_get_error(), NULL);
			MWLOG(LEV_ERROR, MOD_APL, L"SSLConnectionLegacy::loadCertChain: Error parsing certificate #%d. Details: %s",
				i, parsing_error);
		}
		else
		{
			...
			{
				char *loading_error = ERR_error_string(ERR_get_error(), NULL);
				MWLOG(LEV_ERROR, MOD_APL, L"SSLConnectionLegacy::loadCertChain: error adding certificate #%d Details: %s",
				  i, loading_error);
			}
		}
		pCert = NULL;
 ...
 * Initialise OpenSSL
 */
void SSLConnectionLegacy::init_openssl() {
 ...
 */
void SSLConnectionLegacy::CloseConnection() {
...
SignedChallengeResponse * SSLConnectionLegacy::do_SAM_2ndpost(char *challenge, char *kicc)
{
	cJSON *json = NULL;
	char *endpoint = NULL;
	...
	MWLOG(LEV_DEBUG, MOD_APL, L"SSLConnectionLegacy: running do_SAM_2ndpost()");
...
bool SSLConnectionLegacy::do_SAM_4thpost(StartWriteResponse &resp)
{
	cJSON *json = NULL;
	char *server_response = NULL;
	char *json_request = build_json_object_sam(resp);

	MWLOG(LEV_DEBUG, MOD_APL, L"SSLConnectionLegacy: running do_SAM_4thpost()");
	...
	if (!json)
	{
		fprintf(stderr, "SSLConnectionLegacy::do_SAM_4thpost - JSON parsing error before: [%s]\n", cJSON_GetErrorPtr());
		return false;
	}
	else
	{
		...
		if (error_code->type == cJSON_Number)
		{
			...
			MWLOG(LEV_DEBUG, MOD_APL, L"SSLConnectionLegacy::do_SAM_4thpost - Server returned error code: %d\n", error_value);
			cJSON_Delete(json);
...
StartWriteResponse *SSLConnectionLegacy::do_SAM_3rdpost(char * mse_resp, char *internal_auth_resp)
{
	cJSON *json = NULL;
	...
	MWLOG(LEV_DEBUG, MOD_APL, L"SSLConnectionLegacy: running do_SAM_3rdpost()");
...
DHParamsResponse *SSLConnectionLegacy::do_SAM_1stpost(DHParams *p, char *secretCode, char *process, char *serialNumber)
{
	int ret_channel = 0;
	cJSON *json = NULL;
	char *endpoint = NULL;
	...
	MWLOG(LEV_DEBUG, MOD_APL, L"SSLConnectionLegacy: running do_SAM_1stpost()");
...
char * SSLConnectionLegacy::Post(char *cookie, char *url_path, char *body)
{
	int ret_channel = 0;
...
//	fprintf(stderr, "DEBUG: Sending POST Body: \n%s\n", body);
	...
	{
		MWLOG(LEV_DEBUG, MOD_APL, "SSLConnectionLegacy:Post() server response is chunked, calling read_chunked_reply()");
...
BIO * SSLConnectionLegacy::connectToProxyServer(const char * proxy_host, long proxy_port, char *ssl_host, char *proxy_user, char * proxy_pwd, char *ssl_host_andport)
{
		...
		MWLOG(LEV_DEBUG, MOD_APL, "SSLConnectionLegacy: Connecting to proxy Host: %s Port: %ld",
        ...
        if (BIO_do_connect(cbio) <= 0) {
               MWLOG(LEV_ERROR, MOD_APL, L"SSLConnectionLegacy: TCP connection to proxy server failed!");
               return NULL;
        }
        ...
        MWLOG(LEV_DEBUG, MOD_APL, "SSLConnectionLegacy: CONNECT reply: %s", tmpbuf);
...
void SSLConnectionLegacy::log_server_address(BIO * bio) {
 ...
 */
void SSLConnectionLegacy::connect_encrypted(char* host_and_port)
{
	...
	//Check if SSLConnectionLegacy was passed a "valid card object"
	...
	{
		MWLOG(LEV_DEBUG, MOD_APL, "SSLConnectionLegacy using Proxy PAC: %s to URL: %s", proxy_pac, SAM_URL.c_str());
	...
	else
	{
    	...
    	if (BIO_do_connect(bio) <= 0) {
    	   MWLOG(LEV_ERROR, MOD_APL, "SSLConnectionLegacy: BIO_do_connect failed: %s", 
    	   	 ERR_error_string(ERR_get_error(), NULL));
    ...
    {
		long openssl_error = ERR_get_error();
		MWLOG(LEV_ERROR, MOD_APL, "SSLConnectionLegacy: error establishing connection. Detail: %s", ERR_error_string(openssl_error, NULL));
...
void SSLConnectionLegacy::read_chunked_reply(SSL *ssl, NetworkBuffer *net_buffer, bool headersAlreadyRead)
{
	int r;
 ...
 */
unsigned int SSLConnectionLegacy::read_from_stream(SSL* ssl, NetworkBuffer *net_buffer)
{
 ...
 */
unsigned int SSLConnectionLegacy::write_to_stream(SSL* ssl, char* request_string) 
{
 ...
 */
bool SSLConnectionLegacy::InitSAMConnection() 
{
    ...
    *delim = '\0';
    ...
    MWLOG(LEV_DEBUG, MOD_APL, "SSLConnectionLegacy: connecting to SAM server: %s", host_and_port);
