...
#define PTEID_MAX_IMAGEHEADER_LEN                                                                                      \
	(PTEID_MAX_CBEFF_LEN + PTEID_MAX_FACRECH_LEN + PTEID_MAX_FACINFO_LEN + PTEID_MAX_IMAGEINFO_LEN)
...
#define PTEID_E_INSUFFICIENT_BUFFER                                                                                    \
	3								  /* The data buffer to receive returned data is too small for the returned data   \
									   */
...
PTEIDSDK_API long PTEID_Init(char *ReaderName /**< in: the PCSC reader name (as returned by SCardListReaders()),
											  specify NULL if you want to select the first reader */
);

PTEIDSDK_API long
PTEID_Exit(unsigned long ulMode /**< in: exit mode, either PTEID_EXIT_LEAVE_CARD or PTEID_EXIT_UNPOWER */
);
 ...
 * Read the ID data.
 */
PTEIDSDK_API long PTEID_GetID(PTEID_ID *IDData /**< out: the address of a PTEID_ID struct */
);

/**
 * Read the Address data.
 */
PTEIDSDK_API long PTEID_GetAddr(PTEID_ADDR *AddrData /**< out: the address of an PTEID_ADDR struct */
);

/**
 * Read the Picture.
 */
PTEIDSDK_API long PTEID_GetPic(PTEID_PIC *PicData /**< out: the address of a PTEID_PIC struct */
);

/**
 * Read all the user and CA certificates.
 */
PTEIDSDK_API long PTEID_GetCertificates(PTEID_Certifs *Certifs /**< out: the address of a PTEID_Certifs struct */
);
 ...
 */
PTEIDSDK_API long
PTEID_VerifyPIN(unsigned char PinId, /**< in: the PIN ID, see the PTEID_Pins struct */
 ...
 */
PTEIDSDK_API long
PTEID_VerifyPIN_No_Alert(unsigned char PinId, /**< in: the PIN ID, see the PTEID_Pins struct */
						 char *Pin,		 /**< in: the PIN value, if NULL then the user will be prompted for the PIN */
						 long *triesLeft /**< out: the remaining PIN tries */
);
 ...
 */
PTEIDSDK_API long
PTEID_ChangePIN(unsigned char PinId, /**< in: the PIN ID, see the PTEID_Pins struct */
 ...
 */
PTEIDSDK_API long PTEID_GetPINs(PTEIDPins *Pins /**< out: the address of a PTEID_Pins struct */
);

/**
 * Return the PKCS15 TokenInfo contents.
 */
PTEIDSDK_API long PTEID_GetTokenInfo(PTEID_TokenInfo *tokenData /**< out: the address of a PTEID_TokenInfo struct */
);
...
PTEIDSDK_API long PTEID_ReadSOD(unsigned char *out,	  /**< out: the buffer to hold the file contents */
 ...
 */
PTEIDSDK_API long
PTEID_UnblockPIN(unsigned char PinId, /**< in: the PIN ID, see the PTEID_Pins struct */
 ...
 */
PTEIDSDK_API long
PTEID_UnblockPIN_Ext(unsigned char PinId, /**< in: the PIN ID, see the PTEID_Pins struct */
					 char *pszPuk,		  /**< in: the PUK value, if NULL then the user will be prompted for the PUK */
					 char *pszNewPin, /**< in: the new PIN value, if NULL then the user will be prompted for the PIN */
					 long *triesLeft, /**< out: the remaining PUK tries */
 ...
 */
PTEIDSDK_API long PTEID_SelectADF(unsigned char *adf, /**< in: the AID of the ADF */
 ...
 */
PTEIDSDK_API long
PTEID_ReadFile(unsigned char *file,	  /**< in: a byte array containing the file path,
 ...
 */
PTEIDSDK_API long
PTEID_WriteFile(unsigned char *file, /**< in: a byte array containing the file path,
									 e.g. {0x3F, 0x00, 0x5F, 0x00, 0xEF, 0x02} for the ID file */
				int filelen,		 /**< in: file length */
...
PTEIDSDK_API long
PTEID_WriteFile_inOffset(unsigned char *file,	 /**< in: a byte array containing the file path,
												 e.g. {0x3F, 0x00, 0x5F, 0x00, 0xEF, 0x02} for the ID file */
						 int filelen,			 /**< in: file length */
						 unsigned char *in,		 /**< in: the data to be written to the file */
 ...
 */
PTEIDSDK_API long
PTEID_IsActivated(unsigned long *pulStatus /**< out the activation status: 0 if not activate, 1 if activated */
);
 ...
 */
PTEIDSDK_API long
PTEID_Activate(char *pszPin,		   /**< in: the value of the Activation PIN */
 ...
 * certificate.
 */
PTEIDSDK_API long PTEID_SetSODChecking(int bDoCheck /**< in: true to turn on SOD checking, false to turn it off */
);
 ...
 */
PTEIDSDK_API long PTEID_SetSODCAs(PTEID_Certifs *Certifs /**< in: the address of a PTEID_Certifs, or NULL */
);
 ...
 */
PTEIDSDK_API long
PTEID_GetCVCRoot(PTEID_RSAPublicKey *pCVCRootKey /**< in: the address of a PTEID_RSAPublicKey struct */
);
 ...
 */
PTEIDSDK_API long PTEID_SendAPDU(const unsigned char *ucRequest, /**<in: command APDU */
 ...
 */
PTEIDSDK_API long
PTEID_CAP_ChangeCapPin(const char *csServer,				/**<in: Address Change Server, format: <name>:<port>. */
...
PTEIDSDK_API long
PTEID_CVC_Init(const unsigned char *pucCert, /**< in: the CVC as a byte array */
 ...
 */
PTEIDSDK_API long
PTEID_CVC_Authenticate(unsigned char *pucSignedChallenge, /**< in: the challenge that was signed by the
 ...
 * must have been done before.
 */
PTEIDSDK_API long PTEID_CVC_GetAddr(PTEID_ADDR *AddrData /**< out: the address of a PTEID_ADDR struct */
);
