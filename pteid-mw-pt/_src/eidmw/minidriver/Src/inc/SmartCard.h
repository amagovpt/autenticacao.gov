...
extern DWORD PteidReadPrKDF(PCARD_DATA pCardData, DWORD *out_len, PBYTE *data);

extern DWORD PteidParsePrKDF(PCARD_DATA pCardData, DWORD *cbStream, BYTE *pbStream, WORD *cbKeySize);

extern DWORD PteidReadFile(PCARD_DATA pCardData, DWORD dwOffset, DWORD *cbStream, PBYTE pbStream);

extern DWORD PteidAuthenticateExternal(PCARD_DATA pCardData, PDWORD pcAttemptsRemaining, BOOL bSilent, DWORD PinId

);

extern DWORD PteidSelectApplet(PCARD_DATA pCardData);

extern DWORD CCIDfindFeature(BYTE featureTag, BYTE *features, DWORD featuresLength);

extern DWORD CCIDgetFeatures(PFEATURES pFeatures, SCARDHANDLE hCard);

extern DWORD createVerifyCommand(PPIN_VERIFY_STRUCTURE pVerifyCommand, unsigned int pin_ref);
extern DWORD createVerifyCommandGemPC(PPIN_VERIFY_STRUCTURE pVerifyCommand, unsigned int pin_ref);
extern DWORD createVerifyCommandACR83(PPIN_VERIFY_STRUCTURE pVerifyCommand, unsigned int pin_ref);
#endif