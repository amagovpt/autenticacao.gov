...
CARD_ATR CardAtr[] = {
	// IAS v5 cards
	{{0x3b, 0xff, 0x96, 0x00, 0x00, 0x81, 0x31, 0xfe, 0x43, 0x80, 0x31, 0x80, 0x65,
	  0xb0, 0x85, 0x05, 0x00, 0x11, 0x12, 0x0f, 0xff, 0x82, 0x90, 0x00, 0xe1},
	 25}, // CC2 Contact
	{{0x3b, 0x8f, 0x80, 0x01, 0x80, 0x31, 0x80, 0x65, 0xb0, 0x85,
	...
	{{0x3B, 0xFF, 0x96, 0x00, 0x00, 0x81, 0x31, 0x80, 0x43, 0x80, 0x31, 0x80, 0x65,
	  0xb0, 0x85, 0x03, 0x00, 0xEF, 0x12, 0x0F, 0xFF, 0x82, 0x90, 0x00, 0x67},
	 25},
	{{0x3B, 0xFF, 0x96, 0x00, 0x00, 0x81, 0x31, 0xFE, 0x43, 0x80, 0x31, 0x80, 0x65,
	  0xB0, 0x85, 0x04, 0x01, 0x20, 0x12, 0x0F, 0xFF, 0x82, 0x90, 0x00, 0xD0},
	 25}};
...
DWORD WINAPI CardAcquireContext(IN PCARD_DATA pCardData, __in DWORD dwFlags) {
	...
	if ((pCardData->pfnCspAlloc == NULL) || (pCardData->pfnCspReAlloc == NULL) || (pCardData->pfnCspFree == NULL)) {
		LogTrace(LOGTYPE_ERROR, WHERE,
				 "Invalid parameter [pCardData->pfnCspAlloc], [pCardData->pfnCspReAlloc] or [pCardData->pfnCspFree]");
		CLEANUP(SCARD_E_INVALID_PARAMETER);
	}
	...
	SCardStatus(pCardData->hScard, readerName, &readername_len, NULL, &protocol, (LPBYTE)&bAtr, &cByte);
