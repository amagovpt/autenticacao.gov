...
DWORD PteidGetPubKey(PCARD_DATA pCardData, DWORD cbCertif, PBYTE pbCertif, DWORD *pcbPubKey, PBYTE *ppbPubKey) {
	...
	if (!CryptDecodeObject(X509_ASN_ENCODING, RSA_CSP_PUBLICKEYBLOB,
						   ...
						   (((*((*pCertContext).pCertInfo)).SubjectPublicKeyInfo).PublicKey).cbData, 0, NULL,
						   pcbPubKey)) {
	...
	*ppbPubKey = pCardData->pfnCspAlloc(*pcbPubKey);
	...
	if (!CryptDecodeObject(X509_ASN_ENCODING, RSA_CSP_PUBLICKEYBLOB,
						   (((*((*pCertContext).pCertInfo)).SubjectPublicKeyInfo).PublicKey).pbData,
						   (((*((*pCertContext).pCertInfo)).SubjectPublicKeyInfo).PublicKey).cbData, 0, *ppbPubKey,
						   pcbPubKey)) {
		LogTrace(LOGTYPE_ERROR, WHERE, "CryptDecodeObjectEx() returned [0x%02x]", GetLastError());
		CLEANUP(SCARD_E_UNEXPECTED);
	}
...
DWORD PteidCreateMSRoots(PCARD_DATA pCardData, DWORD *pcbMSRoots, PBYTE *ppbMSRoots) {
	...
	if (CertAddCertificateContextToStore(hMemoryStore, // Store handle
										 ...
										 CERT_STORE_ADD_REPLACE_EXISTING, NULL) == FALSE) {
	...
	blob.cbData = 0;
	blob.pbData = NULL;
	if (CertSaveStore(hMemoryStore, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, CERT_STORE_SAVE_AS_PKCS7,
					  CERT_STORE_SAVE_TO_MEMORY, &blob, 0) == FALSE) {
...
BOOL ReadReg(LPCTSTR szSubKey, LPCTSTR szValueName, LPDWORD lpType, LPBYTE pbOutput, LPDWORD lpcbOutputSize) {
	...
	dwRet = RegQueryValueEx(hKey, szValueName, NULL, NULL, (LPBYTE)pbValueCU, &dwData);
...
localmachine:
	...
	dwData = *lpcbOutputSize;
	dwRet = RegQueryValueEx(hKey, szValueName, NULL, NULL, (LPBYTE)pbValueLM, &dwData);
