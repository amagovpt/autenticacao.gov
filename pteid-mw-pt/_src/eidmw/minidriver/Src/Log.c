...
void LogInit() {
	...
	BOOL dwRet = ReadReg(TEXT("Software\\PTEID\\logging"), TEXT("log_level"), NULL, (LPBYTE)&lpData, &dwData);
	...
	dwRet = ReadReg(TEXT("Software\\PTEID\\logging"), TEXT("log_dirname"), NULL, (LPBYTE)&lpData, &dwData);
...
void LogTrace(int info, const char *pWhere, const char *format, ...) {
	...
	if (t != NULL) {
		...
		_snprintf(timebuf, sizeof(timebuf), "%02d/%02d/%04d - %02d:%02d:%02d", t->tm_mday, t->tm_mon + 1, uiYear,
	...
	if (fp != NULL) {
		fprintf(fp, "%s %d %d %s|%30s|%s\n", baseName, GetCurrentProcessId(), GetCurrentThreadId(), timebuf, pWhere,
				buffer);
		fclose(fp);
	}
}
...
void LogDumpBin(char *fileName, int iStreamLg, unsigned char *pa_cStream) {
	...
	if ((fileName == NULL) || (pa_cStream == NULL)) {
		return;
	}
