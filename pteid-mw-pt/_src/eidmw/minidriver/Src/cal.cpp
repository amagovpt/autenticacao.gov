...
long EidmwToScardErr(unsigned long lEidmwErr) {
	...
	case EIDMW_ERR_NO_CARD:
		lRet = SCARD_W_REMOVED_CARD;
		break;
	case EIDMW_ERR_CMD_NOT_ALLOWED:
		lRet = SCARD_E_NO_ACCESS;
		break;
	case EIDMW_ERR_CANT_CONNECT:
		lRet = SCARD_W_UNRESPONSIVE_CARD;
		break;
	case EIDMW_ERR_NO_READER:
		lRet = SCARD_E_NO_SERVICE;
		break;
	case EIDMW_ERR_CARD_RESET:
		lRet = SCARD_W_RESET_CARD;
		break;
	case EIDMW_ERR_CARD_SHARING:
		lRet = SCARD_E_SHARING_VIOLATION;
		break;
	case EIDMW_ERR_ALGO_BAD:
		lRet = SCARD_E_UNSUPPORTED_FEATURE;
		break;
	case EIDMW_ERR_NOT_TRANSACTED:
		lRet = SCARD_E_NOT_TRANSACTED;
		break;
	}
...
DWORD cal_read_pub_key(PCARD_DATA pCardData, DWORD dwCertSpec, DWORD *pcbPubKey, PBYTE *ppbPubKey) {
	...
	try {
		...
		unsigned char cmd[16] = {0x00, 0xCB, 0x00, 0xFF, 0x0A, 0xB6, 0x03, 0x83,
								 0x01, 0x08, 0x7F, 0x49, 0x02, 0x86, 0x00, 0x00};
		...
		*ppbPubKey = static_cast<PBYTE>(pCardData->pfnCspAlloc(*pcbPubKey));
		...
		// It seems the Microsoft documentation is wrong on this cbKey field for EC keys: "The length, in bytes, of the
		// key."
...
DWORD cal_sign_data(PCARD_DATA pCardData, BYTE container_id, DWORD cbToBeSigned, PBYTE pbToBeSigned,
