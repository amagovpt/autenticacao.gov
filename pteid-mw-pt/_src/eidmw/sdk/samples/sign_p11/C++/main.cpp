...
const char* signPkcs11( )
{
...
#ifdef WIN32
	HMODULE hLibrary=LoadLibrary(L"pteidpkcs11.dll");
	if(!hLibrary)
...
#else
#ifdef __APPLE__
	const char *P11_PATH = "/usr/local/lib/libpteidpkcs11.dylib";
#else
	const char *P11_PATH = "/usr/local/lib/libpteidpkcs11.so";
#endif
	void * dllPtr = NULL;
	...
	* error */
	...
	if (!dllPtr)
	{
		...
		return "Library libpteidpkcs11.so not found";
	}
