	...
	unsigned char *sigret, unsigned int *siglen, const RSA * rsa)
{
	...
	{
		MWLOG(LEV_ERROR, MOD_SCAP, "scapclient rsa_sign(): Exception caught in card.Sign. Aborting connection! Error code: %08x", e.GetError());
		return 0;
	}
...
static ECDSA_SIG * pteid_ecdsa_sign(const unsigned char *dgst, int dgstlen,
    ...
    {
        MWLOG(LEV_ERROR, MOD_SCAP, "pteid_ecdsa_sign(): Exception caught in card.Sign. Aborting connection! Error code: %08x", e.GetError());
        return NULL;
    }
