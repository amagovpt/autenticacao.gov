...
void ScapSettings::setScapServerHost(QString const &host)
{
...
std::string ScapSettings::getScapServerHost()
{
    return m_scap_server_host.toStdString();
}
...
std::string ScapSettings::getScapServerPort()
{
    return m_scap_server_port.toStdString();
}
...
void secureSaveSecretKey(const std::string &config_name, const std::string &secretKey) {
	...
	values[1] = CFStringCreateWithBytes(kCFAllocatorDefault, (const UInt8 *)config_name.data(),
				config_name.size(), kCFStringEncodingUTF8, FALSE);
	values[2] = CFSTR(KEYCHAIN_SERVICE_NAME);
	values[3] = CFStringCreateWithBytes(kCFAllocatorDefault, (const UInt8 *)secretKey.data(),
				secretKey.size(), kCFStringEncodingUTF8, FALSE);
	CFDictionaryRef secitem_attributes = CFDictionaryCreate
	(
		( CFAllocatorRef )NULL,
		( const void ** )keys,
		( const void ** )values,
...
void ScapSettings::setAppID(QString const &appID)
{
    m_appID = appID;
...
const QString ScapSettings::getAppID()
{
    return m_appID;
}

QString ScapSettings::getCacheDir()
{
    return m_cache_dir;
}
