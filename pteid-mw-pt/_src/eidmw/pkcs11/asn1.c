...
00 1 00000   20      6      type 0=primitive      contains primitive value, definite length encodeing
													1=boolean, 2=integer, 3=bitstring(padding), 4=octet string, 9=real,
10=enumerated, 12=IA5String

								 1=constructed    contains other TLV's,
												  definite or indefinite length encoding
												  values:
													16=sequence(of):ordered   set of items as prescribed in the
definition, optional items posible 17=set(of)     :unordered set of items                                , optional
items posible
...
11 0 00000   C0      7-8    Class:      Value:
							 ...
							 application  01     valid for one application; application number in lower 6 bits ex. APL.2
= 01 1 00010 context      10     depends on the context,    lower-bits=index private      11     custum defined
...
int skip_item(const unsigned char *p_cInDat,	/**< In: ASN-1 data */
	...
	for (iIndex = 1; iIndex < iPath; iIndex++) {
		...
		if ((*p_cDat == 0) && (*(p_cDat + 1) == 0)) {
			...
			if (p_cDat > p_cEnd) //???check this: if p_cDat==p_cEnd and length = 0, this could be the case for a NULL
								 // element, otherwise p_cDat should be < p_cEnd
...
int asn1_get_item(const unsigned char *p_cInDat, /**< In: ASN-1 data */
	...
	for (; *p_cPath; p_cPath++) {
		...
		/* in CER/DER: unused bits are always zero. And if they aren't zero, we still don't need to know the nr. of
		 * unused bits */
		if ((iNumTag == 0x03)) {
			p_cDat++;
			iLen--;
		...
		if ((*p_cDat == 0) && (*(p_cDat + 1) == 0)) {
			...
			if (iNumTag == TAG_MASK) {
				...
				do {
					...
					if (p_cDat >
						p_cRawDat +
							4) // multi-byte tag should be max 4 bytes with this implementation (ifnot overflow iNumTag)
						return (E_ASN_TAG_LEN);
	...
	p_xItem->l_data = iLen;
	/* construct tag: bit 1 and 2 for class, bit 3 for primitiv or structured and tagnum�ber from bit 4-32 = 29 bits for
	 * number encoding */
...
int asn1_next_item(ASN1_ITEM *p_xLev0Item, /**< In/out: object   */
	...
	if (iReturn == 0) {
		if (p_xLev1Item->l_raw > p_xLev0Item->l_data)
			p_xLev1Item->l_raw = p_xLev0Item->l_data; // make sure not to pass the end
