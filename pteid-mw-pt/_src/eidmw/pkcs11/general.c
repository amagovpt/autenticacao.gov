 ...
 * Copyright (C) 2012-2013, 2016-2018 André Guerreiro - <aguerreiro1985@gmail.com>
					...
					CK_ULONG_PTR pulCount)	  /* receives the number of slots */
{
	...
	// BUG in Adobe Acrobat reader: adobe asks for slots with pSlotList = NULL, so only nr of slots will be returned.
	// This is ok. a second time Adobe calls this, pSlotList still is NULL, so the array with SlotIDs cannot be
	// returned, again, nr of slots is returned. Adobe just assumes that the first slot has ID=0 !!! and uses this ID=0
	// for all further actions. to overcome this problem, we start our SlotIDs from 0 and not 1 !!!
...
CK_FUNCTION_LIST pkcs11_function_list = {{2, 11},
										 C_Initialize,
										 C_Finalize,
										 C_GetInfo,
										 C_GetFunctionList,
