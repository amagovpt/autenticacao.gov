						  ...
						  CK_ULONG ulCount)			  /* attributes in template */
{
	...
	*/
	...
	/*   if (pSlot->login_type < 0) //CKU_SO=0; CKU_USER=1
	   {
	   if (p11_get_attribute_value(pObject->pAttr, pObject->count, CKA_PRIVATE, (CK_VOID_PTR *) &pbPrivate, &len) !=
	   CKR_OK)
		  {
	...
	for (j = 0; j < ulCount; j++) {
		status =
			p11_get_attribute_value(pObject->pAttr, pObject->count, pTemplate[j].type, (CK_VOID_PTR *)&pValue, &len);
		if (status != CKR_OK) {
						...
						CK_ULONG ulCount)			/* attributes in search template */
{
	...
	/* reason to add this is that e.g. Firefox for every certificate in its store starts a find operation with
	 * CKA_CLASS_TYPE */
	...
	/* Sun-PKCS11 cannot handle  CKR_ATTRIBUTE_VALUE_INVALID properly so => initialize search and findObjects will just
	   return 0 matching objects in case of CKO_DATA */
	if (ulCount) {
					...
					CK_ULONG_PTR pulObjectCount)   /* actual number returned */
{
	...
	*/
	...
	/* VSC this code was moved to here since Sun-PKCS11 cannot handle CKR_Attribute_value_invalid in C_FindObjectsInit()
	 * properly!!! */
	...
	// Firefox does not set TOKEN object for CKO_PRIVATE_KEY objects so for the moment we allow looking for them, id has
	// to match anyway.
	len = sizeof(CK_BBOOL);
	if (pData->size > 0) {
		...
		// for the moment if CKA_TOKEN is specified and set to false, we reply that only token objects can be searched
		// for and continue with ok
		if ((ret == CKR_OK) && (len == sizeof(CK_BBOOL)) && (*pbToken == CK_FALSE)) {
	...
	*pulObjectCount = 0;
	...
	for (h = pData->hCurrent; h <= (pSlot->nobjects) && (*pulObjectCount < ulMaxObjectCount); h++, pData->hCurrent++) {
      ...
      if (pSlot->login_type < 0) //CKU_SO=0; CKU_USER=1
		...
		match = 1;
		// if pData->size = 0 => this means that we will search for every object!!! match = 1 so for() is skipped and
		// object is returned
