...
public:
	...
	//------------------------------------------------------
	GUISettings(void)
		: m_bProxySystem(false), m_proxy_port(0), m_GuiLanguage("nl"), m_telemetry_id("0"), m_telemetry_host("0"),
		  m_telemetry_status(8) /* TelemetryStatus::Disabled defined in gapi.h */
		  ,
		  m_bShowAnimations(false), m_bUseSystemScale(false), m_iApplicationScale(0), m_iGraphicsAccel(0),
		  m_bUseCustomSignature(false), m_bPinpadEnabled(false), m_bStartAutoupdate(false), m_bDebugMode(false),
	...
	// Read the setting
	//------------------------------------------------------
	const QString &getGuiLanguageString(void) const { return m_GuiLanguage; }

	void setGuiLanguage(const QString &GuiLanguage = STR_DEF_GUILANGUAGE) { m_GuiLanguage = GuiLanguage; }
	void setLanguage(const QString &language) {
		m_GuiLanguage = language;
		eIDMW::PTEID_Config config(eIDMW::PTEID_PARAM_GENERAL_LANGUAGE);
		config.setString(m_GuiLanguage.toLatin1());
	}
	bool getPinpadEnabled(void) const { return m_bPinpadEnabled; }
	...
	bool getStartAutoupdate(void) const { return m_bStartAutoupdate; }
	...
	QString setDebugMode(bool bDebugMode) {
		...
		if (bDebugMode) {
...
#ifdef WIN32
			QFile debugFileDesktop(QStandardPaths::writableLocation(QStandardPaths::DesktopLocation) +
								   "\\pteid-debug.conf");
			if (debugFileDesktop.exists() && !debugFileDesktop.remove()) {
				eIDMW::PTEID_LOG(eIDMW::PTEID_LOG_LEVEL_ERROR, "eidgui",
								 "setDebugMode: Could not delete desktop debug file at:");
				eIDMW::PTEID_LOG(eIDMW::PTEID_LOG_LEVEL_ERROR, "eidgui",
								 debugFileDesktop.fileName().toLocal8Bit().data());
			...
			if (debugFileDesktop.exists() && !debugFileSystem.remove()) {
				eIDMW::PTEID_LOG(eIDMW::PTEID_LOG_LEVEL_ERROR, "eidgui",
								 "setDebugMode: Could not delete system debug file at:");
				eIDMW::PTEID_LOG(eIDMW::PTEID_LOG_LEVEL_ERROR, "eidgui",
								 debugFileSystem.fileName().toLocal8Bit().data());
	...
	bool getAskToRegisterCmdCert(void) { return m_bAskToRegisterCmdCert; }
	...
	bool getAskToSetCache(void) { return m_bAskToSetCache; }

	bool getAskToSetTelemetry(void) { return m_bAskToSetTelemetry; }
	...
	QString getTelemetryId() { return m_telemetry_id; }
	...
	QString getTelemetryHost() { return m_telemetry_host; }
	...
	long getTelemetryStatus() { return m_telemetry_status; }
	...
	bool getShowAnimations(void) { return m_bShowAnimations; }
	bool getUseSystemScale(void) { return m_bUseSystemScale; }
	int getApplicationScale(void) { return m_iApplicationScale; }
	int getGraphicsAccel(void) { return m_iGraphicsAccel; }
	...
	bool getUseCustomSignature(void) { return m_bUseCustomSignature; }
	...
	bool getNotShowHelpStartUp(void) { return m_bNotShowStartUpHelp; }
	...
	bool getRemoveCertSetting(void) { return m_bRemoveCert; }
	...
	QString getTimeStampHost(void) { return m_time_stamp_host; }

	void setTimeStampHost(const QString &timeStamp_host) {
	...
	void setGuiVersion(const QString &GUIVersion) { m_GUIVersion = GUIVersion; }
	const QString &getGuiVersion(void) { return m_GUIVersion; }

	bool getProxySystem(void) { return m_bProxySystem; }
	...
	void setProxyUsername(const QString &proxy_user) {
		m_proxy_username = proxy_user;
	...
	QString getProxyUsername() { return m_proxy_username; }

	QString getProxyPwd() { return m_proxy_pwd; }

	long getProxyPort() { return m_proxy_port; }
	...
	QString getPteidCachedir() { return m_pteid_cachedir; }
	...
	/* Test Mode */
	bool getTestMode() { return m_test_mode; }

	void setTestMode(bool test_mode) { m_test_mode = test_mode; }

	/* GUI Signature Options */
	bool getShowSignatureOptions() { return m_bShowSignatureOptions; }
	...
	/* GUI Signature Help */
	bool getShowSignatureHelp() { return m_bShowSignatureHelp; }
	...
	bool getUseNumId(void) { return (m_iSignSealOptions & SIGN_SEAL_NUM_ID_OPTION); }

	bool getUseDate(void) { return (m_iSignSealOptions & SIGN_SEAL_DATE_OPTION); }
	...
	void setAdminConfiguration(bool adminConfiguration) { m_showAdminConfiguration = adminConfiguration; }

	bool getAdminConfiguration(void) { return m_showAdminConfiguration; }
...
private:
	...
	bool m_bNotShowStartUpHelp;	  //!< the GUI Show Help	bool	m_bStartMinimized;              //!< startup minimized
								  //!< (T/F)
