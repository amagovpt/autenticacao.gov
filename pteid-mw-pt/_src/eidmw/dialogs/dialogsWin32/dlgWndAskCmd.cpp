...
dlgWndAskCmd::dlgWndAskCmd(DlgCmdOperation operation, bool isValidateOtp, std::wstring &Header, std::wstring *inId,
						   std::wstring *userName, HWND Parent, std::function<void(void)> *fSendSmsCallback,
						   bool askForId)
	...
	if (CreateWnd(tmpTitle.c_str(), Width, Height, IDI_APPICON, Parent)) {
		...
		HWND hTitle = PteidControls::CreateText(contentX, paddingY, titleWidth, titleHeight, m_hWnd,
		...
		// INSERT USERID TEXT EDIT
			...
			std::wstring link =
				L"<A HREF=\"https://www.autenticacao.gov.pt/cmd-pedido-chave\">" + clickToActivateText + L" </A>";
			...
			PteidControls::CreateHyperlink(contentX, linkY, contentWidth, editLabelHeight, m_hWnd, (HMENU)IDC_SYSLINK,
										   m_hInstance, &linkData);
			...
			PteidControls::CreateText(contentX, editIdY, contentWidth, editLabelHeight, m_hWnd, (HMENU)IDC_STATIC_TITLE,
									  m_hInstance, &mobileNumberFieldData);
			...
			PteidControls::CreateComboBox(contentX, editIdY + editLabelHeight, editIdX - contentX - boxSeparationSpace,
										  editFieldHeight - editLabelHeight, m_hWnd, (HMENU)IDC_EDIT_PREFIX_MOBILE,
										  m_hInstance, &mobilePrefixData);
			...
			HWND hIdTextEdit =
				PteidControls::CreateTextField(editIdX, editIdY + editLabelHeight, contentWidth - editIdX + contentX,
		...
		HWND hTextEdit =
			PteidControls::CreateTextField(contentX, editCodeY + editLabelHeight, contentWidth, editFieldHeight, m_hWnd,
										   (HMENU)IDC_EDIT_CODE, m_hInstance, &textFieldCodeData);
...
dlgWndAskCmd::~dlgWndAskCmd() { KillWindow(); }
...
LRESULT dlgWndAskCmd::ProcecEvent(UINT uMsg,	 // Message For This Window
								  ...
								  LPARAM lParam) // Additional Message Information
{
			...
			if (EN_CHANGE == HIWORD(wParam)) {
				if (m_askForId) {
					okBtnProcData.setEnabled(textFieldCodeData.isAcceptableInput() &&
											 textFieldIdData.isAcceptableInput());
				} else {
					okBtnProcData.setEnabled(textFieldCodeData.isAcceptableInput());
				}
			}
	...
	case WM_ACTIVATE: {
		MWLOG(LEV_DEBUG, MOD_DLG, L"  --> dlgWndAskCmd::ProcecEvent WM_ACTIVATE (wParam=%X, lParam=%X)", wParam,
			  lParam);
		break;
	}

	case WM_NCACTIVATE: {
		MWLOG(LEV_DEBUG, MOD_DLG, L"  --> dlgWndAskCmd::ProcecEvent WM_NCACTIVATE (wParam=%X, lParam=%X)", wParam,
			  lParam);
	...
	case WM_SETFOCUS: {
		MWLOG(LEV_DEBUG, MOD_DLG, L"  --> dlgWndAskCmd::ProcecEvent WM_SETFOCUS (wParam=%X, lParam=%X)", wParam,
			  lParam);
		break;
	}

	case WM_KILLFOCUS: {
		MWLOG(LEV_DEBUG, MOD_DLG, L"  --> dlgWndAskCmd::ProcecEvent WM_KILLFOCUS (wParam=%X, lParam=%X)", wParam,
			  lParam);
		break;
	}
