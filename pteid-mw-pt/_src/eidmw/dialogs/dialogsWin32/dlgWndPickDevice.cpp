...
dlgWndPickDevice::dlgWndPickDevice(HWND Parent) : Win32Dialog(L"WndPickDevice") {
	...
	if (CreateWnd(title.c_str(), Width, Height, IDI_APPICON, Parent)) {
		...
		HWND hTitle = PteidControls::CreateText(contentX, titleY, contentWidth, (int)(clientRect.bottom * 0.15), m_hWnd,
		...
		HWND hRadioButtons = PteidControls::CreateRadioButtonGroup(
			contentX, radioButtonsY, contentWidth, (int)(clientRect.bottom * 0.40), m_hWnd, (HMENU)IDC_RADIO_DEVICE,
			m_hInstance, &radioGroupBtnData);
...
dlgWndPickDevice::~dlgWndPickDevice() { KillWindow(); }
...
LRESULT dlgWndPickDevice::ProcecEvent(UINT uMsg,	 // Message For This Window
									  ...
									  LPARAM lParam) // Additional Message Information
{
	...
	case WM_SIZE: {
		MWLOG(LEV_DEBUG, MOD_DLG, L"  --> dlgWndPickDevice::ProcecEvent WM_SIZE (wParam=%X, lParam=%X)", wParam,
			  lParam);
	...
	case WM_NCACTIVATE: {
		MWLOG(LEV_DEBUG, MOD_DLG, L"  --> dlgWndPickDevice::ProcecEvent WM_NCACTIVATE (wParam=%X, lParam=%X)", wParam,
			  lParam);
	...
	case WM_CLOSE: {
		MWLOG(LEV_DEBUG, MOD_DLG, L"  --> dlgWndPickDevice::ProcecEvent WM_CLOSE (wParam=%X, lParam=%X)", wParam,
			  lParam);
