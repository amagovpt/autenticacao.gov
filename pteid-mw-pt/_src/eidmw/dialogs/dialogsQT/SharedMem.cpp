...
SharedMem::~SharedMem() { Delete(m_iShmid); }

void SharedMem::Attach(size_t tMemorySize, const char *csReadableFilePath, void **content) {
...
void SharedMem::Delete(int iSegmentID) {
	...
	if (iAttachedProcs > -1) {
		...
		MWLOG(LEV_DEBUG, MOD_DLG, L"  SharedMem::Delete : request to delete memory segment with ID %d. N proc = %d",
