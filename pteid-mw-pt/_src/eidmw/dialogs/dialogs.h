...
typedef enum { DLG_CMD_SIGNATURE, DLG_CMD_GET_CERTIFICATE } DlgCmdOperation;
 ...
 */
DLGS_EXPORT DlgRet DlgAskPin(DlgPinOperation operation, DlgPinUsage usage, const wchar_t *csPinName, DlgPinInfo pinInfo,
							 wchar_t *csPin, unsigned long ulPinBufferLen, void *wndGeometry = 0);
 ...
 *          DLG_BAD_PARAM or DLG_ERR otherwise
 */
DLGS_EXPORT DlgRet DlgAskPins(DlgPinOperation operation, DlgPinUsage usage, const wchar_t *csPinName,
							  DlgPinInfo pin1Info, wchar_t *csPin1, unsigned long ulPin1BufferLen, DlgPinInfo pin2Info,
							  wchar_t *csPin2, unsigned long ulPin2BufferLen, void *wndGeometry = 0);
 ...
 *          DLG_BAD_PARAM or DLG_ERR otherwise
 */
DLGS_EXPORT DlgRet DlgDisplayPinpadInfo(DlgPinOperation operation, const wchar_t *csReader, DlgPinUsage usage,
										const wchar_t *csPinName, const wchar_t *csMessage, unsigned long *pulHandle,
										void *wndGeometry = 0);
...
DLGS_EXPORT DlgRet DlgAskInputCMD(DlgCmdOperation operation, bool isValidateOtp, wchar_t *csOutCode,
								  unsigned long ulOutCodeBufferLen, wchar_t *csInOutId = NULL,
								  unsigned long csOutIdLen = 0, const wchar_t *csUserName = NULL,
								  unsigned long ulUserNameBufferLen = 0,
...
DLGS_EXPORT DlgRet DlgCMDMessage(DlgCmdOperation operation, DlgCmdMsgType msgType, const wchar_t *message,
								 unsigned long *pulHandle);
/**
 * Show CMD dialog message for "sending Otp" (isOtp == true) or "connecting with server" (isOtp == false).
 */
DLGS_EXPORT DlgRet DlgCMDMessage(DlgCmdOperation operation, DlgCmdMsgType msgType, bool isOtp,
								 unsigned long *pulHandle);
