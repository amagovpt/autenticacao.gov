	...
	if (pRevokeds) {
		...
		for (int i = 0; i < sk_X509_REVOKED_num(pRevokeds); i++) {
			...
			ASN1_ENUMERATED *Revocation_reason =
				(ASN1_ENUMERATED *)X509_REVOKED_get_ext_d2i(current_revoked, NID_crl_reason, &crit, NULL);

			// Converts to long
			long Revocation_reason_long = ASN1_ENUMERATED_get(Revocation_reason);

			// Prints the certificates and the Revocation reason
			cout << "Certificate Serial Number = " << serial_int64
				 << " | Revocation Reason = " << getRevocationReason(Revocation_reason_long) << endl;
		}
	}
...
int main(int argc, char *argv[]) {
	...
	vector<const char *> test_descriptions = {"CRL: Hold -> Delta: Revoked", "CRL: None -> Delta: Revoked",
											  "CRL: None -> Delta: Hold", "CRL: Hold -> Delta: RemoveFromCRL",
											  "Serial number not in CRL"};

	// Gets serial numbers
	vector<uint64_t> serial_nums = {5709674716273064001, 1273658538371892772, 3993204469349930623, 6961655884629467962,
									42};

	// Get expected results
	vector<FWK_CertifStatus> expected_results = {FWK_CERTIF_STATUS_REVOKED, FWK_CERTIF_STATUS_REVOKED,
												 FWK_CERTIF_STATUS_SUSPENDED, FWK_CERTIF_STATUS_VALID,
												 FWK_CERTIF_STATUS_VALID};
