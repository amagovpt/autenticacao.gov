...
TEST(GetPhoto)
{
	PTEID_ReaderContext &reader0=ReaderSet.getReaderByNum(0);	
	PTEID_EIDCard &card0=reader0.getEIDCard();	
	PTEID_Picture &photo0=card0.getPicture();
	card0.setAllowTestCard(true);
	...
	PTEID_ReaderContext &reader2=ReaderSet.getReaderByNum(2);	
	PTEID_EIDCard &card2=reader2.getEIDCard();				
	PTEID_Picture &photo2=card2.getPicture();
	...
	PTEID_ReaderContext &reader3=ReaderSet.getReaderByNum(3);	
	PTEID_EIDCard &card3=reader3.getEIDCard();				
	PTEID_Picture &photo3=card3.getPicture();
	card3.setAllowTestCard(true);
...
TEST(GetDocument)
{
	...
	PTEID_EIDCard &card0=reader0.getEIDCard();	
	card0.setAllowTestCard(true);
	PTEID_XMLDoc &doc0a=card0.getDocument(PTEID_DOCTYPE_ID);
	CHECK(&doc0a);
	PTEID_XMLDoc &doc0b=card0.getDocument(PTEID_DOCTYPE_PICTURE);
	CHECK(&doc0b);


	PTEID_ReaderContext &reader4=ReaderSet.getReaderByNum(4);	
	PTEID_SISCard &card4=reader4.getSISCard();				
	PTEID_XMLDoc &doc4a=card4.getDocument(PTEID_DOCTYPE_ID);
	CHECK(&doc4a);

	try
	{
		PTEID_XMLDoc &doc4b=card4.getDocument(PTEID_DOCTYPE_PICTURE); //Must go to the catch
		CHECK(0);					//No picture on Sis card
	}
	catch(PTEID_ExDocTypeUnknown e)
	{
...
TEST(ReadWriteFile)
{
	...
	//Replace the photo
	PTEID_ByteArray photoChange(_photo2,sizeof(_photo2));
	try
	{
		card0.writeFile("3F00DF014035",photoChange);
		CHECK(0);					//Write not allowed
	}
	catch(PTEID_ExCmdNotAllowed e)
	{
	...
	//Replace the pref
	PTEID_ByteArray prefChange(_prefChange,sizeof(_prefChange));
	card0.writeFile("3F00DF014039",prefChange);

	//Get the pref again
	PTEID_ByteArray prefAfter;
...
TEST(EID_XML)
{
	...
	{
		PTEID_XMLDoc &doc=card.getPicture();
		//std::cout << doc.getXML();
	...
	{
		PTEID_XMLDoc &doc=card.getVersionInfo();
		//std::cout << doc.getXML();
	...
	{
		PTEID_XMLDoc &doc=card.getDocument(PTEID_DOCTYPE_PINS);
		//std::cout << doc.getXML();
	...
	{
		PTEID_XMLDoc &doc=card.getDocument(PTEID_DOCTYPE_CERTIFICATES);
		//std::cout << doc.getXML();
