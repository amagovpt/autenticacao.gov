...
TEST(VirtualRAW_Sis)
{
	...
	PTEID_SISCard *card=&reader.getSISCard();

	PTEID_RawData_Sis RawData;

	RawData.idData.Append(card->getRawData_Id());

	PTEID_SISCard *card_virtual=NULL;
	...
	PTEID_SisId &doc_virtual=card_virtual->getID();
	CHECK_EQUAL("HOEBEN",doc_virtual.getSurname());
}
...
TEST(VirtualXML_Sis)
{
	...
	PTEID_ReaderContext reader_virtual(PTEID_FILETYPE_XML,"sis.xml");
	try
	{
		card_virtual=&reader_virtual.getSISCard();
		CHECK(card_virtual);
	...
	PTEID_SisId &docId_virtual=card_virtual->getID();
	CHECK_EQUAL("HOEBEN",docId_virtual.getSurname());
...
TEST(VirtualCSV_Sis)
{
	...
	PTEID_ReaderContext reader_virtual(PTEID_FILETYPE_CSV,"sis.csv");
	try
	{
		card_virtual=&reader_virtual.getSISCard();
		CHECK(card_virtual);
...
TEST(VirtualTLV_Sis)
{
	...
	PTEID_ReaderContext reader_virtual(PTEID_FILETYPE_TLV,"sis.tlv");
	try
	{
		card_virtual=&reader_virtual.getSISCard();
		CHECK(card_virtual);
