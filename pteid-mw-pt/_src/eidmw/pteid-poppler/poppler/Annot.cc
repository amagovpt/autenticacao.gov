...
void Annot::drawCircle(double cx, double cy, double r, GBool fill) {
	...
	appearBuf->appendf("{0:.2f} {1:.2f} {2:.2f} {3:.2f} {4:.2f} {5:.2f} c\n",
			cx + r, cy + bezierCircle * r, cx + bezierCircle * r, cy + r, cx,
			cy + r);
	appearBuf->appendf("{0:.2f} {1:.2f} {2:.2f} {3:.2f} {4:.2f} {5:.2f} c\n",
			cx - bezierCircle * r, cy + r, cx - r, cy + bezierCircle * r,
			cx - r, cy);
	appearBuf->appendf("{0:.2f} {1:.2f} {2:.2f} {3:.2f} {4:.2f} {5:.2f} c\n",
			cx - r, cy - bezierCircle * r, cx - bezierCircle * r, cy - r, cx,
			cy - r);
...
void Annot::draw(Gfx *gfx, GBool printing) {
	...
	gfx->drawAnnot(&obj, (AnnotBorder *) NULL, color, rect->x1, rect->y1,
			rect->x2, rect->y2);
	obj.free();
}
...
void AnnotText::draw(Gfx *gfx, GBool printing) {
	...
	if (appearance.isNull()) {
		...
		PDFRectangle
				fixedRect(rect->x1, rect->y1, rect->x1 + 24, rect->y1 + 24);
...
void AnnotLink::draw(Gfx *gfx, GBool printing) {
	...
	// draw the appearance stream
	appearance.fetch(xref, &obj);
	gfx->drawAnnot(&obj, border, color, rect->x1, rect->y1, rect->x2, rect->y2);
	obj.free();
}
...
void AnnotFreeText::generateFreeTextAppearance() {
	...
	appearBuf->appendf("BT 1 0 0 1 {0:.2f} {1:.2f} Tm\n", textmargin,
			height - textmargin - fontsize * font->getDescent());
	...
	while (i < contents->getLength()) {
		...
		layoutText(contents, &out, &i, font, &linewidth, textwidth / fontsize,
				NULL, gFalse);
		linewidth *= fontsize;
		switch (quadding) {
...
void AnnotLine::generateLineAppearance() {
	...
	if (caption) {
		...
		appearBuf->appendf(
				"{0:.2f} {1:.2f} {2:.2f} {3:.2f} {4:.2f} {5:.2f} Tm\n",
...
void AnnotTextMarkup::draw(Gfx *gfx, GBool printing) {
	...
	if (appearance.isNull() || type == typeHighlight) {
		...
		for (i = 0; i < quadrilaterals->getQuadrilateralsLength(); ++i) {
			appearBBox->extendTo(quadrilaterals->getX1(i) - rect->x1,
					quadrilaterals->getY1(i) - rect->y1);
			appearBBox->extendTo(quadrilaterals->getX2(i) - rect->x1,
					quadrilaterals->getY2(i) - rect->y1);
			appearBBox->extendTo(quadrilaterals->getX3(i) - rect->x1,
					quadrilaterals->getY3(i) - rect->y1);
			appearBBox->extendTo(quadrilaterals->getX4(i) - rect->x1,
					quadrilaterals->getY4(i) - rect->y1);
		}
		...
		case typeHighlight:
			...
			for (i = 0; i < quadrilaterals->getQuadrilateralsLength(); ++i) {
				...
				appearBuf->appendf("{0:.2f} {1:.2f} m\n", x3, y3);
				appearBuf->appendf(
						"{0:.2f} {1:.2f} {2:.2f} {3:.2f} {4:.2f} {5:.2f} c\n",
		...
		appearBuf = new GooString("/GS0 gs\n/Fm0 Do");
		createResourcesDict("Fm0", &aStream, "GS0", 1,
				blendMultiply ? "Multiply" : NULL, &resDict);
		if (ca == 1) {
			createForm(bbox, gFalse, &resDict, &appearance);
		} else {
...
void Annot::layoutText(GooString *text, GooString *outBuf, int *i,
		GfxFont *font, double *width, double widthLimit, int *charCount,
		GBool noReencode) {
	CharCode c;
	Unicode uChar, *uAux;
	double w = 0.0;
	...
	while (*i < text->getLength()) {
		...
		if (uChar == '\r' || uChar == '\n') {
			...
			if (uChar == '\r' && *i < text->getLength()) {
				if (unicode && text->getChar(*i) == '\0' && text->getChar(
						*i + 1) == '\n')
					*i += 2;
		...
		if (noReencode) {
			...
			if (!ccToUnicode) {
				...
				ccToUnicode->decRefCnt();
				fprintf(stderr, "warning: layoutText: cannot convert U+%04X\n",
						uChar);
			}
		}
		...
		if (outBuf->getLength() > last_o2) {
			...
			font->getNextChar(outBuf->getCString() + last_o2,
					outBuf->getLength() - last_o2, &c, &uAux, &uLen, &dx, &dy,
					&ox, &oy);
			w += dx;
		}
	...
	if (widthLimit > 0.0 && w > widthLimit) {
		if (unicode) {
			while (*i < text->getLength() && text->getChar(*i) == '\0'
					&& text->getChar(*i + 1) == ' ')
				*i += 2;
			if (*i < text->getLength() && text->getChar(*i) == '\0'
					&& text->getChar(*i + 1) == '\r')
				*i += 2;
			if (*i < text->getLength() && text->getChar(*i) == '\0'
					&& text->getChar(*i + 1) == '\n')
...
void AnnotWidget::drawText(GooString *text, GooString *da,
		GfxResources *resources, GBool multiline, int comb, int quadding,
		GBool txField, GBool forceZapfDingbats, GBool password) {
	GooList *daToks;
	...
	if (tfPos >= 0) {
		...
		if (tok->getLength() >= 1 && tok->getChar(0) == '/') {
			if (!resources || !(font = resources->lookupFont(
					tok->getCString() + 1))) {
				if (forceZapfDingbats) {
					...
					font = new Gfx8BitFont(xref, "ZaDb", r,
							new GooString("ZapfDingbats"), fontType1, r, d);
					delete d;
					freeFont = gTrue;
	...
	if (multiline) {
		...
		if (fontSize == 0) {
			for (fontSize = 20; fontSize > 1; --fontSize) {
				...
				i = 0;
				while (i < text->getLength()) {
					layoutText(text, convertedText, &i, font, &w,
							wMax / fontSize, NULL, forceZapfDingbats);
					y -= fontSize;
				}
		...
		xPrev = 0;
		while (i < text->getLength()) {
			layoutText(text, convertedText, &i, font, &w, wMax / fontSize,
					NULL, forceZapfDingbats);
			w *= fontSize;
		...
		if (comb > 0) {
			...
			while (i < comb && len > 0) {
				...
				dx = 0.0;
				n = font->getNextChar(s, len, &code, &uAux, &uLen, &dx, &dy,
						&ox, &oy);
				dx *= fontSize;
...
// Draw the variable text or caption for a field.
void AnnotWidget::drawListBox(FormFieldChoice *fieldChoice, GooString *da,
		GfxResources *resources, int quadding) {
	GooList *daToks;
	GooString *tok, *convertedText;
	GfxFont *font;
	...
	if (fontSize == 0) {
		...
		for (i = 0; i < fieldChoice->getNumChoices(); ++i) {
			j = 0;
			layoutText(fieldChoice->getChoice(i), convertedText, &j, font, &w,
					0.0, NULL, gFalse);
			if (w > wMax) {
				wMax = w;
			}
	...
	for (i = fieldChoice->getTopIndex(); i < fieldChoice->getNumChoices(); ++i) {
		...
		if (fieldChoice->isSelected(i)) {
			...
			appearBuf->appendf("{0:.2f} {1:.2f} {2:.2f} {3:.2f} re f\n",
					borderWidth, y - 0.2 * fontSize,
					rect->x2 - rect->x1 - 2 * borderWidth, 1.1 * fontSize);
		}
...
void AnnotWidget::drawBorder() {
	...
	if (field->getType() == formButton
			&& static_cast<FormFieldButton*> (field)->getButtonType()
					== formButtonRadio && !hasCaption) {
	...
	case formButtonCheck:
		if (appearState && appearState->cmp("Off") != 0) {
			if (!caption) {
				...
				drawText(&checkMark, da, resources, gFalse, 0, fieldQuadCenter,
						gFalse, gTrue);
			} else {
				drawText(caption, da, resources, gFalse, 0, fieldQuadCenter,
						gFalse, gTrue);
...
void AnnotWidget::drawFormFieldText(GfxResources *resources, GooString *da) {
	...
	if (contents) {
		quadding = field->hasTextQuadding() ? field->getTextQuadding()
				: form->getTextQuadding();

		int comb = 0;
		if (fieldText->isComb())
			comb = fieldText->getMaxLen();

		drawText(contents, da, resources, fieldText->isMultiline(), comb,
				quadding, gTrue, gFalse, fieldText->isPassword());
	}
}
...
void AnnotWidget::drawFormFieldChoice(GfxResources *resources, GooString *da) {
	...
	if (fieldChoice->isCombo()) {
		...
		if (selected) {
			drawText(selected, da, resources, gFalse, 0, quadding, gTrue,
					gFalse);
...
void AnnotWidget::draw(Gfx *gfx, GBool printing) {
	...
	if (field) {
		if (appearance.isNull() || (form && form->getNeedAppearances())
				|| ((field->getType() == formText || field->getType()
						== formChoice) && field->isModified()))
...
void AnnotGeometry::draw(Gfx *gfx, GBool printing) {
	...
	if (appearance.isNull()) {
		...
		if (border) {
			...
			if (interiorColor && interiorColor->getSpace()
...
void AnnotPolygon::draw(Gfx *gfx, GBool printing) {
	...
	if (appearance.isNull()) {
		...
		if (vertices->getCoordsLength() != 0) {
			...
			appearBBox->extendTo(vertices->getX(0) - rect->x1,
					vertices->getY(0) - rect->y1);

			for (int i = 1; i < vertices->getCoordsLength(); ++i) {
				appearBuf->appendf("{0:.2f} {1:.2f} l\n",
						vertices->getX(i) - rect->x1,
						vertices->getY(i) - rect->y1);
				appearBBox->extendTo(vertices->getX(i) - rect->x1,
						vertices->getY(i) - rect->y1);
			}
