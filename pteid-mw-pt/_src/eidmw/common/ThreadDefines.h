...
#define THREAD_START(function, arg, hThread, syncHandle, status)                                                       \
	{                                                                                                                  \
		status = pthread_create(&hThread, NULL, function, arg) ? CRYPT_ERROR : CRYPT_OK;                               \
		syncHandle = hThread;                                                                                          \
	}

#define THREAD_END(sync) pthread_exit((void *)0)

#if (defined(__osf__) || defined(__alpha__)) && !defined(pthread_self)
...
#define THREAD_START(function, arg, hThread, syncHandle, status)                                                       \
	{                                                                                                                  \
		...
		hThread = taskSpawn(NULL, T_PRIORITY, TASK_ATTRIBUTES, 16384, function, (int)arg, 0, 0, 0, 0, 0, 0, 0, 0, 0);  \
		if (hThread == ERROR) {                                                                                        \
...
#define THREAD_INITFUNCTION(parm1, arg) unsigned __stdcall parm1(void *arg)
#if defined(_MSC_VER) && VC_GE_2005(_MSC_VER)
#define THREAD_START(function, arg, hThread, syncHandle, status)                                                       \
	{                                                                                                                  \
		uintptr_t oThread;                                                                                             \
		oThread = _beginthreadex(NULL, 0, (function), (arg), 0, &hThread);                                             \
		syncHandle = oThread;                                                                                          \
...
#define THREAD_INITFUNCTION(parm1, arg) DWORD WINAPI parm1(void *arg)
#define THREAD_START(function, arg, hThread, syncHandle, status)                                                       \
	{                                                                                                                  \
		HANDLE oThread;                                                                                                \
		oThread = CreateThread(NULL, 0, (function), (arg), 0, &hThread);                                               \
		syncHandle = oThread;                                                                                          \
...
#define THREAD_OBJ int
#define THREAD_ID int
#define THREAD_START(function, arg, hThread, syncHandle, status) status = CRYPT_ERROR
#define THREAD_END(sync)
