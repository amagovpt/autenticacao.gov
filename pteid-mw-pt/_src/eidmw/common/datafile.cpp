...
bool CDataFile::Load() { return Load(false); }

// Load with locking in order to write the file
bool CDataFile::LoadAndLock() { return Load(true); }
...
bool CDataFile::Load(bool bLock) {
	...
	if (bLock) {
		...
		if (fcntl(fileno(m_stream), F_SETLKW, &m_tFl) == -1) { /* set the lock, waiting if necessary */
...
// Returns the key value as a t_Str object. A return value of
// t_Str(L"") indicates that the key could not be found.
t_Str CDataFile::GetString(t_Str szKey, t_Str szSection) { return GetValue(szKey, szSection); }
...
bool CDataFile::GetBool(t_Str szKey, t_Str szSection) {
	...
	if (szValue.find(L"1") == 0 || CompareNoCase(szValue, L"true") || CompareNoCase(szValue, L"yes")) {
		bValue = true;
	}
...
size_t CDataFile::SectionCount() { return m_Sections.size(); }
