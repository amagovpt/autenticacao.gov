...
- You can specify a default value for each Getxxxx.  The default value is returned in case that the specified key does
not exist.
- Getxxx will first search in the HKEY_CURRENT_USER/CurrentUser  section,
  ...
  -> overview of the functional requirements in function of the storage location: Windows registry or ini-file
(apple/linux) <PRE>
...
#define EIDMW_CNF_MACRO_HOME                                                                                           \
	L"$home" // user home directory, �$common� is taken when used for processes without user (eg. services)
#define EIDMW_CNF_MACRO_COMMON                                                                                         \
	L"$common" // directory readable/writable for every user, used for shared files like the CRL�s
...
#define EIDMW_CNF_GENERAL_TELEMETRY_STATUS L"telemetry_status"
#define EIDMW_CNF_GENERAL_INSTALLDIR                                                                                   \
	L"install_dirname" // string, directory of the basic eid software; �c:/Program Files/pteid/�
...
#define EIDMW_CNF_GENERAL_CARDTXDELAY                                                                                  \
	L"card_transmit_delay" // number, delay while communicating with the smartcard, in mili-seconds, default 1 mSec
#define EIDMW_CNF_GENERAL_CARDCONNDELAY                                                                                \
	L"card_connect_delay" // number, delay before connecting to a smartcard, in mili-seconds, default 0 mSec
...
#define EIDMW_CNF_LOGGING_DIRNAME                                                                                      \
	L"log_dirname" // string, location of the log-file; $home/pteid/	Full path with volume name.
...
#define EIDMW_CNF_LOGGING_LEVEL                                                                                        \
	L"log_level" // string, Specify what should be logged; none, critical, error, warning, info or debug
...
#define EIDMW_CNF_CERTCACHE_CACHEFILE                                                                                  \
	L"cert_cachefile" // number; 0=no, 1=yes, If true, will read certain files (or most parts of them) on the card only
					  // the first time a card is used and cache their contents on disk for later use.
...
#define EIDMW_CNF_GENERAL_ADMIN_CONFIGURATION                                                                          \
	L"admin_config" // number; 0=admin configuration is disabled(default), 1=admin configuration is enabled
...
public:
	...
	static std::wstring GetStringInt(tLocation location, const std::wstring &csName, const std::wstring &czSection,
									 bool bExpand);
#endif
	...
	static std::wstring GetString(const std::wstring &csName, const std::wstring &czSection,
								  const std::wstring &csDefaultValue, bool bExpand = true);
	...
	static void SetString(tLocation location, const std::wstring &csName, const std::wstring &czSection,
						  const std::wstring &csValue);
	static unsigned int CountKeysByPrefix(tLocation location, const struct Param_Str param);
	static unsigned int CountKeysByPrefix(tLocation location, const std::wstring &csName,
	...
	static void SetTestMode(bool bTestMode) {
		bTestModeEnabled = bTestMode;
		return;
	}
	...
	static const struct Param_Num EIDMW_CONFIG_PARAM_GENERAL_TELEMETRY_STATUS;
	...
	static const struct Param_Str EIDMW_CONFIG_PARAM_GENERAL_PTEID_CACHEDIR_CERTS;
