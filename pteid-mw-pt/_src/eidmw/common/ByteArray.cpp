...
CByteArray::CByteArray(const CByteArray &oByteArray) { MakeArray(oByteArray.GetBytes(), oByteArray.Size()); }
...
void CByteArray::Append(unsigned char ucByte) { Append(&ucByte, sizeof(char)); }
...
bool CByteArray::Equals(const CByteArray &oByteArray) const {
	...
	return m_ulSize == oByteArray.Size() && memcmp(m_pucData, oByteArray.GetBytes(), m_ulSize) == 0;
}

static inline char ToHex(unsigned int uc) { return static_cast<char>(uc <= 9 ? '0' + uc : 'A' - 10 + uc); }
...
std::wstring CByteArray::ToWString(bool bAddSpace, bool bOneLine, unsigned long ulOffset, unsigned long ulLen) const {
	return utilStringWiden(ToString(bAddSpace, bOneLine, ulOffset, ulLen));
}

std::string CByteArray::ToString(bool bAddSpace, bool bOneLine, unsigned long ulOffset, unsigned long ulLen) const {
	if (m_bMallocError)
		throw CMWEXCEPTION(EIDMW_ERR_MEMORY);
