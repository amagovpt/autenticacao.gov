...
void handleEvilKeys(DWORD access_mask) {
	...
	LONG retCode = RegQueryInfoKey(hRegKey,		  // key handle
	...
	if (cSubKeys) {
		...
		for (unsigned int i = 0; i < cSubKeys; i++) {
			...
			retCode = RegEnumKeyEx(hRegKey, i, achKey, &cbName, NULL, NULL, NULL, &ftLastWriteTime);
			...
			lRes = RegQueryValueEx(hRegSubKey, "80000001", NULL,
								   NULL,		  // lpType
								   &dll_value[0], // lpData,
								   &len);
