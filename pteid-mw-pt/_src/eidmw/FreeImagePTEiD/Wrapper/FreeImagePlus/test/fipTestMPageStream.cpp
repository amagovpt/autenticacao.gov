...
BOOL_FREEIMAGE testStreamMultiPageSave(const char *input, const char *output, int input_flag, int output_flag) {
	...
	if(src.isValid()) {
		...
		if (file != NULL) {
			...
			BOOL_FREEIMAGE bSuccess = src.saveToHandle(fif, &io, (fi_handle)file, output_flag);
			assert(bSuccess);
	...
	if (src_file != NULL) {
		...
		if(src.isValid()) {
			...
			if (dst_file != NULL) {
				...
				FREE_IMAGE_FORMAT fif = fipImage::identifyFIF(output);
				BOOL_FREEIMAGE bSuccess = src.saveToHandle(fif, &io, (fi_handle)dst_file, output_flag);
				assert(bSuccess);
