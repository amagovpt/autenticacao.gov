...
fipMemoryIO::fipMemoryIO(BYTE_FREEIMAGE *data, DWORD_FREEIMAGE size_in_bytes) {
...
BOOL_FREEIMAGE fipMemoryIO::isValid() const {
	return (_hmem != NULL);
}
...
BOOL_FREEIMAGE fipMemoryIO::save(FREE_IMAGE_FORMAT fif, FIBITMAP *dib, int flags) {
	return FreeImage_SaveToMemory(fif, dib, _hmem, flags);
}

BOOL_FREEIMAGE fipMemoryIO::saveMultiPage(FREE_IMAGE_FORMAT fif, FIMULTIBITMAP *bitmap, int flags) {
...
BOOL_FREEIMAGE fipMemoryIO::seek(long offset, int origin) {
	return FreeImage_SeekMemory(_hmem, offset, origin);
}

BOOL_FREEIMAGE fipMemoryIO::acquire(BYTE_FREEIMAGE **data, DWORD_FREEIMAGE *size_in_bytes) {
