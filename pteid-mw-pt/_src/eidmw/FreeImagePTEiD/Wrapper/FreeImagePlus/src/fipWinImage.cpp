...
BOOL_FREEIMAGE fipWinImage::isValid() const {
...
HANDLE fipWinImage::copyToHandle() const {
	...
	if(_dib) {
		...
		BYTE_FREEIMAGE *dib = (BYTE_FREEIMAGE*)GlobalLock(hMem);

		memset(dib, 0, dib_size);

		BYTE_FREEIMAGE *p_dib = (BYTE_FREEIMAGE*)dib;
		...
		BYTE_FREEIMAGE *bits = FreeImage_GetBits(_dib);
...
BOOL_FREEIMAGE fipWinImage::copyFromHandle(HANDLE hMem) {
	BYTE_FREEIMAGE *lpVoid = NULL;
	BITMAPINFOHEADER *pHead = NULL;
	RGBQUAD *pPalette = NULL;
	BYTE_FREEIMAGE *bits = NULL;
	DWORD_FREEIMAGE bitfields[3] = {0, 0, 0};

	// Get a pointer to the bitmap
	lpVoid = (BYTE_FREEIMAGE *)GlobalLock(hMem);
	...
	if(pHead->biBitCount < 16)
		pPalette = (RGBQUAD *)(((BYTE_FREEIMAGE *)pHead) + sizeof(BITMAPINFOHEADER));

	// Get a pointer to the pixels
	bits = ((BYTE_FREEIMAGE*)pHead + sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * pHead->biClrUsed);
	...
	if(pHead->biCompression == BI_BITFIELDS) {
		...
		unsigned mask_size = 3 * sizeof(DWORD_FREEIMAGE);
...
BOOL_FREEIMAGE fipWinImage::copyFromBitmap(HBITMAP hbmp) {
	if(hbmp) { 
		int Success;
        BITMAP bm;
...
BOOL_FREEIMAGE fipWinImage::copyToClipboard(HWND hWndNewOwner) const {
...
BOOL_FREEIMAGE fipWinImage::pasteFromClipboard() {
...
BOOL_FREEIMAGE fipWinImage::captureWindow(HWND hWndApplicationWindow, HWND hWndSelectedWindow) {
...
void fipWinImage::drawEx(HDC hDC, RECT& rcDest, BOOL_FREEIMAGE useFileBkg, RGBQUAD *appBkColor, FIBITMAP *bg) const {
	...
	if(_bHasChanged) {
		...
		if(image_type == FIT_BITMAP) {
			BOOL_FREEIMAGE bHasBackground = FreeImage_HasBackgroundColor(_dib);
			BOOL_FREEIMAGE bIsTransparent = FreeImage_IsTransparent(_dib);
