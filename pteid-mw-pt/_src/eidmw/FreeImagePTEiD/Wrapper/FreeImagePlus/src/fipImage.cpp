...
BOOL_FREEIMAGE fipImage::replace(FIBITMAP *new_dib) {
	if(new_dib == NULL) 
		return FALSE;
	if(_dib)
...
BOOL_FREEIMAGE fipImage::setSize(FREE_IMAGE_TYPE image_type, unsigned width, unsigned height, unsigned bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {
	if(_dib) {
		FreeImage_Unload(_dib);
	}
...
BOOL_FREEIMAGE fipImage::copySubImage(fipImage& dst, int left, int top, int right, int bottom) const {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::pasteSubImage(fipImage& src, int left, int top, int alpha) {
	if(_dib) {
		BOOL_FREEIMAGE bResult = FreeImage_Paste(_dib, src._dib, left, top, alpha);
		_bHasChanged = TRUE;
		return bResult;
	}
	return FALSE;
...
BOOL_FREEIMAGE fipImage::crop(int left, int top, int right, int bottom) {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::isValid() const {
...
LONG_FREEIMAGE fipImage::getImageSize() const {
...
BOOL_FREEIMAGE fipImage::isGrayscale() const {
...
BOOL_FREEIMAGE fipImage::getThumbnail(fipImage& image) const {
	image = FreeImage_Clone( FreeImage_GetThumbnail(_dib) );
	return image.isValid();
}

BOOL_FREEIMAGE fipImage::setThumbnail(const fipImage& image) {
	return FreeImage_SetThumbnail(_dib, (FIBITMAP*)image._dib);
}

BOOL_FREEIMAGE fipImage::hasThumbnail() const {
	return (FreeImage_GetThumbnail(_dib) != NULL);
}

BOOL_FREEIMAGE fipImage::clearThumbnail() {
...
BYTE_FREEIMAGE* fipImage::accessPixels() const {
	return FreeImage_GetBits(_dib); 
}

BYTE_FREEIMAGE* fipImage::getScanLine(unsigned scanline) const {
...
BOOL_FREEIMAGE fipImage::getPixelIndex(unsigned x, unsigned y, BYTE_FREEIMAGE *value) const {
	return FreeImage_GetPixelIndex(_dib, x, y, value);
}

BOOL_FREEIMAGE fipImage::getPixelColor(unsigned x, unsigned y, RGBQUAD *value) const {
	return FreeImage_GetPixelColor(_dib, x, y, value);
}

BOOL_FREEIMAGE fipImage::setPixelIndex(unsigned x, unsigned y, BYTE_FREEIMAGE *value) {
	_bHasChanged = TRUE;
	return FreeImage_SetPixelIndex(_dib, x, y, value);
}

BOOL_FREEIMAGE fipImage::setPixelColor(unsigned x, unsigned y, RGBQUAD *value) {
	_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::load(const char* lpszPathName, int flag) {
	FREE_IMAGE_FORMAT fif = FIF_UNKNOWN;
...
BOOL_FREEIMAGE fipImage::loadU(const wchar_t* lpszPathName, int flag) {
	FREE_IMAGE_FORMAT fif = FIF_UNKNOWN;
...
BOOL_FREEIMAGE fipImage::loadFromHandle(FreeImageIO *io, fi_handle handle, int flag) {
	FREE_IMAGE_FORMAT fif = FIF_UNKNOWN;
...
BOOL_FREEIMAGE fipImage::loadFromMemory(fipMemoryIO& memIO, int flag) {
	FREE_IMAGE_FORMAT fif = FIF_UNKNOWN;
...
BOOL_FREEIMAGE fipImage::save(const char* lpszPathName, int flag) const {
	FREE_IMAGE_FORMAT fif = FIF_UNKNOWN;
...
BOOL_FREEIMAGE fipImage::saveU(const wchar_t* lpszPathName, int flag) const {
	FREE_IMAGE_FORMAT fif = FIF_UNKNOWN;
	BOOL_FREEIMAGE bSuccess = FALSE;
...
BOOL_FREEIMAGE fipImage::saveToHandle(FREE_IMAGE_FORMAT fif, FreeImageIO *io, fi_handle handle, int flag) const {
	BOOL_FREEIMAGE bSuccess = FALSE;
...
BOOL_FREEIMAGE fipImage::saveToMemory(FREE_IMAGE_FORMAT fif, fipMemoryIO& memIO, int flag) const {
	BOOL_FREEIMAGE bSuccess = FALSE;
...
BOOL_FREEIMAGE fipImage::convertToType(FREE_IMAGE_TYPE image_type, BOOL_FREEIMAGE scale_linear) {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::threshold(BYTE_FREEIMAGE T) {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertTo4Bits() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertTo8Bits() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertTo16Bits555() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertTo16Bits565() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertTo24Bits() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertTo32Bits() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertToGrayscale() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::colorQuantize(FREE_IMAGE_QUANTIZE algorithm) {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::dither(FREE_IMAGE_DITHER algorithm) {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertToFloat() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertToRGBF() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertToUINT16() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::convertToRGB16() {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::toneMapping(FREE_IMAGE_TMO tmo, double first_param, double second_param, double third_param, double fourth_param) {
	if(_dib) {
		FIBITMAP *dst = NULL;
...
BOOL_FREEIMAGE fipImage::isTransparent() const {
...
BYTE_FREEIMAGE* fipImage::getTransparencyTable() const {
	return FreeImage_GetTransparencyTable(_dib);
}

void fipImage::setTransparencyTable(BYTE_FREEIMAGE *table, int count) {
	FreeImage_SetTransparencyTable(_dib, table, count);
	_bHasChanged = TRUE;
}

BOOL_FREEIMAGE fipImage::hasFileBkColor() const {
	return FreeImage_HasBackgroundColor(_dib);
}

BOOL_FREEIMAGE fipImage::getFileBkColor(RGBQUAD *bkcolor) const {
	return FreeImage_GetBackgroundColor(_dib, bkcolor);
}

BOOL_FREEIMAGE fipImage::setFileBkColor(RGBQUAD *bkcolor) {
	_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::getChannel(fipImage& image, FREE_IMAGE_COLOR_CHANNEL channel) const {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::setChannel(fipImage& image, FREE_IMAGE_COLOR_CHANNEL channel) {
	if(_dib) {
		_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::splitChannels(fipImage& RedChannel, fipImage& GreenChannel, fipImage& BlueChannel) {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::combineChannels(fipImage& red, fipImage& green, fipImage& blue) {
	if(!_dib) {
...
BOOL_FREEIMAGE fipImage::rotateEx(double angle, double x_shift, double y_shift, double x_origin, double y_origin, BOOL_FREEIMAGE use_mask) {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::rotate(double angle, const void *bkcolor) {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::flipVertical() {
	if(_dib) {
		_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::flipHorizontal() {
	if(_dib) {
		_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::invert() {
	if(_dib) {
		_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::adjustCurve(BYTE_FREEIMAGE *LUT, FREE_IMAGE_COLOR_CHANNEL channel) {
	if(_dib) {
		_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::adjustGamma(double gamma) {
	if(_dib) {
		_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::adjustBrightness(double percentage) {
	if(_dib) {
		_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::adjustContrast(double percentage) {
	if(_dib) {
		_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::adjustBrightnessContrastGamma(double brightness, double contrast, double gamma) {
	if(_dib) {
		_bHasChanged = TRUE;
...
BOOL_FREEIMAGE fipImage::getHistogram(DWORD_FREEIMAGE *histo, FREE_IMAGE_COLOR_CHANNEL channel) const {
	if(_dib) {
...
BOOL_FREEIMAGE fipImage::rescale(unsigned new_width, unsigned new_height, FREE_IMAGE_FILTER filter) {
	if(_dib) {
		switch(FreeImage_GetImageType(_dib)) {
			case FIT_BITMAP:
			case FIT_UINT16:
...
BOOL_FREEIMAGE fipImage::makeThumbnail(unsigned max_size, BOOL_FREEIMAGE convert) {
	if(_dib) {
		switch(FreeImage_GetImageType(_dib)) {
			case FIT_BITMAP:
			case FIT_UINT16:
...
BOOL_FREEIMAGE fipImage::getMetadata(FREE_IMAGE_MDMODEL model, const char *key, fipTag& tag) const {
...
BOOL_FREEIMAGE fipImage::setMetadata(FREE_IMAGE_MDMODEL model, const char *key, fipTag& tag) {
