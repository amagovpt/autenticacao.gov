...
_ReadProc(void *buffer, unsigned size, unsigned count, fi_handle handle) {
	BYTE_FREEIMAGE *tmp = (BYTE_FREEIMAGE *)buffer;

	for (unsigned c = 0; c < count; c++) {
		memcpy(tmp, g_load_address, size);

		g_load_address = (BYTE_FREEIMAGE *)g_load_address + size;
...
_SeekProc(fi_handle handle, long offset, int origin) {
	...
	if (origin == SEEK_SET) {
		g_load_address = (BYTE_FREEIMAGE *)handle + offset;
	} else {
		g_load_address = (BYTE_FREEIMAGE *)g_load_address + offset;
	}
...
main(int argc, char *argv[]) {
	...
	BYTE_FREEIMAGE *test = new BYTE_FREEIMAGE[159744];
