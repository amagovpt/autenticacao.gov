...
*/
BOOL_FREEIMAGE testHeader(const char *lpszPathName) {
...
*/
BOOL_FREEIMAGE testHeaderData(const char *lpszPathName) {
	int flags = FIF_LOAD_NOPIXELS;
	...
	try {
		...
		BOOL_FREEIMAGE bHasPixel = FreeImage_HasPixels(dib);
		...
		BYTE_FREEIMAGE *bits = FreeImage_GetBits(dib);
		assert(bits == NULL);
...
testExifRawFile(const char *lpszPathName, int load_flags, int save_flags) {
	...
	try {
		...
		FITAG *tag = NULL;
		BOOL_FREEIMAGE bResult = FreeImage_GetMetadata(FIMD_EXIF_RAW, dib, "ExifRaw", &tag);
		if(tag) {
			const char *key = FreeImage_GetTagKey(tag);
			WORD id = FreeImage_GetTagID(tag);
			FREE_IMAGE_MDTYPE type = FreeImage_GetTagType(tag);
			DWORD_FREEIMAGE count = FreeImage_GetTagCount(tag);
			DWORD_FREEIMAGE length = FreeImage_GetTagLength(tag);
			BYTE_FREEIMAGE *value = (BYTE_FREEIMAGE*)FreeImage_GetTagValue(tag);
			...
			FITAG *dst_tag = NULL;
			BOOL_FREEIMAGE bResult = FreeImage_GetMetadata(FIMD_EXIF_RAW, dib, "ExifRaw", &dst_tag);
			if(dst_tag) {
				...
				DWORD_FREEIMAGE dst_count = FreeImage_GetTagCount(dst_tag);
				DWORD_FREEIMAGE dst_length = FreeImage_GetTagLength(dst_tag);
				BYTE_FREEIMAGE *dst_value = (BYTE_FREEIMAGE*)FreeImage_GetTagValue(dst_tag);
