...
BOOL_FREEIMAGE testClone(const char *lpszPathName) {
...
void testAllocateCloneUnload(const char *lpszPathName) {
	...
	BOOL_FREEIMAGE bResult = testClone(lpszPathName);
	assert(bResult);
}

BOOL_FREEIMAGE testAllocateCloneUnloadType(FREE_IMAGE_TYPE image_type, unsigned width, unsigned height) {
	FIBITMAP *image = NULL;
	FIBITMAP *clone = NULL;
	...
	try {
		...
		switch(image_type) {
			case FIT_BITMAP:
				if(FreeImage_GetBPP(image) == 8) {
					for(y = 0; y < FreeImage_GetHeight(image); y++) {
						BYTE_FREEIMAGE *bits = (BYTE_FREEIMAGE *)FreeImage_GetScanLine(image, y);
			...
			case FIT_UINT32:
				for(y = 0; y < FreeImage_GetHeight(image); y++) {
					DWORD_FREEIMAGE *bits = (DWORD_FREEIMAGE *)FreeImage_GetScanLine(image, y);
					for(x = 0; x < FreeImage_GetWidth(image); x++) {
						bits[x] = 128;
					}
				}
			...
			case FIT_INT32:
				for(y = 0; y < FreeImage_GetHeight(image); y++) {
					LONG_FREEIMAGE *bits = (LONG_FREEIMAGE *)FreeImage_GetScanLine(image, y);
					for(x = 0; x < FreeImage_GetWidth(image); x++) {
						bits[x] = 128;
					}
				}
		...
		switch(image_type) {
			case FIT_BITMAP:
				if(FreeImage_GetBPP(clone) == 8) {
					for(y = 0; y < FreeImage_GetHeight(clone); y++) {
						BYTE_FREEIMAGE *bits = (BYTE_FREEIMAGE *)FreeImage_GetScanLine(clone, y);
			...
			case FIT_UINT32:
				for(y = 0; y < FreeImage_GetHeight(clone); y++) {
					DWORD_FREEIMAGE *bits = (DWORD_FREEIMAGE *)FreeImage_GetScanLine(clone, y);
					for(x = 0; x < FreeImage_GetWidth(clone); x++) {
						if(bits[x] != 128)
							throw(1);
					}
			...
			case FIT_INT32:
				for(y = 0; y < FreeImage_GetHeight(clone); y++) {
					LONG_FREEIMAGE *bits = (LONG_FREEIMAGE *)FreeImage_GetScanLine(clone, y);
					for(x = 0; x < FreeImage_GetWidth(clone); x++) {
						if(bits[x] != 128)
							throw(1);
					}
...
BOOL_FREEIMAGE testLoadSaveConvertImageType(FIBITMAP *src, FREE_IMAGE_TYPE image_type) {
	FIBITMAP *dst = NULL;
	FIBITMAP *chk = NULL;
...
BOOL_FREEIMAGE testLoadSaveConvertComplexType(FIBITMAP *src, FREE_IMAGE_COLOR_CHANNEL channel) {
	FIBITMAP *dst = NULL;
