...
void testLoadMultiBitmapFromMemory(const char *lpszPathName) {
	...
	if(result == 0) {
		...
		BYTE_FREEIMAGE *mem_buffer = (BYTE_FREEIMAGE*)malloc(buf.st_size * sizeof(BYTE_FREEIMAGE));
		if(mem_buffer) {
			FILE *stream = fopen(lpszPathName, "rb");
			if(stream) {
				fread(mem_buffer, sizeof(BYTE_FREEIMAGE), buf.st_size, stream);
				fclose(stream);
				...
				// extract pages 
				BOOL_FREEIMAGE bSuccess = extractPagesFromMemory(fif, hmem);
				assert(bSuccess);
...
BOOL_FREEIMAGE testSaveMultiBitmapToMemory(const char *input, const char *output, int output_flag) {
	BOOL_FREEIMAGE bSuccess;

	BOOL_FREEIMAGE bCreateNew = FALSE;
	BOOL_FREEIMAGE bReadOnly = TRUE;
	BOOL_FREEIMAGE bMemoryCache = TRUE;
	...
	if(src) {
		...
		BYTE_FREEIMAGE *mem_buffer = NULL;
		DWORD_FREEIMAGE size_in_bytes = 0;
		...
		if(stream) {
			fwrite(mem_buffer, sizeof(BYTE_FREEIMAGE), size_in_bytes, stream);
			fclose(stream);
		}
...
static BOOL_FREEIMAGE  
loadBuffer(const char *lpszPathName, BYTE_FREEIMAGE **buffer, DWORD_FREEIMAGE *length) {
	...
	if(result == 0) {
		// allocate a memory buffer and load temporary data
		*buffer = (BYTE_FREEIMAGE*)malloc(file_info.st_size * sizeof(BYTE_FREEIMAGE));
		if(*buffer) {
			FILE *stream = fopen(lpszPathName, "rb");
			if(stream) {
				*length = (DWORD_FREEIMAGE)fread(*buffer, sizeof(BYTE_FREEIMAGE), file_info.st_size, stream);
				fclose(stream);
...
BOOL_FREEIMAGE testMemoryStreamMultiPageOpenSave(const char *lpszPathName, char *output, int input_flag, int output_flag) {
	BOOL_FREEIMAGE bSuccess = FALSE;

	BYTE_FREEIMAGE *buffer = NULL;
	DWORD_FREEIMAGE buffer_size = 0;
	...
	if(src) {
		...
		BYTE_FREEIMAGE *dst_buffer = NULL;
		DWORD_FREEIMAGE size_in_bytes = 0;
		...
		FILE *stream = fopen(output, "wb");
		if(stream) {
			fwrite(dst_buffer, sizeof(BYTE_FREEIMAGE), size_in_bytes, stream);
			fclose(stream);
		}
