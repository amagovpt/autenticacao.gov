...
#define MAX_BYTE_FREEIMAGES_IN_MARKER 65533L		// maximum data length of a JPEG marker
#define MAX_DATA_BYTE_FREEIMAGES_IN_MARKER 65519L	// maximum data length of a JPEG APP2 marker

#define MAX_JFXX_THUMB_SIZE (MAX_BYTE_FREEIMAGES_IN_MARKER - 5 - 1)
...
jpeg_read_comment(FIBITMAP *dib, const BYTE_FREEIMAGE *dataptr, unsigned int datalen) {
...
marker_is_icc(jpeg_saved_marker_ptr marker) {
	...
	const BYTE_FREEIMAGE icc_signature[12] = { 0x49, 0x43, 0x43, 0x5F, 0x50, 0x52, 0x4F, 0x46, 0x49, 0x4C, 0x45, 0x00 };
...
jpeg_read_icc_profile(j_decompress_ptr cinfo, JOCTET **icc_data_ptr, unsigned *icc_data_len) {
	...
	BYTE_FREEIMAGE marker_present[MAX_SEQ_NO+1];	// 1 if marker found
...
*/
static BOOL_FREEIMAGE 
jpeg_read_iptc_profile(FIBITMAP *dib, const BYTE_FREEIMAGE *dataptr, unsigned int datalen) {
...
*/
static BOOL_FREEIMAGE  
jpeg_read_xmp_profile(FIBITMAP *dib, const BYTE_FREEIMAGE *dataptr, unsigned int datalen) {
	...
	if(memcmp(xmp_signature, profile, strlen(xmp_signature)) == 0) {
		...
		if(tag) {
			...
			FreeImage_SetTagLength(tag, (DWORD_FREEIMAGE)length);
			FreeImage_SetTagCount(tag, (DWORD_FREEIMAGE)length);
			FreeImage_SetTagType(tag, FIDT_ASCII);
...
*/
static BOOL_FREEIMAGE  
jpeg_read_exif_profile_raw(FIBITMAP *dib, const BYTE_FREEIMAGE *profile, unsigned int length) {
	...
	if(tag) {
		...
		FreeImage_SetTagLength(tag, (DWORD_FREEIMAGE)length);
		FreeImage_SetTagCount(tag, (DWORD_FREEIMAGE)length);
		FreeImage_SetTagType(tag, FIDT_BYTE_FREEIMAGE);
		FreeImage_SetTagValue(tag, profile);
...
*/
static BOOL_FREEIMAGE 
jpeg_read_jfxx(FIBITMAP *dib, const BYTE_FREEIMAGE *dataptr, unsigned int datalen) {
	if(datalen < 6) {
		return FALSE;
	}
	
	const int id_length = 5;
	const BYTE_FREEIMAGE *data = dataptr + id_length;
	unsigned remaining = datalen - id_length;
		
	const BYTE_FREEIMAGE type = *data;
	++data, --remaining;
	...
	switch(type) {
		...
		{
			// load the thumbnail
			FIMEMORY* hmem = FreeImage_OpenMemory(const_cast<BYTE_FREEIMAGE*>(data), remaining);
...
read_markers(j_decompress_ptr cinfo, FIBITMAP *dib) {
	...
	// ICC profile
	BYTE_FREEIMAGE *icc_profile = NULL;
...
jpeg_write_comment(j_compress_ptr cinfo, FIBITMAP *dib) {
	...
	if(tag) {
		...
		if(NULL != tag_value) {
			for(long i = 0; i < (long)strlen(tag_value); i+= MAX_BYTE_FREEIMAGES_IN_MARKER) {
				jpeg_write_marker(cinfo, JPEG_COM, (BYTE_FREEIMAGE*)tag_value + i, MIN((long)strlen(tag_value + i), MAX_BYTE_FREEIMAGES_IN_MARKER));
			}
			return TRUE;
		}
	}
...
jpeg_write_icc_profile(j_compress_ptr cinfo, FIBITMAP *dib) {
    // marker identifying string "ICC_PROFILE" (null-terminated)
	BYTE_FREEIMAGE icc_signature[12] = { 0x49, 0x43, 0x43, 0x5F, 0x50, 0x52, 0x4F, 0x46, 0x49, 0x4C, 0x45, 0x00 };
	...
	if (iccProfile->size && iccProfile->data) {
		...
		BYTE_FREEIMAGE *profile = (BYTE_FREEIMAGE*)malloc((iccProfile->size + ICC_HEADER_SIZE) * sizeof(BYTE_FREEIMAGE));
		if(profile == NULL) return FALSE;
		memcpy(profile, icc_signature, 12);

		for(long i = 0; i < (long)iccProfile->size; i += MAX_DATA_BYTE_FREEIMAGES_IN_MARKER) {
			unsigned length = MIN((long)(iccProfile->size - i), MAX_DATA_BYTE_FREEIMAGES_IN_MARKER);
			// sequence number
			profile[12] = (BYTE_FREEIMAGE) ((i / MAX_DATA_BYTE_FREEIMAGES_IN_MARKER) + 1);
			// number of markers
			profile[13] = (BYTE_FREEIMAGE) (iccProfile->size / MAX_DATA_BYTE_FREEIMAGES_IN_MARKER + 1);

			memcpy(profile + ICC_HEADER_SIZE, (BYTE_FREEIMAGE*)iccProfile->data + i, length);
	...
	if(FreeImage_GetMetadataCount(FIMD_IPTC, dib)) {
		...
		if(write_iptc_profile(dib, &profile, &profile_size)) {
			...
			for(long i = 0; i < (long)profile_size; i += 65517L) {
				...
				BYTE_FREEIMAGE *iptc_profile = (BYTE_FREEIMAGE*)malloc(length + roundup + tag_length);
				...
				// segment size
				iptc_profile[24] = (BYTE_FREEIMAGE)(length >> 8);
				iptc_profile[25] = (BYTE_FREEIMAGE)(length & 0xFF);
				// segment data
...
jpeg_write_xmp_profile(j_compress_ptr cinfo, FIBITMAP *dib) {
	...
	if(tag_xmp) {
		const BYTE_FREEIMAGE *tag_value = (BYTE_FREEIMAGE*)FreeImage_GetTagValue(tag_xmp);

		if(NULL != tag_value) {
			// XMP signature is 29 bytes long
			unsigned int xmp_header_size = (unsigned int)strlen(xmp_signature) + 1;

			DWORD_FREEIMAGE tag_length = FreeImage_GetTagLength(tag_xmp);

			BYTE_FREEIMAGE *profile = (BYTE_FREEIMAGE*)malloc((tag_length + xmp_header_size) * sizeof(BYTE_FREEIMAGE));
			if(profile == NULL) return FALSE;
			memcpy(profile, xmp_signature, xmp_header_size);

			for(DWORD_FREEIMAGE i = 0; i < tag_length; i += 65504L) {
...
jpeg_write_exif_profile_raw(j_compress_ptr cinfo, FIBITMAP *dib) {
	...
	if(tag_exif) {
		const BYTE_FREEIMAGE *tag_value = (BYTE_FREEIMAGE*)FreeImage_GetTagValue(tag_exif);
		...
		if(NULL != tag_value) {
			DWORD_FREEIMAGE tag_length = FreeImage_GetTagLength(tag_exif);

			BYTE_FREEIMAGE *profile = (BYTE_FREEIMAGE*)malloc(tag_length * sizeof(BYTE_FREEIMAGE));
			if(profile == NULL) return FALSE;
...
jpeg_write_jfxx(j_compress_ptr cinfo, FIBITMAP *dib) {
	...
	BYTE_FREEIMAGE* thData = NULL;
	DWORD_FREEIMAGE thSize = 0;
	
	FreeImage_AcquireMemory(stream, &thData, &thSize);	
	
	BYTE_FREEIMAGE id_length = 5; //< "JFXX"
	BYTE_FREEIMAGE type = JFXX_TYPE_JPEG;
	
	DWORD_FREEIMAGE totalsize = id_length + sizeof(type) + thSize;
	...
	BYTE_FREEIMAGE* & out = dest->pub.next_output_byte;
	size_t & bufRemain = dest->pub.free_in_buffer;
	
	const BYTE_FREEIMAGE *thData_end = thData + thSize;
...
Validate(FreeImageIO *io, fi_handle handle) {
	BYTE_FREEIMAGE jpeg_signature[] = { 0xFF, 0xD8 };
	BYTE_FREEIMAGE signature[2] = { 0, 0 };
		...
		try {
			...
			if((cinfo.num_components == 4) && (cinfo.out_color_space == JCS_CMYK)) {
				...
				if (cinfo.num_components == 1) {
					...
					for (int i = 0; i < 256; i++) {
						colors[i].rgbRed   = (BYTE_FREEIMAGE)i;
						colors[i].rgbGreen = (BYTE_FREEIMAGE)i;
						colors[i].rgbBlue  = (BYTE_FREEIMAGE)i;
					}
				}
			}
			if(scale_denom != 1) {
			...
			if((cinfo.out_color_space == JCS_CMYK) && ((flags & JPEG_CMYK) != JPEG_CMYK)) {
				...
				while (cinfo.output_scanline < cinfo.output_height) {
					...
					for(unsigned x = 0; x < FreeImage_GetWidth(dib); x++) {
						WORD K = (WORD)src[3];
						dst[FI_RGBA_RED]   = (BYTE_FREEIMAGE)((K * src[0]) / 255);
						dst[FI_RGBA_GREEN] = (BYTE_FREEIMAGE)((K * src[1]) / 255);
						dst[FI_RGBA_BLUE]  = (BYTE_FREEIMAGE)((K * src[2]) / 255);
						src += 4;
						dst += 3;
					}
				}
				...
				if(cinfo.num_components == 3) {
					for(unsigned y = 0; y < FreeImage_GetHeight(dib); y++) {
						BYTE_FREEIMAGE *target = FreeImage_GetScanLine(dib, y);
						for(unsigned x = 0; x < FreeImage_GetWidth(dib); x++) {
	...
	if ((dib) && (handle)) {
		try {
			...
			if(color_type == FIC_RGB) {
				...
				BYTE_FREEIMAGE *target = (BYTE_FREEIMAGE*)malloc(pitch * sizeof(BYTE_FREEIMAGE));
				if (target == NULL) {
				...
				while (cinfo.next_scanline < cinfo.image_height) {
					...
					BYTE_FREEIMAGE *target_p = target;
				...
				BYTE_FREEIMAGE *target = (BYTE_FREEIMAGE*)malloc(cinfo.image_width * 3);
				if (target == NULL) {
					throw FI_MSG_ERROR_MEMORY;
				}

				while (cinfo.next_scanline < cinfo.image_height) {
					BYTE_FREEIMAGE *source = FreeImage_GetScanLine(dib, FreeImage_GetHeight(dib) - cinfo.next_scanline - 1);
				...
				unsigned i;
				BYTE_FREEIMAGE reverse[256];
				BYTE_FREEIMAGE *target = (BYTE_FREEIMAGE *)malloc(cinfo.image_width);
				if (target == NULL) {
					throw FI_MSG_ERROR_MEMORY;
				}

				for(i = 0; i < 256; i++) {
					reverse[i] = (BYTE_FREEIMAGE)(255 - i);
				}
