...
void DLL_CALLCONV
FreeImage_Initialise(BOOL_FREEIMAGE load_local_plugins_only) {
...
void * DLL_CALLCONV
FreeImage_Open(PluginNode *node, FreeImageIO *io, fi_handle handle, BOOL_FREEIMAGE open_for_reading) {
...
FreeImage_Save(FREE_IMAGE_FORMAT fif, FIBITMAP *dib, const char *filename, int flags) {
	...
	if (handle) {
		BOOL_FREEIMAGE success = FreeImage_SaveToHandle(fif, dib, &io, (fi_handle)handle, flags);
...
int DLL_CALLCONV
FreeImage_SetPluginEnabled(FREE_IMAGE_FORMAT fif, BOOL_FREEIMAGE enable) {
	if (s_plugins != NULL) {
		PluginNode *node = s_plugins->FindNodeFromFIF(fif);

		if (node != NULL) {
			BOOL_FREEIMAGE previous_state = node->m_enabled;
...
FreeImage_Validate(FREE_IMAGE_FORMAT fif, FreeImageIO *io, fi_handle handle) {
	if (s_plugins != NULL) {
		BOOL_FREEIMAGE validated = FALSE;
