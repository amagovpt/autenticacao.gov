...
ConvertInPlaceRGBFToYxy(FIBITMAP *dib) {
	...
	BYTE_FREEIMAGE *bits = (BYTE_FREEIMAGE*)FreeImage_GetBits(dib);
...
LuminanceFromYxy(FIBITMAP *Yxy, float *maxLum, float *minLum, float *worldLum) {
	...
	BYTE_FREEIMAGE *bits = (BYTE_FREEIMAGE*)FreeImage_GetBits(Yxy);
	for(unsigned y = 0; y < height; y++) {
...
ClampConvertRGBFTo24(FIBITMAP *src) {
	...
	BYTE_FREEIMAGE *src_bits = (BYTE_FREEIMAGE*)FreeImage_GetBits(src);
	BYTE_FREEIMAGE *dst_bits = (BYTE_FREEIMAGE*)FreeImage_GetBits(dst);
	...
	for(unsigned y = 0; y < height; y++) {
		...
		for(unsigned x = 0; x < width; x++) {
			...
			dst_pixel[FI_RGBA_RED]   = (BYTE_FREEIMAGE)(255.0F * red   + 0.5F);
			dst_pixel[FI_RGBA_GREEN] = (BYTE_FREEIMAGE)(255.0F * green + 0.5F);
			dst_pixel[FI_RGBA_BLUE]  = (BYTE_FREEIMAGE)(255.0F * blue  + 0.5F);
			dst_pixel += 3;
		}
		src_bits += src_pitch;
		dst_bits += dst_pitch;
