...
/** Convert a greyscale image of type Tsrc to a 8-bit grayscale dib.
	...
	or a rounding from src_pixel to (BYTE_FREEIMAGE) MIN(255, MAX(0, q)) where int q = int(src_pixel + 0.5); 
*/
template<class Tsrc>
class CONVERT_TO_BYTE_FREEIMAGE
{
public:
	FIBITMAP* convert(FIBITMAP *src, BOOL_FREEIMAGE scale_linear);
};

template<class Tsrc> FIBITMAP* 
CONVERT_TO_BYTE_FREEIMAGE<Tsrc>::convert(FIBITMAP *src, BOOL_FREEIMAGE scale_linear) {
	FIBITMAP *dst = NULL;
	unsigned x, y;
	...
	for(int i = 0; i < 256; i++) {
		pal[i].rgbRed = (BYTE_FREEIMAGE)i;
		pal[i].rgbGreen = (BYTE_FREEIMAGE)i;
		pal[i].rgbBlue = (BYTE_FREEIMAGE)i;
	}
	...
	if(scale_linear) {
		...
		for(y = 0; y < height; y++) {
			...
			BYTE_FREEIMAGE *dst_bits = FreeImage_GetScanLine(dst, y);
			for(x = 0; x < width; x++) {
				dst_bits[x] = (BYTE_FREEIMAGE)( scale * (src_bits[x] - min) + 0.5);
			}
		}
	} else {
		for(y = 0; y < height; y++) {
			...
			for(x = 0; x < width; x++) {
				...
				dst_bits[x] = (BYTE_FREEIMAGE) MIN(255, MAX(0, q));
			}
		}
	}
...
// Convert from type BYTE_FREEIMAGE to type X
CONVERT_TYPE<unsigned short, BYTE_FREEIMAGE>	convertByteToUShort;
CONVERT_TYPE<short, BYTE_FREEIMAGE>			convertByteToShort;
CONVERT_TYPE<DWORD_FREEIMAGE, BYTE_FREEIMAGE>			convertByteToULong;
CONVERT_TYPE<LONG_FREEIMAGE, BYTE_FREEIMAGE>			convertByteToLong;
CONVERT_TYPE<float, BYTE_FREEIMAGE>			convertByteToFloat;
CONVERT_TYPE<double, BYTE_FREEIMAGE>			convertByteToDouble;

// Convert from type X to type BYTE_FREEIMAGE
CONVERT_TO_BYTE_FREEIMAGE<unsigned short>	convertUShortToByte;
CONVERT_TO_BYTE_FREEIMAGE<short>			convertShortToByte;
CONVERT_TO_BYTE_FREEIMAGE<DWORD_FREEIMAGE>			convertULongToByte;
CONVERT_TO_BYTE_FREEIMAGE<LONG_FREEIMAGE>			convertLongToByte;
CONVERT_TO_BYTE_FREEIMAGE<float>			convertFloatToByte;
CONVERT_TO_BYTE_FREEIMAGE<double>			convertDoubleToByte;
...
CONVERT_TYPE<float, DWORD_FREEIMAGE>			convertULongToFloat;
CONVERT_TYPE<float, LONG_FREEIMAGE>			convertLongToFloat;
...
CONVERT_TYPE<double, DWORD_FREEIMAGE>				convertULongToDouble;
CONVERT_TYPE<double, LONG_FREEIMAGE>				convertLongToDouble;
CONVERT_TYPE<double, float>				convertFloatToDouble;

// Convert from type X to type FICOMPLEX
CONVERT_TO_COMPLEX<BYTE_FREEIMAGE>			convertByteToComplex;
CONVERT_TO_COMPLEX<unsigned short>	convertUShortToComplex;
CONVERT_TO_COMPLEX<short>			convertShortToComplex;
CONVERT_TO_COMPLEX<DWORD_FREEIMAGE>			convertULongToComplex;
CONVERT_TO_COMPLEX<LONG_FREEIMAGE>			convertLongToComplex;
...
FreeImage_ConvertToStandardType(FIBITMAP *src, BOOL_FREEIMAGE scale_linear) {
	FIBITMAP *dst = NULL;
...
FIBITMAP* DLL_CALLCONV
FreeImage_ConvertToType(FIBITMAP *src, FREE_IMAGE_TYPE dst_type, BOOL_FREEIMAGE scale_linear) {
	FIBITMAP *dst = NULL;
