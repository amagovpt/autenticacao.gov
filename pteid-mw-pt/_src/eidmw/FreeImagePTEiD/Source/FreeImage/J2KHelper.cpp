	...
	try {
		...
		if(image->comps[0].prec <= 8) {
			if(numcomps == 1) {
				...
				for(int y = 0; y < hrr; y++) {		
					BYTE_FREEIMAGE *bits = FreeImage_GetScanLine(dib, hrr - 1 - y);
				...
				for(int y = 0; y < hrr; y++) {		
					...
					for(int x = 0; x < wrr; x++) {
						...
						bits[FI_RGBA_RED]   = (BYTE_FREEIMAGE)r;
						bits[FI_RGBA_GREEN] = (BYTE_FREEIMAGE)g;
						bits[FI_RGBA_BLUE]  = (BYTE_FREEIMAGE)b;
						bits += 3;
				...
				for(int y = 0; y < hrr; y++) {		
					...
					for(int x = 0; x < wrr; x++) {
						...
						bits[FI_RGBA_GREEN] = (BYTE_FREEIMAGE)g;
						bits[FI_RGBA_BLUE]  = (BYTE_FREEIMAGE)b;
						bits[FI_RGBA_ALPHA] = (BYTE_FREEIMAGE)a;
						bits += 4;
		...
		if(prec == 8) {
			switch(numcomps) {
				case 1:
					index = 0;
					for(y = 0; y < h; y++) {
						BYTE_FREEIMAGE *bits = FreeImage_GetScanLine(dib, h - 1 - y);
						for(x = 0; x < w; x++) {
