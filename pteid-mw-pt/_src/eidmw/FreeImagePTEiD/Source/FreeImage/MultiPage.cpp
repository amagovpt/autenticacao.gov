...
FI_STRUCT (MULTIBITMAPHEADER) {
	...
	BOOL_FREEIMAGE changed;
	int page_count;
	BlockList m_blocks;
	char *m_filename;
	BOOL_FREEIMAGE read_only;
...
FreeImage_OpenMultiBitmap(FREE_IMAGE_FORMAT fif, const char *filename, BOOL_FREEIMAGE create_new, BOOL_FREEIMAGE read_only, BOOL_FREEIMAGE keep_cache_in_memory, int flags) {
...
FreeImage_OpenMultiBitmapFromHandle(FREE_IMAGE_FORMAT fif, FreeImageIO *io, fi_handle handle, int flags) {
	try {
		BOOL_FREEIMAGE read_only = FALSE;	// modifications (if any) will be stored into the memory cache
				...
				if (success) {
					switch((*i)->m_type) {
						...
						case BLOCK_REFERENCE:
						{
							...
							BYTE_FREEIMAGE *compressed_data = (BYTE_FREEIMAGE*)malloc(ref->m_size * sizeof(BYTE_FREEIMAGE));
							
							header->m_cachefile->readFile((BYTE_FREEIMAGE *)compressed_data, ref->m_reference, ref->m_size);
...
FreeImage_AppendPage(FIMULTIBITMAP *bitmap, FIBITMAP *data) {
	if ((bitmap) && (data)) {
		...
		if ((!header->read_only) && (header->locked_pages.empty())) {
			DWORD_FREEIMAGE compressed_size = 0;
			BYTE_FREEIMAGE *compressed_data = NULL;
...
void DLL_CALLCONV
FreeImage_UnlockPage(FIMULTIBITMAP *bitmap, FIBITMAP *page, BOOL_FREEIMAGE changed) {
	if ((bitmap) && (page)) {
		MULTIBITMAPHEADER *header = FreeImage_GetMultiBitmapHeader(bitmap);
