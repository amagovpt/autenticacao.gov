...
*/
static BOOL_FREEIMAGE GaussianPyramid(FIBITMAP *H, FIBITMAP **pyramid, int nlevels) {
	try {
...
@return Returns TRUE if successful, returns FALSE otherwise
*/
static BOOL_FREEIMAGE GradientPyramid(FIBITMAP **pyramid, int nlevels, FIBITMAP **gradients, float *avgGrad) {
	try {
		for(int k = 0; k < nlevels; k++) {
...
static FIBITMAP* LogLuminance(FIBITMAP *Y) {
	...
	try {
		...
		BYTE_FREEIMAGE *bits = (BYTE_FREEIMAGE*)FreeImage_GetBits(H);
		for(unsigned y = 0; y < height; y++) {
		...
		bits = (BYTE_FREEIMAGE*)FreeImage_GetBits(H);
		for(unsigned y = 0; y < height; y++) {
...
static void ExpLuminance(FIBITMAP *Y) {
	...
	BYTE_FREEIMAGE *bits = (BYTE_FREEIMAGE*)FreeImage_GetBits(Y);
	for(unsigned y = 0; y < height; y++) {
		float *pixel = (float*)bits;
		for(unsigned x = 0; x < width; x++) {
...
FreeImage_TmoFattal02(FIBITMAP *dib, double color_saturation, double attenuation) {	
	...
	try {
		...
		BYTE_FREEIMAGE *bits      = (BYTE_FREEIMAGE*)FreeImage_GetBits(src);
		BYTE_FREEIMAGE *bits_yin  = (BYTE_FREEIMAGE*)FreeImage_GetBits(Yin);
		BYTE_FREEIMAGE *bits_yout = (BYTE_FREEIMAGE*)FreeImage_GetBits(Yout);
