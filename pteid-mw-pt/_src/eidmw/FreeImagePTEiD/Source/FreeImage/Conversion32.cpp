...
void DLL_CALLCONV
FreeImage_ConvertLine1To32(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels, RGBQUAD *palette) {
...
void DLL_CALLCONV
FreeImage_ConvertLine4To32(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels, RGBQUAD *palette) {
	BOOL_FREEIMAGE low_nibble = FALSE;
	int x = 0;
...
void DLL_CALLCONV
FreeImage_ConvertLine8To32(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels, RGBQUAD *palette) {
	for (int cols = 0; cols < width_in_pixels; cols++) {
...
void DLL_CALLCONV
FreeImage_ConvertLine16To32_555(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	WORD *bits = (WORD *)source;

	for (int cols = 0; cols < width_in_pixels; cols++) {
		target[FI_RGBA_RED]   = (BYTE_FREEIMAGE)((((bits[cols] & FI16_555_RED_MASK) >> FI16_555_RED_SHIFT) * 0xFF) / 0x1F);
		target[FI_RGBA_GREEN] = (BYTE_FREEIMAGE)((((bits[cols] & FI16_555_GREEN_MASK) >> FI16_555_GREEN_SHIFT) * 0xFF) / 0x1F);
		target[FI_RGBA_BLUE]  = (BYTE_FREEIMAGE)((((bits[cols] & FI16_555_BLUE_MASK) >> FI16_555_BLUE_SHIFT) * 0xFF) / 0x1F);
		target[FI_RGBA_ALPHA] = 0xFF;
		target += 4;
	}
}
...
void DLL_CALLCONV
FreeImage_ConvertLine16To32_565(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	WORD *bits = (WORD *)source;

	for (int cols = 0; cols < width_in_pixels; cols++) {
		target[FI_RGBA_RED]   = (BYTE_FREEIMAGE)((((bits[cols] & FI16_565_RED_MASK) >> FI16_565_RED_SHIFT) * 0xFF) / 0x1F);
		target[FI_RGBA_GREEN] = (BYTE_FREEIMAGE)((((bits[cols] & FI16_565_GREEN_MASK) >> FI16_565_GREEN_SHIFT) * 0xFF) / 0x3F);
		target[FI_RGBA_BLUE]  = (BYTE_FREEIMAGE)((((bits[cols] & FI16_565_BLUE_MASK) >> FI16_565_BLUE_SHIFT) * 0xFF) / 0x1F);
		target[FI_RGBA_ALPHA] = 0xFF;
		target += 4;
	}
}
/*
void DLL_CALLCONV
FreeImage_ConvertLine24To32(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	for (int cols = 0; cols < width_in_pixels; cols++) {
		*(DWORD_FREEIMAGE *)target = (*(DWORD_FREEIMAGE *) source & FI_RGBA_RGB_MASK) | FI_RGBA_ALPHA_MASK;
		target += 4;
		source += 3;
	}
}
...
inline void 
FreeImage_ConvertLine1To32MapTransparency(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels, RGBQUAD *palette, BYTE_FREEIMAGE *table, int transparent_pixels) {
	for (int cols = 0; cols < width_in_pixels; cols++) {
		int index = (source[cols>>3] & (0x80 >> (cols & 0x07))) != 0 ? 1 : 0;
...
inline void 
FreeImage_ConvertLine4To32MapTransparency(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels, RGBQUAD *palette, BYTE_FREEIMAGE *table, int transparent_pixels) {
	BOOL_FREEIMAGE low_nibble = FALSE;
	int x = 0;
...
inline void 
FreeImage_ConvertLine8To32MapTransparency(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels, RGBQUAD *palette, BYTE_FREEIMAGE *table, int transparent_pixels) {
	for (int cols = 0; cols < width_in_pixels; cols++) {
		target[FI_RGBA_BLUE]	= palette[source[cols]].rgbBlue;
		target[FI_RGBA_GREEN]	= palette[source[cols]].rgbGreen;
		target[FI_RGBA_RED]		= palette[source[cols]].rgbRed;
...
FreeImage_ConvertTo32Bits(FIBITMAP *dib) {
	...
	if(image_type == FIT_BITMAP) {
		...
		BOOL_FREEIMAGE bIsTransparent = FreeImage_IsTransparent(dib);
		...
		const BYTE_FREEIMAGE *src_bits = FreeImage_GetBits(dib);
		BYTE_FREEIMAGE *dst_bits = FreeImage_GetBits(new_dib);
		for (int rows = 0; rows < height; rows++) {
			const FIRGB16 *src_pixel = (FIRGB16*)src_bits;
			RGBQUAD *dst_pixel = (RGBQUAD*)dst_bits;
			for(int cols = 0; cols < width; cols++) {
				dst_pixel[cols].rgbRed		= (BYTE_FREEIMAGE)(src_pixel[cols].red   >> 8);
				dst_pixel[cols].rgbGreen	= (BYTE_FREEIMAGE)(src_pixel[cols].green >> 8);
				dst_pixel[cols].rgbBlue		= (BYTE_FREEIMAGE)(src_pixel[cols].blue  >> 8);
				dst_pixel[cols].rgbReserved = (BYTE_FREEIMAGE)0xFF;
			}
			src_bits += src_pitch;
			dst_bits += dst_pitch;
		}
		...
		for (int rows = 0; rows < height; rows++) {
			...
			for(int cols = 0; cols < width; cols++) {
				...
				dst_pixel[cols].rgbGreen	= (BYTE_FREEIMAGE)(src_pixel[cols].green >> 8);
				dst_pixel[cols].rgbBlue		= (BYTE_FREEIMAGE)(src_pixel[cols].blue  >> 8);
				dst_pixel[cols].rgbReserved = (BYTE_FREEIMAGE)(src_pixel[cols].alpha >> 8);
			}
			src_bits += src_pitch;
			dst_bits += dst_pitch;
		}		
