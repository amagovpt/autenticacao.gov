...
void DLL_CALLCONV
FreeImage_ConvertLine1To24(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels, RGBQUAD *palette) {
	for (int cols = 0; cols < width_in_pixels; cols++) {
		BYTE_FREEIMAGE index = (source[cols >> 3] & (0x80 >> (cols & 0x07))) != 0 ? 1 : 0;
...
void DLL_CALLCONV
FreeImage_ConvertLine4To24(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels, RGBQUAD *palette) {
...
void DLL_CALLCONV
FreeImage_ConvertLine8To24(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels, RGBQUAD *palette) {
	for (int cols = 0; cols < width_in_pixels; cols++) {
...
void DLL_CALLCONV
FreeImage_ConvertLine16To24_555(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
...
void DLL_CALLCONV
FreeImage_ConvertLine16To24_565(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	WORD *bits = (WORD *)source;
...
void DLL_CALLCONV
FreeImage_ConvertLine32To24(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	for (int cols = 0; cols < width_in_pixels; cols++) {
			...
			case 48:
			{
				...
				for (int rows = 0; rows < height; rows++) {
					...
					for(int cols = 0; cols < width; cols++) {
						dst_pixel[cols].rgbtRed   = (BYTE_FREEIMAGE)(src_pixel[cols].red   >> 8);
						dst_pixel[cols].rgbtGreen = (BYTE_FREEIMAGE)(src_pixel[cols].green >> 8);
						dst_pixel[cols].rgbtBlue  = (BYTE_FREEIMAGE)(src_pixel[cols].blue  >> 8);
					}
					src_bits += src_pitch;
					dst_bits += dst_pitch;
				}
