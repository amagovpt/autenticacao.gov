...
WuQuantizer::WuQuantizer(FIBITMAP *dib) {
	...
	wt = (LONG_FREEIMAGE*)malloc(SIZE_3D * sizeof(LONG_FREEIMAGE));
	mr = (LONG_FREEIMAGE*)malloc(SIZE_3D * sizeof(LONG_FREEIMAGE));
	mg = (LONG_FREEIMAGE*)malloc(SIZE_3D * sizeof(LONG_FREEIMAGE));
	mb = (LONG_FREEIMAGE*)malloc(SIZE_3D * sizeof(LONG_FREEIMAGE));
	...
	memset(wt, 0, SIZE_3D * sizeof(LONG_FREEIMAGE));
	memset(mr, 0, SIZE_3D * sizeof(LONG_FREEIMAGE));
	memset(mg, 0, SIZE_3D * sizeof(LONG_FREEIMAGE));
	memset(mb, 0, SIZE_3D * sizeof(LONG_FREEIMAGE));
...
void 
WuQuantizer::Hist3D(LONG_FREEIMAGE *vwt, LONG_FREEIMAGE *vmr, LONG_FREEIMAGE *vmg, LONG_FREEIMAGE *vmb, float *m2, int ReserveSize, RGBQUAD *ReservePalette) {
	int ind = 0;
	...
	for(y = 0; y < height; y++) {
		BYTE_FREEIMAGE *bits = FreeImage_GetScanLine(m_dib, y);
...
void 
WuQuantizer::M3D(LONG_FREEIMAGE *vwt, LONG_FREEIMAGE *vmr, LONG_FREEIMAGE *vmg, LONG_FREEIMAGE *vmb, float *m2) {
	unsigned ind1, ind2;
	BYTE_FREEIMAGE i, r, g, b;
	LONG_FREEIMAGE line, line_r, line_g, line_b;
	LONG_FREEIMAGE area[33], area_r[33], area_g[33], area_b[33];
...
LONG_FREEIMAGE 
WuQuantizer::Vol( Box *cube, LONG_FREEIMAGE *mmt ) {
...
LONG_FREEIMAGE 
WuQuantizer::Bottom(Box *cube, BYTE_FREEIMAGE dir, LONG_FREEIMAGE *mmt) {
    switch(dir)
	{
		case FI_RGBA_RED:
...
LONG_FREEIMAGE 
WuQuantizer::Top(Box *cube, BYTE_FREEIMAGE dir, int pos, LONG_FREEIMAGE *mmt) {
    switch(dir)
	{
		case FI_RGBA_RED:
...
float
WuQuantizer::Maximize(Box *cube, BYTE_FREEIMAGE dir, int first, int last , int *cut, LONG_FREEIMAGE whole_r, LONG_FREEIMAGE whole_g, LONG_FREEIMAGE whole_b, LONG_FREEIMAGE whole_w) {
	LONG_FREEIMAGE half_r, half_g, half_b, half_w;
	int i;
	float temp;

    LONG_FREEIMAGE base_r = Bottom(cube, dir, mr);
    LONG_FREEIMAGE base_g = Bottom(cube, dir, mg);
    LONG_FREEIMAGE base_b = Bottom(cube, dir, mb);
    LONG_FREEIMAGE base_w = Bottom(cube, dir, wt);
...
WuQuantizer::Cut(Box *set1, Box *set2) {
    ...
    LONG_FREEIMAGE whole_r = Vol(set1, mr);
    LONG_FREEIMAGE whole_g = Vol(set1, mg);
    LONG_FREEIMAGE whole_b = Vol(set1, mb);
    LONG_FREEIMAGE whole_w = Vol(set1, wt);
...
void
WuQuantizer::Mark(Box *cube, int label, BYTE_FREEIMAGE *tag) {
    for (int r = cube->r0 + 1; r <= cube->r1; r++) {
		for (int g = cube->g0 + 1; g <= cube->g1; g++) {
			for (int b = cube->b0 + 1; b <= cube->b1; b++) {
				tag[INDEX(r, g, b)] = (BYTE_FREEIMAGE)label;
			}
		}
	}
}
...
WuQuantizer::Quantize(int PaletteSize, int ReserveSize, RGBQUAD *ReservePalette) {
	BYTE_FREEIMAGE *tag = NULL;

	try {
		Box	cube[MAXCOLOR];
		int	next;
		LONG_FREEIMAGE i, weight;
		int k;
		...
		tag = (BYTE_FREEIMAGE*) malloc(SIZE_3D * sizeof(BYTE_FREEIMAGE));
		if (tag == NULL) {
			throw FI_MSG_ERROR_MEMORY;
		}
		memset(tag, 0, SIZE_3D * sizeof(BYTE_FREEIMAGE));
		...
		for (k = 0; k < PaletteSize ; k++) {
			...
			if (weight) {
				new_pal[k].rgbRed	= (BYTE_FREEIMAGE)(((float)Vol(&cube[k], mr) / (float)weight) + 0.5f);
				new_pal[k].rgbGreen = (BYTE_FREEIMAGE)(((float)Vol(&cube[k], mg) / (float)weight) + 0.5f);
				new_pal[k].rgbBlue	= (BYTE_FREEIMAGE)(((float)Vol(&cube[k], mb) / (float)weight) + 0.5f);
			} else {
				// Error: bogus box 'k'
		...
		for (unsigned y = 0; y < height; y++) {
			BYTE_FREEIMAGE *new_bits = FreeImage_GetBits(new_dib) + (y * npitch);
