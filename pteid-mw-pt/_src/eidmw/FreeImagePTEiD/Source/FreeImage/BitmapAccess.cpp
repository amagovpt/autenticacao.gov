...
FI_STRUCT (FREEIMAGEHEADER) {
	...
	BOOL_FREEIMAGE transparent;			// why another table? for easy transparency table retrieval!
	int  transparency_count;	// transparency could be stored in the palette, which is better
	BYTE_FREEIMAGE transparent_table[256];// overall, but it requires quite some changes and it will render
	...
	BOOL_FREEIMAGE has_pixels;			// FALSE if the FIBITMAP only contains the header and no pixel data

	FIBITMAP *thumbnail;		// optionally contains a thumbnail attached to the bitmap

	//BYTE_FREEIMAGE filler[1];			 // fill to 32-bit alignment
};
...
*/
static size_t 
FreeImage_GetImageSizeHeader(BOOL_FREEIMAGE header_only, unsigned width, unsigned height, unsigned bpp) {
...
FreeImage_AllocateHeaderT(BOOL_FREEIMAGE header_only, FREE_IMAGE_TYPE type, int width, int height, int bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {
	...
	if (bitmap != NULL) {
		...
		switch(type) {
			...
			case FIT_INT16:
				...
				break;
			case FIT_UINT32:
				bpp = 8 * sizeof(DWORD_FREEIMAGE);
				break;
			case FIT_INT32:
				bpp = 8 * sizeof(LONG_FREEIMAGE);
				break;
			case FIT_FLOAT:
				bpp = 8 * sizeof(float);
				break;
		...
		bitmap->data = (BYTE_FREEIMAGE *)FreeImage_Aligned_Malloc(dib_size * sizeof(BYTE_FREEIMAGE), FIBITMAP_ALIGNMENT);
...
FIBITMAP * DLL_CALLCONV
FreeImage_AllocateHeader(BOOL_FREEIMAGE header_only, int width, int height, int bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask) {
...
FreeImage_Clone(FIBITMAP *dib) {
	...
	BOOL_FREEIMAGE header_only = FreeImage_HasPixels(dib) ? FALSE : TRUE;
					...
					if(bkgnd_color->rgbRed == pal[i].rgbRed) {
						if(bkgnd_color->rgbGreen == pal[i].rgbGreen) {
							if(bkgnd_color->rgbBlue == pal[i].rgbBlue) {
								bkcolor->rgbReserved = (BYTE_FREEIMAGE)i;
								return TRUE;
							}
						}
					}
...
void DLL_CALLCONV
FreeImage_SetTransparencyTable(FIBITMAP *dib, BYTE_FREEIMAGE *table, int count) {
	if (dib) {
		count = MIN(count, 256);
		if (FreeImage_GetBPP(dib) <= 8) {
...
FreeImage_SetTransparentIndex(FIBITMAP *dib, int index) {
	if (dib) {
		...
		if (count) {
			BYTE_FREEIMAGE *new_tt = (BYTE_FREEIMAGE *)malloc(count * sizeof(BYTE_FREEIMAGE));
...
FreeImage_GetTransparentIndex(FIBITMAP *dib) {
	...
	BYTE_FREEIMAGE *tt = FreeImage_GetTransparencyTable(dib);
...
FreeImage_GetPalette(FIBITMAP *dib) {
	return (dib && FreeImage_GetBPP(dib) < 16) ? (RGBQUAD *)(((BYTE_FREEIMAGE *)FreeImage_GetInfoHeader(dib)) + sizeof(BITMAPINFOHEADER)) : NULL;
}
...
FreeImage_FindFirstMetadata(FREE_IMAGE_MDMODEL model, FIBITMAP *dib, FITAG **tag) {
	...
	if(tagmap) {
		...
		if(handle) {
			...
			handle->data = (BYTE_FREEIMAGE *)malloc(header_size * sizeof(BYTE_FREEIMAGE));
			
			if(handle->data) {
				memset(handle->data, 0, header_size * sizeof(BYTE_FREEIMAGE));
