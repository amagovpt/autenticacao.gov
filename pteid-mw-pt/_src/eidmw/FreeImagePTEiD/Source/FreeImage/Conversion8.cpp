...
void DLL_CALLCONV
FreeImage_ConvertLine1To8(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	for (int cols = 0; cols < width_in_pixels; cols++)
		target[cols] = (source[cols >> 3] & (0x80 >> (cols & 0x07))) != 0 ? 255 : 0;	
}

void DLL_CALLCONV
FreeImage_ConvertLine4To8(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	int count_new = 0;
	int count_org = 0;
...
void DLL_CALLCONV
FreeImage_ConvertLine16To8_555(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
...
void DLL_CALLCONV
FreeImage_ConvertLine16To8_565(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	WORD *bits = (WORD *)source;
...
void DLL_CALLCONV
FreeImage_ConvertLine24To8(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	for (int cols = 0; cols < width_in_pixels; cols++) {
...
void DLL_CALLCONV
FreeImage_ConvertLine32To8(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	for (int cols = 0; cols < width_in_pixels; cols++) {
		target[cols] = GREY(source[FI_RGBA_RED], source[FI_RGBA_GREEN], source[FI_RGBA_BLUE]);
...
FreeImage_ConvertTo8Bits(FIBITMAP *dib) {
	...
	if(bpp != 8) {
		...
		for(int i = 0; i < 256; i++) {
			new_pal[i].rgbRed	= (BYTE_FREEIMAGE)i;
			new_pal[i].rgbGreen = (BYTE_FREEIMAGE)i;
			new_pal[i].rgbBlue	= (BYTE_FREEIMAGE)i;
		}
		...
		if(image_type == FIT_BITMAP) {
			...
			switch(bpp) {
				case 1:
				{
					if(FreeImage_GetColorType(dib) == FIC_PALETTE) {
						...
						for(int i = 0; i < 256; i++) {
							new_pal[i].rgbRed = new_pal[i].rgbGreen = new_pal[i].rgbBlue = (BYTE_FREEIMAGE)(255 - i);
						}
					}
			...
			for (int rows = 0; rows < height; rows++) {
				...
				BYTE_FREEIMAGE *dst_pixel = (BYTE_FREEIMAGE*)dst_bits;
				for(int cols = 0; cols < width; cols++) {
					dst_pixel[cols] = (BYTE_FREEIMAGE)(src_pixel[cols] >> 8);
				}
				src_bits += src_pitch;
				dst_bits += dst_pitch;
			}
...
FreeImage_ConvertToGreyscale(FIBITMAP *dib) {
	...
	if((color_type == FIC_PALETTE) || (color_type == FIC_MINISWHITE)) {
		...
		BYTE_FREEIMAGE *buffer = (BYTE_FREEIMAGE*)malloc( CalculatePitch(CalculateLine(width, 24)) * sizeof(BYTE_FREEIMAGE) );
		if(NULL == buffer) {
