...
FreeImage_OpenMemory(BYTE_FREEIMAGE *data, DWORD_FREEIMAGE size_in_bytes) {
	// allocate a memory handle
	FIMEMORY *stream = (FIMEMORY*)malloc(sizeof(FIMEMORY));
	if(stream) {
		stream->data = (BYTE_FREEIMAGE*)malloc(sizeof(FIMEMORYHEADER));
		...
		if(stream->data) {
			...
			if(data && size_in_bytes) {
				...
				mem_header->data = (BYTE_FREEIMAGE*)data;
...
BOOL_FREEIMAGE DLL_CALLCONV
FreeImage_AcquireMemory(FIMEMORY *stream, BYTE_FREEIMAGE **data, DWORD_FREEIMAGE *size_in_bytes) {
	if (stream) {
		FIMEMORYHEADER *mem_header = (FIMEMORYHEADER*)(stream->data);

		*data = (BYTE_FREEIMAGE*)mem_header->data;
