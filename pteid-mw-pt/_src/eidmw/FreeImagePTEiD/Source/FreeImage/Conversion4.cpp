...
void DLL_CALLCONV
FreeImage_ConvertLine1To4(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
...
void DLL_CALLCONV
FreeImage_ConvertLine8To4(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels, RGBQUAD *palette) {
	BOOL_FREEIMAGE hinibble = TRUE;
...
void DLL_CALLCONV
FreeImage_ConvertLine16To4_555(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
...
void DLL_CALLCONV
FreeImage_ConvertLine16To4_565(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	WORD *bits = (WORD *)source;
	BOOL_FREEIMAGE hinibble = TRUE;
...
void DLL_CALLCONV
FreeImage_ConvertLine24To4(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	BOOL_FREEIMAGE hinibble = TRUE;
...
void DLL_CALLCONV
FreeImage_ConvertLine32To4(BYTE_FREEIMAGE *target, BYTE_FREEIMAGE *source, int width_in_pixels) {
	BOOL_FREEIMAGE hinibble = TRUE;
...
FreeImage_ConvertTo4Bits(FIBITMAP *dib) {
	...
	if(bpp != 4) {
		...
		for(int i = 0; i < 16; i++) {
			new_pal[i].rgbRed	= (BYTE_FREEIMAGE)((i << 4) + i);
			new_pal[i].rgbGreen = (BYTE_FREEIMAGE)((i << 4) + i);
			new_pal[i].rgbBlue	= (BYTE_FREEIMAGE)((i << 4) + i);
		}
		...
		switch(bpp) {
			case 1:
			{
				if(FreeImage_GetColorType(dib) == FIC_PALETTE) {
					...
					for(int i = 0; i < 16; i++) {
						new_pal[i].rgbRed = new_pal[i].rgbGreen = new_pal[i].rgbBlue = (BYTE_FREEIMAGE)(255 - ((i << 4) + i));
					}
				}
