...
BOOL_FREEIMAGE SwapRedBlue32(FIBITMAP* dib) {
	...
	BYTE_FREEIMAGE* line = FreeImage_GetBits(dib);
	for(unsigned y = 0; y < height; ++y, line += pitch) {
		for(BYTE_FREEIMAGE* pixel = line; pixel < line + lineSize ; pixel += bytesperpixel) {
...
static inline void 
assignRGB(BYTE_FREEIMAGE r, BYTE_FREEIMAGE g, BYTE_FREEIMAGE b, BYTE_FREEIMAGE* out) {
	out[FI_RGBA_RED]	= r;
	out[FI_RGBA_GREEN]	= g;
	out[FI_RGBA_BLUE]	= b;
}
...
template <class T>
static void 
_convertCMYKtoRGBA(unsigned width, unsigned height, BYTE_FREEIMAGE* line_start, unsigned pitch, unsigned samplesperpixel) {
	const BOOL_FREEIMAGE hasBlack = (samplesperpixel > 3) ? TRUE : FALSE;
...
ConvertCMYKtoRGBA(FIBITMAP* dib) {
	...
	const unsigned height = FreeImage_GetHeight(dib);
	BYTE_FREEIMAGE *line_start = FreeImage_GetScanLine(dib, 0);
	const unsigned pitch = FreeImage_GetPitch(dib);
	...
	if(channelSize == sizeof(WORD)) {
		...
		_convertCMYKtoRGBA<BYTE_FREEIMAGE>(width, height, line_start, pitch, samplesperpixel);
	}
...
template<class T>
static void 
_convertLABtoRGB(unsigned width, unsigned height, BYTE_FREEIMAGE* line_start, unsigned pitch, unsigned samplesperpixel) {
...
ConvertLABtoRGB(FIBITMAP* dib) {
	...
	if(channelSize == 1) {
		_convertLABtoRGB<BYTE_FREEIMAGE>(width, height, line_start, pitch, samplesperpixel);
	}
	else {
...
FIBITMAP * DLL_CALLCONV
FreeImage_ConvertFromRawBits(BYTE_FREEIMAGE *bits, int width, int height, int pitch, unsigned bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask, BOOL_FREEIMAGE topdown) {
...
void DLL_CALLCONV
FreeImage_ConvertToRawBits(BYTE_FREEIMAGE *bits, FIBITMAP *dib, int pitch, unsigned bpp, unsigned red_mask, unsigned green_mask, unsigned blue_mask, BOOL_FREEIMAGE topdown) {
	if (FreeImage_HasPixels(dib) && (bits != NULL)) {
		for (unsigned i = 0; i < FreeImage_GetHeight(dib); ++i) {
			BYTE_FREEIMAGE *scanline = FreeImage_GetScanLine(dib, topdown ? (FreeImage_GetHeight(dib) - i - 1) : i);
