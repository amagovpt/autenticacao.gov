...
CacheFile::CacheFile(const std::string filename, BOOL_FREEIMAGE keep_in_memory) :
m_file(NULL),
m_filename(filename),
m_free_pages(),
m_page_cache_mem(),
...
CacheFile::allocateBlock() {
	...
	block->data = new BYTE_FREEIMAGE[BLOCK_SIZE];
	block->next = 0;
	...
	if (m_current_block == NULL) {
		...
		if (it != m_page_map.end()) {
			...
			if (m_current_block->data == NULL) {
				m_current_block->data = new BYTE_FREEIMAGE[BLOCK_SIZE];
...
BOOL_FREEIMAGE
CacheFile::readFile(BYTE_FREEIMAGE *data, int nr, int size) {
	if ((data) && (size > 0)) {
		int s = 0;
		int block_nr = nr;
...
int
CacheFile::writeFile(BYTE_FREEIMAGE *data, int size) {
	if ((data) && (size > 0)) {
