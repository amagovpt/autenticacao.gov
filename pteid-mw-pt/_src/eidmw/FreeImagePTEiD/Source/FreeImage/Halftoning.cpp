...
#define INITERR(X, Y) (((int) X) - (((int) Y) ? WHITE : BLACK) + ((WHITE/2)-((int)X)) / 2)
	...
	for(y = 0; y < height; y++) {
		...
		p = (pixel > threshold) ? WHITE : BLACK;
		error = pixel - p;
		new_bits[width-1] = (BYTE_FREEIMAGE)p;
	}
	// top border
...
static FIBITMAP* OrderedDispersedDot(FIBITMAP *dib, int order) {
	...
	BYTE_FREEIMAGE *matrix = (BYTE_FREEIMAGE*)malloc(l*l * sizeof(BYTE_FREEIMAGE));
	for(int i = 0; i < l*l; i++) {
		// according to "Purdue University: Digital Image Processing Laboratory: Image Halftoning, April 30th, 2006
		matrix[i] = (BYTE_FREEIMAGE)( 255 * (((double)dithervalue(i / l, i % l, order) + 0.5) / (l*l)) );
	}
...
FreeImage_Dither(FIBITMAP *dib, FREE_IMAGE_DITHER algorithm) {
	...
	for(int i = 0; i < 256; i++) {
		grey_pal[i].rgbRed	= (BYTE_FREEIMAGE)i;
		grey_pal[i].rgbGreen = (BYTE_FREEIMAGE)i;
		grey_pal[i].rgbBlue	= (BYTE_FREEIMAGE)i;
	}
...
//
FIBITMAP * DLL_CALLCONV
FreeImage_Threshold(FIBITMAP *dib, BYTE_FREEIMAGE T) {
	FIBITMAP *dib8 = NULL;
	...
	for(int y = 0; y < height; y++) {
		BYTE_FREEIMAGE *bits8 = FreeImage_GetScanLine(dib8, y);
		BYTE_FREEIMAGE *bits1 = FreeImage_GetScanLine(new_dib, y);
