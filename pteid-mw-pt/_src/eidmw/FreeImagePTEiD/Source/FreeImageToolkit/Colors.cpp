		...
		switch(bpp) {
			...
			case 8 :
			{
				...
				if (FreeImage_GetColorType(src) == FIC_PALETTE) {
					...
					for(y = 0; y < height; y++) {
						BYTE_FREEIMAGE *bits = FreeImage_GetScanLine(src, y);
...
*/
BOOL_FREEIMAGE DLL_CALLCONV 
FreeImage_AdjustCurve(FIBITMAP *src, BYTE_FREEIMAGE *LUT, FREE_IMAGE_COLOR_CHANNEL channel) {
	unsigned x, y;
	BYTE_FREEIMAGE *bits = NULL;
...
FreeImage_AdjustGamma(FIBITMAP *src, double gamma) {
	BYTE_FREEIMAGE LUT[256];		// Lookup table
	...
	for(int i = 0; i < 256; i++) {
		...
		if(color > 255)
			color = 255;
		LUT[i] = (BYTE_FREEIMAGE)floor(color + 0.5);
	}
...
FreeImage_AdjustBrightness(FIBITMAP *src, double percentage) {
	...
	for(int i = 0; i < 256; i++) {
		...
		LUT[i] = (BYTE_FREEIMAGE)floor(value + 0.5);
	}
	return FreeImage_AdjustCurve(src, LUT, FICC_RGB);
}
...
*/
BOOL_FREEIMAGE DLL_CALLCONV 
FreeImage_GetHistogram(FIBITMAP *src, DWORD_FREEIMAGE *histo, FREE_IMAGE_COLOR_CHANNEL channel) {
	BYTE_FREEIMAGE pixel;
	BYTE_FREEIMAGE *bits = NULL;
	unsigned x, y;
	...
	if(bpp == 8) {
		...
		memset(histo, 0, 256 * sizeof(DWORD_FREEIMAGE));
 ...
 */
int DLL_CALLCONV
FreeImage_GetAdjustColorsLookupTable(BYTE_FREEIMAGE *LUT, double brightness, double contrast, double gamma, BOOL_FREEIMAGE invert) {
	double dblLUT[256];
	double value;
	int result = 0;
	...
	if (!invert) {
		for (int i = 0; i < 256; i++) {
			LUT[i] = (BYTE_FREEIMAGE)floor(dblLUT[i] + 0.5);
		}
	} else {
		for (int i = 0; i < 256; i++) {
			LUT[i] = 255 - (BYTE_FREEIMAGE)floor(dblLUT[i] + 0.5);
		}
		result++;
	}
 ...
 */
BOOL_FREEIMAGE DLL_CALLCONV
FreeImage_AdjustColors(FIBITMAP *dib, double brightness, double contrast, double gamma, BOOL_FREEIMAGE invert) {
	BYTE_FREEIMAGE LUT[256];
...
FreeImage_ApplyColorMapping(FIBITMAP *dib, RGBQUAD *srccolors, RGBQUAD *dstcolors, unsigned count, BOOL_FREEIMAGE ignore_alpha, BOOL_FREEIMAGE swap) {
	unsigned result = 0;
	...
	switch (bpp) {
		...
		case 24: {
			...
			RGBQUAD *a, *b;
			for (unsigned y = 0; y < height; y++) {
				BYTE_FREEIMAGE *bits = FreeImage_GetScanLine(dib, y);
 ...
 */
unsigned DLL_CALLCONV
FreeImage_SwapColors(FIBITMAP *dib, RGBQUAD *color_a, RGBQUAD *color_b, BOOL_FREEIMAGE ignore_alpha) {
 ...
 */
unsigned DLL_CALLCONV
FreeImage_ApplyPaletteIndexMapping(FIBITMAP *dib, BYTE_FREEIMAGE *srcindices,	BYTE_FREEIMAGE *dstindices, unsigned count, BOOL_FREEIMAGE swap) {
	unsigned result = 0;
 ...
 */
unsigned DLL_CALLCONV 
FreeImage_SwapPaletteIndices(FIBITMAP *dib, BYTE_FREEIMAGE *index_a, BYTE_FREEIMAGE *index_b) {
