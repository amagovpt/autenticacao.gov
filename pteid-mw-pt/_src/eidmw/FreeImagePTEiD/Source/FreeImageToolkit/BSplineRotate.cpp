...
static FIBITMAP * Rotate8Bit(FIBITMAP *dib, double angle, double x_shift, double y_shift, double x_origin, double y_origin, long spline_degree, BOOL_FREEIMAGE use_mask);
...
*/
static FIBITMAP * 
Rotate8Bit(FIBITMAP *dib, double angle, double x_shift, double y_shift, double x_origin, double y_origin, long spline_degree, BOOL_FREEIMAGE use_mask) {
	...
	for(int i = 0; i < 256; i++) {
		pal[i].rgbRed = pal[i].rgbGreen = pal[i].rgbBlue = (BYTE_FREEIMAGE)i;
	}
	...
	for(y = 0; y < height; y++) {
		...
		BYTE_FREEIMAGE *src_bits = FreeImage_GetScanLine(dib, height-1-y);
	...
	for(y = 0; y < height; y++) {
		BYTE_FREEIMAGE *dst_bits = FreeImage_GetScanLine(dst, height-1-y);
		...
		for(x = 0; x < width; x++) {
			...
			// clamp and convert to BYTE_FREEIMAGE
			dst_bits[x] = (BYTE_FREEIMAGE)MIN(MAX((int)0, (int)(p + 0.5)), (int)255);
		}
	}
...
FreeImage_RotateEx(FIBITMAP *dib, double angle, double x_shift, double y_shift, double x_origin, double y_origin, BOOL_FREEIMAGE use_mask) {

	int x, y, bpp;
	int channel, nb_channels;
	BYTE_FREEIMAGE *src_bits, *dst_bits;
