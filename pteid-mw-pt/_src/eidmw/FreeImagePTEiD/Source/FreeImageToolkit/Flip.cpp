...
FreeImage_FlipHorizontal(FIBITMAP *src) {
	...
	BYTE_FREEIMAGE *new_bits = (BYTE_FREEIMAGE*)FreeImage_Aligned_Malloc(line * sizeof(BYTE_FREEIMAGE), FIBITMAP_ALIGNMENT);
		...
		switch (FreeImage_GetBPP(src)) {
			...
			{				
				for(unsigned x = 0; x < width; x++) {
					// get pixel at (x, y)
					BOOL_FREEIMAGE value = (new_bits[x >> 3] & (0x80 >> (x & 0x07))) != 0;
			...
			case 4 :
			{
				for(unsigned c = 0; c < line; c++) {
					...
					BYTE_FREEIMAGE nibble = (bits[c] & 0xF0) >> 4;
			...
			case 8:
			{				
				BYTE_FREEIMAGE *dst_data = (BYTE_FREEIMAGE*) bits; 				
				BYTE_FREEIMAGE *src_data = (BYTE_FREEIMAGE*) (new_bits + line - bytespp); 				
...
FreeImage_FlipVertical(FIBITMAP *src) {
	...
	// copy between aligned memories
	Mid = (BYTE_FREEIMAGE*)FreeImage_Aligned_Malloc(pitch * sizeof(BYTE_FREEIMAGE), FIBITMAP_ALIGNMENT);
	if (!Mid) return FALSE;
