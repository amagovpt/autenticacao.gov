...
FreeImage_Composite(FIBITMAP *fg, BOOL_FREEIMAGE useFileBkg, RGBQUAD *appBkColor, FIBITMAP *bg) {
	...
	int x, y, c;
	BYTE_FREEIMAGE alpha = 0, not_alpha;
	BYTE_FREEIMAGE index;
	...
	BOOL_FREEIMAGE bIsTransparent = FreeImage_IsTransparent(fg);
	BYTE_FREEIMAGE *trns = FreeImage_GetTransparencyTable(fg);

	// retrieve the background color from the foreground image
	BOOL_FREEIMAGE bHasBkColor = FALSE;
	...
	for(y = 0; y < height; y++) {
		// foreground
		BYTE_FREEIMAGE *fg_bits = FreeImage_GetScanLine(fg, y);
		// background
		BYTE_FREEIMAGE *bg_bits = FreeImage_GetScanLine(bg, y);
		// composite image
		BYTE_FREEIMAGE *cp_bits = FreeImage_GetScanLine(composite, y);
		...
		for(x = 0; x < width; x++) {
			...
			if(!bHasBkColor) {
				if(bg) {
					...
					c = c ? c : 255;
					bkc.rgbBlue  = (BYTE_FREEIMAGE)c;
					bkc.rgbGreen = (BYTE_FREEIMAGE)c;
					bkc.rgbRed   = (BYTE_FREEIMAGE)c;
				}
			}
			...
			if(alpha == 0) {
				...
				not_alpha = (BYTE_FREEIMAGE)~alpha;
				cp_bits[FI_RGBA_BLUE] = (BYTE_FREEIMAGE)((alpha * (WORD)fgc.rgbBlue  + not_alpha * (WORD)bkc.rgbBlue) >> 8);
				cp_bits[FI_RGBA_GREEN] = (BYTE_FREEIMAGE)((alpha * (WORD)fgc.rgbGreen + not_alpha * (WORD)bkc.rgbGreen) >> 8);
				cp_bits[FI_RGBA_RED] = (BYTE_FREEIMAGE)((alpha * (WORD)fgc.rgbRed   + not_alpha * (WORD)bkc.rgbRed) >> 8);
			}
...
FreeImage_PreMultiplyWithAlpha(FIBITMAP *dib) {
	...
	for(int y = 0; y < height; y++) {
		...
		for (int x = 0; x < width; x++, bits += 4) {
			const BYTE_FREEIMAGE alpha = bits[FI_RGBA_ALPHA];
			...
			if(alpha == 0x00) {
				...
				bits[FI_RGBA_BLUE] = (BYTE_FREEIMAGE)( (alpha * (WORD)bits[FI_RGBA_BLUE] + 127) / 255 );
				bits[FI_RGBA_GREEN] = (BYTE_FREEIMAGE)( (alpha * (WORD)bits[FI_RGBA_GREEN] + 127) / 255 );
				bits[FI_RGBA_RED] = (BYTE_FREEIMAGE)( (alpha * (WORD)bits[FI_RGBA_RED] + 127) / 255 );
			}
		}
	}
	return TRUE;
