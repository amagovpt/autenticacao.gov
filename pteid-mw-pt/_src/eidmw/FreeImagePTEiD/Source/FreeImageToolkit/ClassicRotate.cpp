...
Parameter T can be BYTE_FREEIMAGE, WORD of float. 
...
HorizontalSkewT(FIBITMAP *src, FIBITMAP *dst, int row, int iOffset, double weight, const void *bkcolor = NULL) {
	...
	BYTE_FREEIMAGE *src_bits = FreeImage_GetScanLine(src, row);
	BYTE_FREEIMAGE *dst_bits = FreeImage_GetScanLine(dst, row);
	...
	switch(image_type) {
		case FIT_BITMAP:
			switch(FreeImage_GetBPP(src)) {
				...
				case 24:
				case 32:
					HorizontalSkewT<BYTE_FREEIMAGE>(src, dst, row, iOffset, dWeight, bkcolor);
				break;
			}
			break;
		case FIT_UINT16:
...
VerticalSkewT(FIBITMAP *src, FIBITMAP *dst, int col, int iOffset, double weight, const void *bkcolor = NULL) {
	...
	BYTE_FREEIMAGE *src_bits = FreeImage_GetBits(src) + index;
	BYTE_FREEIMAGE *dst_bits = FreeImage_GetBits(dst) + index;
	...
	switch(image_type) {
		case FIT_BITMAP:
			switch(FreeImage_GetBPP(src)) {
				...
				case 24:
				case 32:
					VerticalSkewT<BYTE_FREEIMAGE>(src, dst, col, iOffset, dWeight, bkcolor);
					break;
			}
			break;
			case FIT_UINT16:
	...
	switch(image_type) {
		case FIT_BITMAP:
			if(bpp == 1) {
				...
				BYTE_FREEIMAGE *sbits, *dbits, *dbitsmax, bitpos, *nrow, *srcdisp;
				div_t div_r;

				BYTE_FREEIMAGE *bsrc  = FreeImage_GetBits(src); 
				BYTE_FREEIMAGE *bdest = FreeImage_GetBits(dst);
				...
				for(y = 0; y < src_height; y++) {
					...
					bitpos = (BYTE_FREEIMAGE)(128 >> div_r.rem);
				...
				BYTE_FREEIMAGE *bsrc  = FreeImage_GetBits(src);  // source pixels
				BYTE_FREEIMAGE *bdest = FreeImage_GetBits(dst);  // destination pixels
				...
				for(xs = 0; xs < dst_width; xs += RBLOCK) {    // for all image blocks of RBLOCK*RBLOCK pixels
					for(ys = 0; ys < dst_height; ys += RBLOCK) {
						for(y = ys; y < MIN(dst_height, ys + RBLOCK); y++) {    // do rotation
							...
							BYTE_FREEIMAGE *src_bits = bsrc + (xs * src_pitch) + (y2 * bytespp);
							// point to dst pixel at (xs, y)
							BYTE_FREEIMAGE *dst_bits = bdest + (y * dst_pitch) + (xs * bytespp);
		...
		case FIT_RGBAF:
		{
			...
			for(y = 0; y < dst_height; y++) {
				BYTE_FREEIMAGE *src_bits = bsrc + (src_width - 1 - y) * bytespp;
				BYTE_FREEIMAGE *dst_bits = bdest + (y * dst_pitch);
		...
		case FIT_BITMAP:
			if(bpp == 1) {
				for(int y = 0; y < src_height; y++) {
					BYTE_FREEIMAGE *src_bits = FreeImage_GetScanLine(src, y);
					BYTE_FREEIMAGE *dst_bits = FreeImage_GetScanLine(dst, dst_height - y - 1);
				...
				for(y = 0; y < src_height; y++) {
					BYTE_FREEIMAGE *src_bits = FreeImage_GetScanLine(src, y);
					BYTE_FREEIMAGE *dst_bits = FreeImage_GetScanLine(dst, dst_height - y - 1) + (dst_width - 1) * bytespp;
...
Rotate270(FIBITMAP *src) {
	...
	switch(image_type) {
		case FIT_BITMAP:
			if(bpp == 1) {
				...
				for(y = 0; y < src_height; y++) {
					...
					// set bit pos of src column byte
					bitpos = (BYTE_FREEIMAGE)(1 << div_r.rem);
					srcdisp = bsrc + y * src_pitch;
					for (x = 0; x < src_pitch; x++) {
						// get source bits
						sbits = srcdisp + x;
				...
				for(xs = 0; xs < dst_width; xs += RBLOCK) {    // for all image blocks of RBLOCK*RBLOCK pixels
					for(ys = 0; ys < dst_height; ys += RBLOCK) {
						for(x = xs; x < MIN(dst_width, xs + RBLOCK); x++) {    // do rotation
							...
							BYTE_FREEIMAGE *src_bits = bsrc + (x2 * src_pitch) + (ys * bytespp);
							// point to dst pixel at (x, ys)
							BYTE_FREEIMAGE *dst_bits = bdest + (ys * dst_pitch) + (x * bytespp);
		...
		case FIT_RGBAF:
		{
			...
			for(y = 0; y < dst_height; y++) {
				BYTE_FREEIMAGE *src_bits = bsrc + (src_height - 1) * src_pitch + y * bytespp;
				BYTE_FREEIMAGE *dst_bits = bdest + (y * dst_pitch);
				for(x = 0; x < dst_width; x++) {
					for(int j = 0; j < bytespp; j++) {
						dst_bits[j] = src_bits[j];					
