...
*/
CWeightsTable::CWeightsTable(CGenericFilter *pFilter, DWORD_FREEIMAGE uDstSize, DWORD_FREEIMAGE uSrcSize) {
	DWORD_FREEIMAGE u;
	double dWidth;
	double dFScale = 1.0;
...
CWeightsTable::~CWeightsTable() {
	for(DWORD_FREEIMAGE u = 0; u < m_LineLength; u++) {
...
FIBITMAP* CResizeEngine::scale(FIBITMAP *src, unsigned dst_width, unsigned dst_height) { 
	DWORD_FREEIMAGE src_width  = FreeImage_GetWidth(src); 
	DWORD_FREEIMAGE src_height = FreeImage_GetHeight(src);
		...
		if(FreeImage_GetColorType(src) == FIC_MINISWHITE) {
			...
			for(int i = 0; i < 256; i++) {
				dst_pal[i].rgbRed = dst_pal[i].rgbGreen =
					dst_pal[i].rgbBlue = (BYTE_FREEIMAGE)(255 - i);
			}
		} else {
			...
			for(int i = 0; i < 256; i++) {
				dst_pal[i].rgbRed = dst_pal[i].rgbGreen =
					dst_pal[i].rgbBlue = (BYTE_FREEIMAGE)i;
			}
		}
	}
...
void CResizeEngine::horizontalFilter(FIBITMAP *src, unsigned src_width, unsigned src_height, FIBITMAP *dst, unsigned dst_width, unsigned dst_height) { 
	if(dst_width == src_width) {
		...
		switch(FreeImage_GetBPP(src)) {
			...
			default:
			{
				BYTE_FREEIMAGE *src_bits = FreeImage_GetBits(src);
						...
						for(unsigned y = 0; y < dst_height; y++) {
							...
							for(unsigned x = 0; x < dst_width; x++) {
								...
								for(int i = iLeft; i <= iRight; i++) {
									...
									BYTE_FREEIMAGE pixel = (src_bits[i >> 3] & (0x80 >> (i & 0x07))) != 0;
									value += (weight * (double)pixel);
								} 
								value *= 255;

								// clamp and place result in destination pixel
								dst_bits[x] = (BYTE_FREEIMAGE)MIN(MAX((int)0, (int)(value + 0.5)), (int)255);
							} 
						}
					}
					break;
					...
					case 32:
					{
						...
						for(unsigned y = 0; y < dst_height; y++) {
							...
							for(unsigned x = 0; x < dst_width; x++) {
								...
								// clamp and place result in destination pixel
								for (unsigned j = 0; j < bytespp; j++) {
									dst_bits[j] = (BYTE_FREEIMAGE)MIN(MAX((int)0, (int)(value[j] + 0.5)), (int)255);
								}
				...
				switch(FreeImage_GetBPP(src)) {
					case 1:
					{
						...
						for(unsigned x = 0; x < dst_width; x++) {
							...
							BYTE_FREEIMAGE *dst_bits = FreeImage_GetBits(dst) + x;
							...
							for(unsigned y = 0; y < dst_height; y++) {
								...
								BYTE_FREEIMAGE *src_bits = FreeImage_GetScanLine(src, iLeft);
								...
								for(int i = iLeft; i <= iRight; i++) {
									...
									BYTE_FREEIMAGE pixel = (src_bits[x >> 3] & (0x80 >> (x & 0x07))) != 0;
									value += (weight * (double)pixel);
								...
								// clamp and place result in destination pixel
								*dst_bits = (BYTE_FREEIMAGE)MIN(MAX((int)0, (int)(value + 0.5)), (int)255);
					...
					case 32:
					{
						...
						for(unsigned x = 0; x < dst_width; x++) {
							...
							for(unsigned y = 0; y < dst_height; y++) {
								...
								BYTE_FREEIMAGE *src_bits = FreeImage_GetScanLine(src, iLeft) + index;
