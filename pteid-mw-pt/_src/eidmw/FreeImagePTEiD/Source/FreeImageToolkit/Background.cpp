...
GetPaletteIndex(FIBITMAP *dib, const RGBQUAD *color, int options, FREE_IMAGE_COLOR_TYPE *color_type) {
	...
	if (options & FI_COLOR_FIND_EQUAL_COLOR) {
		...
		unsigned ncolors = FreeImage_GetColorsUsed(dib);
		BYTE_FREEIMAGE *palette = (BYTE_FREEIMAGE *)FreeImage_GetPalette(dib);
		BYTE_FREEIMAGE red, green, blue;
		if (!IsVisualGreyscaleImage(dib)) {
			red = color->rgbRed;
...
GetAlphaBlendedColor(const RGBQUAD *bgcolor, const RGBQUAD *fgcolor, RGBQUAD *blended) {
	...
	BYTE_FREEIMAGE alpha = fgcolor->rgbReserved;
	BYTE_FREEIMAGE not_alpha = ~alpha;
	
	blended->rgbRed   = (BYTE_FREEIMAGE)( ((WORD)fgcolor->rgbRed   * alpha + not_alpha * (WORD)bgcolor->rgbRed)   >> 8 );
	blended->rgbGreen = (BYTE_FREEIMAGE)( ((WORD)fgcolor->rgbGreen * alpha + not_alpha * (WORD)bgcolor->rgbGreen) >> 8) ;
	blended->rgbBlue  = (BYTE_FREEIMAGE)( ((WORD)fgcolor->rgbRed   * alpha + not_alpha * (WORD)bgcolor->rgbBlue)  >> 8 );
...
FillBackgroundBitmap(FIBITMAP *dib, const RGBQUAD *color, int options) {
	...
	BYTE_FREEIMAGE *src_bits = FreeImage_GetScanLine(dib, 0);
	BYTE_FREEIMAGE *dst_bits = src_bits;	
	
	BOOL_FREEIMAGE supports_alpha = ((bpp >= 24) || ((bpp == 8) && (color_type != FIC_PALETTE)));
			...
			case 1: {
				...
				if (palette != NULL) {
					...
					if ((*urgb & 0xFFFFFF) == 0x000000) {
						...
						BYTE_FREEIMAGE index = ((RGBQUAD *)color)->rgbReserved & 0x01;
			...
			case 4: {
				...
				if (palette != NULL) {
					...
					if ((rgb->rgbRed == rgb->rgbGreen) && (rgb->rgbRed == rgb->rgbBlue)) {
						...
						// black-only palette. We use color->rgbReserved as the
						// desired palette index.
						BYTE_FREEIMAGE index = (rgb->rgbReserved & 0x0F);
			...
			case 8: {
				...
				if (palette != NULL) {
					...
					if ((rgb->rgbRed == rgb->rgbGreen) && (rgb->rgbRed == rgb->rgbBlue)) {
						...
						// black-only palette. We use color->rgbReserved as the
						// desired palette index.
						BYTE_FREEIMAGE index = rgb->rgbReserved;
						((unsigned *)pal)[index] = *((unsigned *)rgb) & 0x00FFFFFF;  
					}
					options |= FI_COLOR_ALPHA_IS_INDEX;
				}
...
FreeImage_EnlargeCanvas(FIBITMAP *src, int left, int top, int right, int bottom, const void *color, int options) {
	...
	if ((type == FIT_BITMAP) && (bpp <= 4)) {
		...
		int bytespp = bpp / 8;
		BYTE_FREEIMAGE *srcPtr = FreeImage_GetScanLine(src, height - 1 - ((top >= 0) ? 0 : -top));
		BYTE_FREEIMAGE *dstPtr = FreeImage_GetScanLine(dst, newHeight - 1 - ((top <= 0) ? 0 : top));
