...
FI_STRUCT (FITAGHEADER) { 
	...
	DWORD_FREEIMAGE count;		// number of components (in 'tag data types' units)
	DWORD_FREEIMAGE length;		// value length in bytes
...
FreeImage_CreateTag() {
	...
	if (tag != NULL) {
		...
		tag->data = (BYTE_FREEIMAGE *)malloc(tag_size * sizeof(BYTE_FREEIMAGE));
		if (tag->data != NULL) {
...
FreeImage_CloneTag(FITAG *tag) {
	...
	switch(dst_tag->type) {
		case FIDT_ASCII:
			...
			break;
		default:
			dst_tag->value = (BYTE_FREEIMAGE*)malloc(src_tag->length * sizeof(BYTE_FREEIMAGE));
...
BOOL_FREEIMAGE DLL_CALLCONV 
FreeImage_SetTagCount(FITAG *tag, DWORD_FREEIMAGE count) {
	if(tag) {
		FITAGHEADER *tag_header = (FITAGHEADER *)tag->data;
...
BOOL_FREEIMAGE DLL_CALLCONV 
FreeImage_SetTagLength(FITAG *tag, DWORD_FREEIMAGE length) {
	if(tag) {
		FITAGHEADER *tag_header = (FITAGHEADER *)tag->data;
	...
	if(tag && value) {
		...
		switch(tag_header->type) {
			case FIDT_ASCII:
			{
				...
				for(DWORD_FREEIMAGE i = 0; i < tag_header->length; i++) {
			...
			default:
				tag_header->value = malloc(tag_header->length * sizeof(BYTE_FREEIMAGE));
				if(!tag_header->value) {
					return FALSE;
				}
