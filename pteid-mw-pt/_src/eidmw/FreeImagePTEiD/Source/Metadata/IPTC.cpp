...
*/
BOOL_FREEIMAGE 
read_iptc_profile(FIBITMAP *dib, const BYTE_FREEIMAGE *dataptr, unsigned int datalen) {
	char defaultKey[16];
    ...
    while (offset < length) {
		...
		BYTE_FREEIMAGE *iptc_value = (BYTE_FREEIMAGE*)malloc((tagByteCount + 1) * sizeof(BYTE_FREEIMAGE));
		memset(iptc_value, 0, (tagByteCount + 1) * sizeof(BYTE_FREEIMAGE));
	...
	if(Keywords.length()) {
		...
		FreeImage_SetTagLength(tag, (DWORD_FREEIMAGE)Keywords.length());
		FreeImage_SetTagCount(tag, (DWORD_FREEIMAGE)Keywords.length());
	...
	if(SupplementalCategory.length()) {
		...
		FreeImage_SetTagLength(tag, (DWORD_FREEIMAGE)SupplementalCategory.length());
		FreeImage_SetTagCount(tag, (DWORD_FREEIMAGE)SupplementalCategory.length());
...
static BYTE_FREEIMAGE* 
append_iptc_tag(BYTE_FREEIMAGE *profile, unsigned *profile_size, WORD id, DWORD_FREEIMAGE length, const void *value) {
	BYTE_FREEIMAGE *buffer = NULL;

	// calculate the new buffer size
	size_t buffer_size = (5 + *profile_size + length) * sizeof(BYTE_FREEIMAGE);
	buffer = (BYTE_FREEIMAGE*)malloc(buffer_size);
	if(!buffer)
		return NULL;
	...
	buffer[1] = 0x02;
	// add the tag type
	buffer[2] = (BYTE_FREEIMAGE)(id & 0x00FF);
	// add the tag length
	buffer[3] = (BYTE_FREEIMAGE)(length >> 8);
	buffer[4] = (BYTE_FREEIMAGE)(length & 0xFF);
	// add the tag value
	memcpy(buffer + 5, (BYTE_FREEIMAGE*)value, length);
...
*/
BOOL_FREEIMAGE 
write_iptc_profile(FIBITMAP *dib, BYTE_FREEIMAGE **profile, unsigned *profile_size) {
	FITAG *tag = NULL;
	...
	if(mdhandle) {
		do {
			...
			switch(tag_id) {
				...
				case TAG_KEYWORDS:
					if(FreeImage_GetTagType(tag) == FIDT_ASCII) {
						...
						for(int i = 0; i < (int)output.size(); i++) {
							...
							buffer = append_iptc_tag(buffer, &buffer_size, tag_id, (DWORD_FREEIMAGE)tag_value.length(), tag_value.c_str());
						}
				...
				case TAG_URGENCY:
					if(FreeImage_GetTagType(tag) == FIDT_ASCII) {
						DWORD_FREEIMAGE length = 1;	// keep the first octet only
				...
				default:
					if(FreeImage_GetTagType(tag) == FIDT_ASCII) {
						DWORD_FREEIMAGE length = FreeImage_GetTagLength(tag);	
						buffer = append_iptc_tag(buffer, &buffer_size, tag_id, length, FreeImage_GetTagValue(tag));
					}					
					break;
			}
