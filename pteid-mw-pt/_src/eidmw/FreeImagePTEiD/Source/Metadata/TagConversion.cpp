...
ConvertAnyTag(FITAG *tag) {
	...
	DWORD_FREEIMAGE tag_count = FreeImage_GetTagCount(tag);

	switch(tag_type) {
		case FIDT_BYTE_FREEIMAGE:		// N x 8-bit unsigned integer 
		{
			BYTE_FREEIMAGE *pvalue = (BYTE_FREEIMAGE*)FreeImage_GetTagValue(tag);

			sprintf(format, "%ld",	(LONG_FREEIMAGE) pvalue[0]);
			buffer += format;
			for(i = 1; i < tag_count; i++) {
				sprintf(format, " %ld",	(LONG_FREEIMAGE) pvalue[i]);
				buffer += format;
			}
			break;
		}
		...
		case FIDT_LONG_FREEIMAGE:		// N x 32-bit unsigned integer 
		{
			DWORD_FREEIMAGE *pvalue = (DWORD_FREEIMAGE *)FreeImage_GetTagValue(tag);
		...
		case FIDT_SBYTE_FREEIMAGE:	// N x 8-bit signed integer 
		{
		...
		case FIDT_SLONG_FREEIMAGE:	// N x 32-bit signed integer 
		{
			LONG_FREEIMAGE *pvalue = (LONG_FREEIMAGE *)FreeImage_GetTagValue(tag);
...
ConvertExifTag(FITAG *tag) {
	...
	switch(FreeImage_GetTagID(tag)) {
		...
		case TAG_COMPONENTS_CONFIGURATION:
		{
			...
			BYTE_FREEIMAGE *pvalue = (BYTE_FREEIMAGE*)FreeImage_GetTagValue(tag);
			for(DWORD_FREEIMAGE i = 0; i < MIN((DWORD_FREEIMAGE)4, FreeImage_GetTagCount(tag)); i++) {
				int j = pvalue[i];
				if(j > 0 && j < 7)
		...
		{
			FIRational r(tag);
			LONG_FREEIMAGE apexValue = r.longValue();
			LONG_FREEIMAGE apexPower = 1 << apexValue;
		...
		case TAG_SCENE_TYPE:
		{
			BYTE_FREEIMAGE sceneType = *((BYTE_FREEIMAGE*)FreeImage_GetTagValue(tag));
			if (sceneType == 1) {
		...
		{
			BYTE_FREEIMAGE fileSource = *((BYTE_FREEIMAGE*)FreeImage_GetTagValue(tag));
			if (fileSource == 3) {
		...
		case TAG_USER_COMMENT:
		{
			...
			const BYTE_FREEIMAGE *userComment = (BYTE_FREEIMAGE*)FreeImage_GetTagValue(tag);
			for(DWORD_FREEIMAGE i = 8; i < FreeImage_GetTagLength(tag); i++) {
...
ConvertExifGPSTag(FITAG *tag) {
	...
	switch(FreeImage_GetTagID(tag)) {
		...
		case TAG_GPS_LONG_FREEIMAGEITUDE:
		...
		case TAG_GPS_LONG_FREEIMAGEITUDE_REF:
		...
		case TAG_GPS_DEST_LONG_FREEIMAGEITUDE_REF:
		case TAG_GPS_DEST_LONG_FREEIMAGEITUDE:
