...
FreeImage_strnicmp(const char *s1, const char *s2, size_t len) {
	...
	if(len) {
		do {
			...
			if (c1 == c2)
				continue;
			c1 = (BYTE_FREEIMAGE)tolower(c1);
			c2 = (BYTE_FREEIMAGE)tolower(c2);
			if (c1 != c2)
				break;
		} while (--len);
	}
...
static short 
ReadInt16(BOOL_FREEIMAGE msb_order, const void *buffer) {
	short value;

	if(msb_order) {
		value = (short)((((BYTE_FREEIMAGE*) buffer)[0] << 8) | ((BYTE_FREEIMAGE*) buffer)[1]);
		return value;
    }
	value = (short)((((BYTE_FREEIMAGE*) buffer)[1] << 8) | ((BYTE_FREEIMAGE*) buffer)[0]);
	return value;
}

static LONG_FREEIMAGE 
ReadInt32(BOOL_FREEIMAGE msb_order, const void *buffer) {
	LONG_FREEIMAGE value;

	if(msb_order) {
		value = (LONG_FREEIMAGE)((((BYTE_FREEIMAGE*) buffer)[0] << 24) | (((BYTE_FREEIMAGE*) buffer)[1] << 16) | (((BYTE_FREEIMAGE*) buffer)[2] << 8) | (((BYTE_FREEIMAGE*) buffer)[3]));
		return value;
    }
	value = (LONG_FREEIMAGE)((((BYTE_FREEIMAGE*) buffer)[3] << 24) | (((BYTE_FREEIMAGE*) buffer)[2] << 16) | (((BYTE_FREEIMAGE*) buffer)[1] << 8 ) | (((BYTE_FREEIMAGE*) buffer)[0]));
	return value;
}

static unsigned short 
ReadUint16(BOOL_FREEIMAGE msb_order, const void *buffer) {
	unsigned short value;
	
	if(msb_order) {
		value = (unsigned short) ((((BYTE_FREEIMAGE*) buffer)[0] << 8) | ((BYTE_FREEIMAGE*) buffer)[1]);
		return value;
    }
	value = (unsigned short) ((((BYTE_FREEIMAGE*) buffer)[1] << 8) | ((BYTE_FREEIMAGE*) buffer)[0]);
	return value;
}

static DWORD_FREEIMAGE 
ReadUint32(BOOL_FREEIMAGE msb_order, const void *buffer) {
	return ((DWORD_FREEIMAGE) ReadInt32(msb_order, buffer) & 0xFFFFFFFF);
}
...
*/
static void 
processIFDOffset(FITAG *tag, char *pval, BOOL_FREEIMAGE msb_order, DWORD_FREEIMAGE *subdirOffset, TagLib::MDMODEL *md_model) {
	// get the IFD offset
	*subdirOffset = (DWORD_FREEIMAGE) ReadUint32(msb_order, pval);
...
*/
static void 
processMakerNote(FIBITMAP *dib, char *pval, BOOL_FREEIMAGE msb_order, DWORD_FREEIMAGE *subdirOffset, TagLib::MDMODEL *md_model) {
	FITAG *tagMake = NULL;
	...
	*md_model = TagLib::UNKNOWN;
	...
	if((strncmp("OLYMP\x00\x01", pval, 7) == 0) || (strncmp("OLYMP\x00\x02", pval, 7) == 0) || (strncmp("EPSON", pval, 5) == 0) || (strncmp("AGFA", pval, 4) == 0)) {
		...
		*md_model = TagLib::EXIF_MAKERNOTE_FUJIFILM;
        DWORD_FREEIMAGE ifdStart = (DWORD_FREEIMAGE) ReadUint32(msb_order, pval + 8);
...
processCanonMakerNoteTag(FIBITMAP *dib, FITAG *tag) {
    ...
    for (DWORD_FREEIMAGE i = startIndex; i < FreeImage_GetTagCount(tag); i++) {
...
*/
static void 
processExifTag(FIBITMAP *dib, FITAG *tag, char *pval, BOOL_FREEIMAGE msb_order, TagLib::MDMODEL md_model) {
	char defaultKey[16];
	int n;
	DWORD_FREEIMAGE i;

	// allocate a buffer to store the tag value
	BYTE_FREEIMAGE *exif_value = (BYTE_FREEIMAGE*)malloc(FreeImage_GetTagLength(tag) * sizeof(BYTE_FREEIMAGE));
	if(NULL == exif_value) {
		// out of memory ...
		return;
	}
	memset(exif_value, 0, FreeImage_GetTagLength(tag) * sizeof(BYTE_FREEIMAGE));
	...
	switch(FreeImage_GetTagType(tag)) {
		...
		case FIDT_LONG_FREEIMAGE:
		{
			DWORD_FREEIMAGE *value = (DWORD_FREEIMAGE*)&exif_value[0];
			for(i = 0; i < FreeImage_GetTagCount(tag); i++) {
				value[i] = ReadUint32(msb_order, pval + i * sizeof(DWORD_FREEIMAGE));
			}
			FreeImage_SetTagValue(tag, value);
			break;
		}
		case FIDT_SLONG_FREEIMAGE:
		{
			LONG_FREEIMAGE *value = (LONG_FREEIMAGE*)&exif_value[0];
			for(i = 0; i < FreeImage_GetTagCount(tag); i++) {
				value[i] = ReadInt32(msb_order, pval + i * sizeof(LONG_FREEIMAGE));
			}
			FreeImage_SetTagValue(tag, value);
			break;
		}
		...
		case FIDT_BYTE_FREEIMAGE:
		case FIDT_ASCII:
		case FIDT_SBYTE_FREEIMAGE:
		case FIDT_UNDEFINED:
		case FIDT_FLOAT:
		case FIDT_DOUBLE:
		default:
...
*/
static BOOL_FREEIMAGE 
jpeg_read_exif_dir(FIBITMAP *dib, const BYTE_FREEIMAGE *tiffp, unsigned long offset, unsigned int length, BOOL_FREEIMAGE msb_order) {
	WORD de, nde;

	std::stack<WORD>			destack;	// directory entries stack
	std::stack<const BYTE_FREEIMAGE*>		ifdstack;	// IFD stack
	...
	std::map<DWORD_FREEIMAGE, int> visitedIFD;
	...
	*/
	...
	const BYTE_FREEIMAGE *ifd0th = (BYTE_FREEIMAGE*)tiffp + offset;

	const BYTE_FREEIMAGE *ifdp = ifd0th;
	...
	do {
		...
		DWORD_FREEIMAGE visited = (DWORD_FREEIMAGE)( (((size_t)ifdp & 0xFFFF) << 16) | (size_t)de );
		...
		for(; de < nde; de++) {
            ...
            // check that tag length (size of the tag value in bytes) will fit in a DWORD_FREEIMAGE
            int tag_data_width = FreeImage_TagDataWidth(FreeImage_GetTagType(tag));
            if (tag_data_width != 0 && FreeImage_GetTagCount(tag) > ~(DWORD_FREEIMAGE)0 / tag_data_width) {
                FreeImage_DeleteTag(tag);
                // jump to next entry
                continue;
            }
			...
			if(FreeImage_GetTagLength(tag) <= 4) {
				...
				DWORD_FREEIMAGE offset_value = ReadUint32(msb_order, pde + 8);
			...
			BOOL_FREEIMAGE isIFDOffset = FALSE;
			switch(FreeImage_GetTagID(tag)) {
				case TAG_EXIF_OFFSET:
				case TAG_GPS_OFFSET:
				case TAG_INTEROP_OFFSET:
			...
			if(isIFDOffset)	{
				DWORD_FREEIMAGE sub_offset = 0;
				TagLib::MDMODEL next_mdmodel = md_model;
				const BYTE_FREEIMAGE *next_ifd = ifdp;
				...
				if (FreeImage_GetTagID(tag) == TAG_MAKER_NOTE) {
					...
					next_ifd = (BYTE_FREEIMAGE*)pval + sub_offset;
				} else {
					processIFDOffset(tag, pval, msb_order, &sub_offset, &next_mdmodel);
					next_ifd = (BYTE_FREEIMAGE*)tiffp + sub_offset;
				}

				if((sub_offset < (DWORD_FREEIMAGE) length) && (next_mdmodel != TagLib::UNKNOWN)) {
	...
	DWORD_FREEIMAGE next_offset = ReadUint32(msb_order, DIR_ENTRY_ADDR(ifd0th, entriesCount0th));
	if((next_offset == 0) || (next_offset >= length)) {
		return TRUE; //< no thumbnail
	}
	
	const BYTE_FREEIMAGE* const ifd1st = (BYTE_FREEIMAGE*)tiffp + next_offset;
	...
	for(int e = 0; e < entriesCount1st; e++) {
		...
		// point to the directory entry
		const BYTE_FREEIMAGE* base = DIR_ENTRY_ADDR(ifd1st, e);
		...
		// get number of components
		DWORD_FREEIMAGE count = ReadUint32(msb_order, base + sizeof(WORD) + sizeof(WORD));
		// get the tag value
		DWORD_FREEIMAGE offset = ReadUint32(msb_order, base + sizeof(WORD) + sizeof(WORD) + sizeof(DWORD_FREEIMAGE));
	...
	const BYTE_FREEIMAGE *thLocation = tiffp + thOffset;
	
	FIMEMORY* hmem = FreeImage_OpenMemory(const_cast<BYTE_FREEIMAGE*>(thLocation), thSize);
...
*/
BOOL_FREEIMAGE  
jpeg_read_exif_profile(FIBITMAP *dib, const BYTE_FREEIMAGE *dataptr, unsigned int datalen) {
    // marker identifying string for Exif = "Exif\0\0"
    BYTE_FREEIMAGE exif_signature[6] = { 0x45, 0x78, 0x69, 0x66, 0x00, 0x00 };
	BYTE_FREEIMAGE lsb_first[4] = { 0x49, 0x49, 0x2A, 0x00 };		// Intel order
	BYTE_FREEIMAGE msb_first[4] = { 0x4D, 0x4D, 0x00, 0x2A };		// Motorola order

	unsigned int length = datalen;
	BYTE_FREEIMAGE *profile = (BYTE_FREEIMAGE*)dataptr;
	...
	if(memcmp(exif_signature, profile, sizeof(exif_signature)) == 0) {
		...
		BOOL_FREEIMAGE bMotorolaOrder = TRUE;
