...
void FIRational::initialize(LONG_FREEIMAGE n, LONG_FREEIMAGE d) {
	if(d) {
		_numerator = n;
		_denominator = d;
		// normalize rational
...
FIRational::FIRational(LONG_FREEIMAGE n, LONG_FREEIMAGE d) {
	initialize(n, d);
}
...
FIRational::FIRational(const FITAG *tag) {
	switch(FreeImage_GetTagType((FITAG*)tag)) {
		...
		{
			DWORD_FREEIMAGE *pvalue = (DWORD_FREEIMAGE*)FreeImage_GetTagValue((FITAG*)tag);
			initialize((LONG_FREEIMAGE)pvalue[0], (LONG_FREEIMAGE)pvalue[1]);
			break;
		}

		case FIDT_SRATIONAL:	// 64-bit signed fraction 
		{
			LONG_FREEIMAGE *pvalue = (LONG_FREEIMAGE*)FreeImage_GetTagValue((FITAG*)tag);
			initialize((LONG_FREEIMAGE)pvalue[0], (LONG_FREEIMAGE)pvalue[1]);
			break;
		}
	}
...
FIRational::FIRational(float value) {
	if (value == (float)((LONG_FREEIMAGE)value)) {
	   _numerator = (LONG_FREEIMAGE)value;
	   _denominator = 1L;
	} else {
		int k, count;
		LONG_FREEIMAGE n[4];
		...
		for(int i = count - 1; i >= 0; i--) {
			if(n[i] == 0) break;
			LONG_FREEIMAGE _num = (n[i] * _numerator + _denominator);
			LONG_FREEIMAGE _den = _numerator;
			_numerator = _num;
			_denominator = _den;
		}
		_numerator *= sign;
...
/// Get the numerator
LONG_FREEIMAGE FIRational::getNumerator() {
	return _numerator;
}

/// Get the denominator
LONG_FREEIMAGE FIRational::getDenominator() {
	return _denominator;
}

/// Calculate GCD
LONG_FREEIMAGE FIRational::gcd(LONG_FREEIMAGE a, LONG_FREEIMAGE b) {
	LONG_FREEIMAGE temp;
...
void FIRational::normalize() {
	if (_numerator != 1 && _denominator != 1) {	// Is there something to do?
		 // Calculate GCD
		LONG_FREEIMAGE common = gcd(_numerator, _denominator);
...
BOOL_FREEIMAGE FIRational::isInteger() {
