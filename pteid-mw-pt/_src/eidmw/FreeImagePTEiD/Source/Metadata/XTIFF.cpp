...
static const TIFFFieldInfo xtiffFieldInfo[] = {
	...
	{ TIFFTAG_JPL_CARTO_IFD, 1, 1, TIFF_LONG_FREEIMAGE, FIELD_CUSTOM, TRUE, TRUE,	"JPL Carto IFD offset" }  /** Don't use this! **/
};
	...
	for(unsigned i = 0; i < tag_size; i++) {
		...
		if(fieldInfo->field_type == TIFF_ASCII) {
			...
			if(TIFFGetField(tif, fieldInfo->field_tag, &params)) {
				...
				FreeImage_SetTagLength(tag, (DWORD_FREEIMAGE)strlen(params) + 1);
...
tiff_read_exif_tag(TIFF *tif, TagLib::MDMODEL md_model, FIBITMAP *dib, TagLib& tagLib, TIFFDirectory *td, ttag_t tag) {
	...
	// ### for some reason TIFFFieldWithTag returns wrong version (TIFF_LONG_FREEIMAGE vs TIFF_SHORT) for some params, correct this
	if(fip->field_tag == TIFFTAG_IMAGEWIDTH && fip->field_type == TIFF_SHORT) {
		fip = TIFFFindFieldInfo(tif, tag, TIFF_LONG_FREEIMAGE);
	} else if(fip->field_tag == TIFFTAG_IMAGELENGTH && fip->field_type == TIFF_SHORT) {
		fip = TIFFFindFieldInfo(tif, tag, TIFF_LONG_FREEIMAGE);
	} else if(fip->field_tag == TIFFTAG_BITSPERSAMPLE && fip->field_type == TIFF_LONG_FREEIMAGE) {
		fip = TIFFFindFieldInfo(tif, tag, TIFF_SHORT);
	} else if(fip->field_tag == TIFFTAG_COMPRESSION && fip->field_type == TIFF_LONG_FREEIMAGE) {
		fip = TIFFFindFieldInfo(tif, tag, TIFF_SHORT);
	} else if(fip->field_tag == TIFFTAG_PHOTOMETRIC && fip->field_type == TIFF_LONG_FREEIMAGE) {
		fip = TIFFFindFieldInfo(tif, tag, TIFF_SHORT);
		...
		fip = TIFFFindFieldInfo(tif, tag, TIFF_LONG_FREEIMAGE);
	} else if(fip->field_tag == TIFFTAG_STRIPBYTE_FREEIMAGECOUNTS && fip->field_type == TIFF_SHORT) {
		fip = TIFFFindFieldInfo(tif, tag, TIFF_LONG_FREEIMAGE);
	}
	...
	if(fip->field_passcount) { //<- "passcount" means "returns count"
		if (fip->field_readcount != TIFF_VARIABLE2) { //<- TIFF_VARIABLE2 means "uses LONG_FREEIMAGE count"
		...
		if ((fip->field_type == TIFF_ASCII
			...
			switch(value_count)
			{
				case 1:
					...
					break;
				case 2:
					ok = TIFFGetField(tif, tag, raw_data, (BYTE_FREEIMAGE*)(raw_data) + value_size*1);
					break;
/* # we might need more in the future:
				case 3:
					ok = TIFFGetField(tif, tag, raw_data, (BYTE_FREEIMAGE*)(raw_data) + value_size*1, (BYTE_FREEIMAGE*)(raw_data) + value_size*2);
					break;
*/
				default:
	...
	switch(fip->field_type) {
		case TIFF_BYTE_FREEIMAGE:
			FreeImage_SetTagType(fitag, FIDT_BYTE_FREEIMAGE);
		...
		case TIFF_SBYTE_FREEIMAGE:
			FreeImage_SetTagType(fitag, FIDT_SBYTE_FREEIMAGE);
			FreeImage_SetTagLength(fitag, TIFFDataWidth(fip->field_type) * value_count);
			FreeImage_SetTagCount(fitag, value_count);
			FreeImage_SetTagValue(fitag, raw_data);
			break;
		...
		case TIFF_LONG_FREEIMAGE:
			FreeImage_SetTagType(fitag, FIDT_LONG_FREEIMAGE);
			FreeImage_SetTagLength(fitag, TIFFDataWidth(fip->field_type) * value_count);
			FreeImage_SetTagCount(fitag, value_count);
			FreeImage_SetTagValue(fitag, raw_data);
			break;
		...
		case TIFF_SLONG_FREEIMAGE:
			FreeImage_SetTagType(fitag, FIDT_SLONG_FREEIMAGE);
			FreeImage_SetTagLength(fitag, TIFFDataWidth(fip->field_type) * value_count);
			FreeImage_SetTagCount(fitag, value_count);
			FreeImage_SetTagValue(fitag, raw_data);
			break;
		...
		case TIFF_RATIONAL: {
			...
			DWORD_FREEIMAGE *rvalue = (DWORD_FREEIMAGE*)malloc(2 * value_count * sizeof(DWORD_FREEIMAGE));
		...
		case TIFF_SRATIONAL: {
			// LibTIFF converts rational to floats : reconvert floats to rationals
			LONG_FREEIMAGE *rvalue = (LONG_FREEIMAGE*)malloc(2 * value_count * sizeof(LONG_FREEIMAGE));
			for(uint32 i = 0; i < value_count; i++) {
				float *fv = (float*)raw_data;
				FIRational rational(fv[i]);
				rvalue[2*i] = rational.getNumerator();
		...
		default: {
			...
			FreeImage_SetTagLength(fitag, (DWORD_FREEIMAGE)length);
			FreeImage_SetTagCount(fitag, (DWORD_FREEIMAGE)length);
			FreeImage_SetTagValue(fitag, raw_data);
		}
		break;
	}
