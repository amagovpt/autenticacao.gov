...
*/
DLL_API FIBITMAP * DLL_CALLCONV FreeImage_AllocateHeaderT(BOOL_FREEIMAGE header_only, FREE_IMAGE_TYPE type, int width, int height, int bpp FI_DEFAULT(8), unsigned red_mask FI_DEFAULT(0), unsigned green_mask FI_DEFAULT(0), unsigned blue_mask FI_DEFAULT(0));
...
*/
DLL_API FIBITMAP * DLL_CALLCONV FreeImage_AllocateHeader(BOOL_FREEIMAGE header_only, int width, int height, int bpp, unsigned red_mask FI_DEFAULT(0), unsigned green_mask FI_DEFAULT(0), unsigned blue_mask FI_DEFAULT(0));
...
// structs in FreeImagePTEiD.h which are for in-memory bitmap handling
...
*/
inline void 
AssignPixel(BYTE_FREEIMAGE* dst, const BYTE_FREEIMAGE* src, unsigned bytesperpixel) {
	switch (bytesperpixel) {
		...
		case 4: // FIT_BITMAP (32-bit) / FIT_UINT32 / FIT_INT32 / FIT_FLOAT
			*(reinterpret_cast<DWORD_FREEIMAGE*>(dst)) = *(reinterpret_cast<const DWORD_FREEIMAGE*> (src));
			break;
		...
		case 8: // FIT_RGBA16 (4 x 16-bit)
			*(reinterpret_cast<DWORD_FREEIMAGE*>(dst)) = *(reinterpret_cast<const DWORD_FREEIMAGE*> (src));
			*(reinterpret_cast<DWORD_FREEIMAGE*>(dst + 4)) = *(reinterpret_cast<const DWORD_FREEIMAGE*> (src + 4));	
			break;
...
*/
BOOL_FREEIMAGE SwapRedBlue32(FIBITMAP* dib);
...
@see See definition in Conversion.cpp
*/
BOOL_FREEIMAGE ConvertCMYKtoRGBA(FIBITMAP* dib);
...
@see See definition in Conversion.cpp
*/
BOOL_FREEIMAGE ConvertLABtoRGB(FIBITMAP* dib);
...
inline DWORD_FREEIMAGE 
__SwapUInt32(DWORD_FREEIMAGE arg) { 
#if defined(_MSC_VER) && _MSC_VER >= 1310 
...
#define GREY(r, g, b) (BYTE_FREEIMAGE)LUMA_REC709(r, g, b)
/*
#define GREY(r, g, b) (BYTE_FREEIMAGE)(((WORD)r * 77 + (WORD)g * 150 + (WORD)b * 29) >> 8)	// .299R + .587G + .114B
*/
/*
#define GREY(r, g, b) (BYTE_FREEIMAGE)(((WORD)r * 169 + (WORD)g * 256 + (WORD)b * 87) >> 9)	// .33R + 0.5G + .17B
*/
