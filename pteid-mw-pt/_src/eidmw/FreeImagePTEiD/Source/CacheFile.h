...
public :
	CacheFile(const std::string filename, BOOL_FREEIMAGE keep_in_memory);
	~CacheFile();

	BOOL_FREEIMAGE open();
	void close();
	BOOL_FREEIMAGE readFile(BYTE_FREEIMAGE *data, int nr, int size);
	int writeFile(BYTE_FREEIMAGE *data, int size);
...
private :
	...
	BOOL_FREEIMAGE unlockBlock(int nr);
	BOOL_FREEIMAGE deleteBlock(int nr);
...
private :
	...
	BOOL_FREEIMAGE m_keep_in_memory;
};
