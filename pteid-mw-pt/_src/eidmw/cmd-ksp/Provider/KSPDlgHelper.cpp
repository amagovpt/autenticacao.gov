 ...
 */
SECURITY_STATUS
CmdKspOpenDialogSign(const __in wchar_t *userId, __in unsigned long userIdLen, __out wchar_t *pin,
					 __in unsigned long pinLen, const __in wchar_t *userName, __in unsigned long userNameLen,
	...
	DlgRet ret = DlgAskInputCMD(DlgCmdOperation::DLG_CMD_SIGNATURE, false, pin, pinLen, (wchar_t *)userId, 0, userName,
								userNameLen);
	if (ret == DLG_OK) {
 ...
 */
SECURITY_STATUS
CmdKspOpenDialogOtp(__out wchar_t *otp, __in unsigned long otpLen, __in LPWSTR pszDocname, __in HWND parentWindow) {
	if (parentWindow == NULL) {
		parentWindow = GetActiveWindow();
	}
	MWLOG_DEBUG(logBuf, "appWindow = [0x%08x]", parentWindow);
	...
	DlgRet ret = DlgAskInputCMD(DlgCmdOperation::DLG_CMD_SIGNATURE, true, otp, otpLen, (wchar_t *)pszDocname, 0, NULL,
								0, &fSendSmsCallback);
	if (ret == DLG_OK) {
		return ERROR_SUCCESS;
	} else if (ret == DLG_CANCEL) {
...
SECURITY_STATUS
CmdKspOpenDialogProgress(__in bool isValidateOtp, __in HWND parentWindow, __in unsigned long *pulHandle) {
	if (parentWindow == NULL) {
		parentWindow = GetActiveWindow();
	}
	MWLOG_DEBUG(logBuf, "appWindow = [0x%08x]", parentWindow);
	SetApplicationWindow(parentWindow);
	DlgRet ret =
		DlgCMDMessage(DlgCmdOperation::DLG_CMD_SIGNATURE, DlgCmdMsgType::DLG_CMD_PROGRESS,
...
void CmdKspOpenDialogError(const __in wchar_t *message, __in DlgCmdMsgType msgType, __in HWND parentWindow,
...
CmdSignThread::CmdSignThread(CMDProxyInfo *cmd_proxyinfo, CMDSignature *cmdSignature, std::string mobileNumber,
							 std::string pin, CByteArray hash, char *docname, PCCERT_CONTEXT pCert,
...
void CmdSignThread::Run() {
	...
	if (m_isValidateOtp) {
		...
		DWORD dwFlags = 0;
		pIssuerCert = CertGetIssuerCertificateFromStore(hCAStore, m_pCert, pIssuerCert, &dwFlags);
...
int CmdSignThread::GetSignResult() { return m_signResult; }