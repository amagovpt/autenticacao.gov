...
BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam) {
	...
	if (GetClassName(hwnd, lpClassName, 256) == 0) {
		MWLOG_ERR(logBuf, "Error getting window class name.");
		return TRUE;
	}

	if (lpdwProcessId == GetCurrentProcessId() && strcmp(lpClassName, (const char *)lParam) == 0) {
		g_HWND = hwnd;
		return FALSE;
	}
	return TRUE;
...
CMDKSP_PROVIDER *CmdKspValidateProvHandle(__in NCRYPT_PROV_HANDLE hProvider) {
	...
	if (pProvider->cbLength < sizeof(CMDKSP_PROVIDER) || pProvider->dwMagic != CMDKSP_PROVIDER_MAGIC) {
		return NULL;
	}
...
CMDKSP_KEY *CmdKspValidateKeyHandle(__in NCRYPT_KEY_HANDLE hKey) {
	...
	if (pKey->cbLength < sizeof(CMDKSP_KEY) || pKey->dwMagic != CMDKSP_KEY_MAGIC) {
		return NULL;
	}
...
CreateNewKeyObject(__in_opt LPCWSTR pszKeyName, __deref_out CMDKSP_KEY **ppKey) {
	...
	if (g_hRSAProvider == NULL) {
		...
		ntStatus = BCryptOpenAlgorithmProvider(&g_hRSAProvider, BCRYPT_RSA_ALGORITHM, NULL, 0);
	...
	// process can change it with KSPSetKeyProperty)
	{
		...
		GetModuleBaseName(GetCurrentProcess(), NULL, lpProcessName, FILENAME_MAX);
...

	{
		...
		DWORD cbBufferSize;
		if (!CryptStringToBinaryW(hashHex.c_str(), (DWORD)hashHex.size(), CRYPT_STRING_HEXRAW, NULL, &cbBufferSize,
								  NULL, NULL)) {
		...
		if (!CryptStringToBinaryW(hashHex.c_str(), (DWORD)hashHex.size(), CRYPT_STRING_HEXRAW, pbHashBuffer,
								  &cbBufferSize, NULL, NULL)) {
		...
		PCCERT_CONTEXT pCert = CertFindCertificateInStore(hMyStore, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0,
														  CERT_FIND_HASH, (const void *)&hash, NULL);
		...
		if (!CertGetCertificateContextProperty(pKey->pCert, CERT_FIRST_USER_PROP_ID, NULL, &cbMobileNumber)) {
			MWLOG_ERR(logBuf, "Could not get size of certificate's friendly name.");
			Status = NTE_INTERNAL_ERROR;
			goto cleanup;
		}
		LPWSTR pszMobileNumber = new WCHAR[cbMobileNumber];
		if (!CertGetCertificateContextProperty(pKey->pCert, CERT_FIRST_USER_PROP_ID, pszMobileNumber,
											   &cbMobileNumber)) {
	...
	pKey->dwKeyBitLength =
		CertGetPublicKeyLength(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, &pKey->pCert->pCertInfo->SubjectPublicKeyInfo);
...
#define SHORT_HASH_LEN 8
	...
	if (!CryptBinaryToStringW(pbHash, cbHash, dwFlags, NULL, &cchHash)) {
		MWLOG_ERR(logBuf, "Error getting string size CryptBinaryToString");
		return L"";
	}

	LPWSTR chHash = new WCHAR[cchHash];
	if (!CryptBinaryToStringW(pbHash, cbHash, dwFlags, chHash, &cchHash)) {
...
void getDocName(__in HWND hParentWindow, __in LPSTR lpProcessName, __out LPSTR pszBuffer, __in DWORD cbBuffer,
	...
	if (strcmp(lpProcessName, "EXCEL.EXE") == 0 || strcmp(lpProcessName, "WINWORD.EXE") == 0 ||
		strcmp(lpProcessName, "AcroRd32.exe") == 0 || strcmp(lpProcessName, "Acrobat.exe") == 0 ||
...
/* Return TRUE if cert status is not revoked/suspended. It returns TRUE if the validation fails to avoid stopping the
 * signature. */
BOOL validateCert(__in PCCERT_CONTEXT pCert, bool showDialog) {
	...
	DWORD dwFlags = 0;
	pIssuerCert = CertGetIssuerCertificateFromStore(hCAStore, pCert, pIssuerCert, &dwFlags);
	...
	return (status != FWK_CertifStatus::FWK_CERTIF_STATUS_REVOKED &&
			status != FWK_CertifStatus::FWK_CERTIF_STATUS_SUSPENDED);
}