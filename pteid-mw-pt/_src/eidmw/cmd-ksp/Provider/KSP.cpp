...
//
///////////////////////////////////////////////////////////////////////////////
NCRYPT_KEY_STORAGE_FUNCTION_TABLE KSPFunctionTable = {CMDKSP_INTERFACE_VERSION,
													  KSPOpenProvider,
													  KSPOpenKey,
...
KSPGetProviderProperty(__in NCRYPT_PROV_HANDLE hProvider, __in LPCWSTR pszProperty,
	...
	if (wcscmp(pszProperty, NCRYPT_IMPL_TYPE_PROPERTY) == 0) {
		...
		if (pProvider->pszContext) {
			cbResult = (DWORD)(wcslen(pProvider->pszContext) + 1) * sizeof(WCHAR);
		}
...
KSPGetKeyProperty(__in NCRYPT_PROV_HANDLE hProvider, __in NCRYPT_KEY_HANDLE hKey, __in LPCWSTR pszProperty,
	...
	if ((pszProperty == NULL) || (wcslen(pszProperty) > NCRYPT_MAX_PROPERTY_NAME) || (pcbResult == NULL)) {
		Status = NTE_INVALID_PARAMETER;
		goto cleanup;
	}
	...
	if (wcscmp(pszProperty, NCRYPT_SECURITY_DESCR_PROPERTY) == 0) {
		...
		if ((dwFlags == 0) ||
			((dwFlags & ~(OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION |
						  LABEL_SECURITY_INFORMATION | SACL_SECURITY_INFORMATION)) != 0)) {
			Status = NTE_BAD_FLAGS;
			goto cleanup;
		}
	} else {
	...
	*pcbResult = cbResult;
	...
	case CMDKSP_LENGTHS_PROPERTY: {
		...
		Status = BCryptGetProperty(pKey->hProvider, BCRYPT_KEY_LENGTHS, pbOutput, cbOutput, &cbTmp, 0);
...
KSPSetProviderProperty(__in NCRYPT_PROV_HANDLE hProvider, __in LPCWSTR pszProperty, __in_bcount(cbInput) PBYTE pbInput,
	...
	if ((pszProperty == NULL) || (wcslen(pszProperty) > NCRYPT_MAX_PROPERTY_NAME) || (pbInput == NULL)) {
		Status = NTE_INVALID_PARAMETER;
		goto cleanup;
	}
...
SECURITY_STATUS
WINAPI
KSPSetKeyProperty(__in NCRYPT_PROV_HANDLE hProvider, __inout NCRYPT_KEY_HANDLE hKey, __in LPCWSTR pszProperty,
...
SECURITY_STATUS
WINAPI
KSPEnumAlgorithms(__in NCRYPT_PROV_HANDLE hProvider, __in DWORD dwAlgOperations, __out DWORD *pdwAlgCount,
				  __deref_out_ecount(*pdwAlgCount) NCryptAlgorithmName **ppAlgList, __in DWORD dwFlags) {
	SECURITY_STATUS Status = NTE_INTERNAL_ERROR;
	CMDKSP_PROVIDER *pProvider = NULL;
	...
	if (dwAlgOperations == 0 || ((dwAlgOperations & NCRYPT_ASYMMETRIC_ENCRYPTION_OPERATION) != 0) ||
	...
	CopyMemory(pbCurrent, BCRYPT_RSA_ALGORITHM, sizeof(BCRYPT_RSA_ALGORITHM));
...
KSPExportKey(__in NCRYPT_PROV_HANDLE hProvider, __in NCRYPT_KEY_HANDLE hKey, __in_opt NCRYPT_KEY_HANDLE hExportKey,
			 ...
			 __out_bcount_part_opt(cbOutput, *pcbResult) PBYTE pbOutput, __in DWORD cbOutput, __out DWORD *pcbResult,
			 __in DWORD dwFlags) {
	CMDKSP_PROVIDER *pProvider = NULL;
	CMDKSP_KEY *pKey = NULL;
...
void handleErrorAndShowDlg(bool proxyUsed, int error, SECURITY_STATUS *hStatus) {
	...
	case ERR_INV_CERTIFICATE:
		...
		*hStatus = NTE_KEYSET_ENTRY_BAD;
		break;
	case EIDMW_ERR_CMD_INVALID_STORED_CERTIFICATE:
	...
	if (!msg.empty()) {
		DlgRet ret =
			DlgCMDMessage(DlgCmdOperation::DLG_CMD_SIGNATURE, DlgCmdMsgType::DLG_CMD_WARNING_MSG, msg.c_str(), NULL);
...
KSPSignHash(__in NCRYPT_PROV_HANDLE hProvider, __in NCRYPT_KEY_HANDLE hKey, __in_opt VOID *pPaddingInfo,
			...
			__out_bcount_part_opt(cbSignaturee, *pcbResult) PBYTE pbSignature, __in DWORD cbSignature,
			__out DWORD *pcbResult, __in DWORD dwFlags) {
	SECURITY_STATUS Status = NTE_INTERNAL_ERROR;
	int Status2 = 0;
	NTSTATUS ntStatus = STATUS_INTERNAL_ERROR;
	...
	// (see compiler error C2362)
	{
		...
		DWORD dwSubjectLen = CertGetNameStringW(pKey->pCert, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, NULL, 0);
		csSubject = new CHAR[dwSubjectLen];
		CertGetNameStringA(pKey->pCert, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, csSubject, dwSubjectLen);
...
KSPVerifySignature(__in NCRYPT_PROV_HANDLE hProvider, __in NCRYPT_KEY_HANDLE hKey, __in_opt VOID *pPaddingInfo,
				   __in_bcount(cbHashValue) PBYTE pbHashValue, __in DWORD cbHashValue,
				   __in_bcount(cbSignaturee) PBYTE pbSignature, __in DWORD cbSignaturee, __in DWORD dwFlags) {
	NTSTATUS ntStatus = STATUS_INTERNAL_ERROR;
	SECURITY_STATUS Status = NTE_INTERNAL_ERROR;
	CMDKSP_KEY *pKey;
	...
	ntStatus =
		BCryptVerifySignature(hPublicKey, pPaddingInfo, pbHashValue, cbHashValue, pbSignature, cbSignaturee, dwFlags);
