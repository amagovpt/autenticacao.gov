...
DEFINE_LANG_STR(PossibleProxyError) = {L"A connection error has occurred. Check proxy settings.",
									   L"Ocorreu um erro na ligação. Verifique a configuração do proxy."};
...
DEFINE_LANG_STR(OtpValidationFailed) = {
	...
	L"Não foi possível validar o código de confirmação. O pedido não foi encontrado ou o tempo de validação expirou."};
...
DEFINE_LANG_STR(VerifyIfDigitalSignatureIsActive) = {L"Verify that the digital signature subscription of the Chave "
													 L"Móvel Digital is active in https://www.autenticacao.gov.pt.",
													 L"Verifique que a subscrição da assinatura digital da Chave Móvel "
													 L"Digital está ativada em https://www.autenticacao.gov.pt.",
													 L"", L""};
DEFINE_LANG_STR(RegisterCertificateAgain) = {
	L"Please try to register your CMD certificate in the application again.",
	L"Por favor, tente registar novamente o seu certificado da CMD na aplicação.", L"", L""};
DEFINE_LANG_STR(NoCMDCredentials) = {L"The Citizen Card Middleware installed does not support CMD. An official "
									 ...
									 L"O Middleware do Cartão de Cidadão instalado não possui suporte para CMD. "
