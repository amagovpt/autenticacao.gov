...
static int EnumProviders(void);

static int RegisterProvider(void);

static int UnRegisterProvider(void);

static int RemoveCmdCertificates(void);
...
//
PWSTR AlgorithmNames[1] = {NCRYPT_KEY_STORAGE_ALGORITHM};
...
// by the provider...
//
PCRYPT_INTERFACE_REG AlgorithmClasses[1] = {&AlgorithmClass};
...
int RegisterProvider(void) {
	...
	//
	ntStatus = BCryptAddContextFunction(CRYPT_LOCAL,				  // Scope: local machine only
	...
	//
	ntStatus = BCryptAddContextFunctionProvider(CRYPT_LOCAL,				  // Scope: local machine only
												NULL,						  // Application context: default
												NCRYPT_KEY_STORAGE_INTERFACE, // Algorithm class
												NCRYPT_KEY_STORAGE_ALGORITHM, // Algorithm name
...
int UnRegisterProvider() {
	...
	// this algorithm.
	//
	ntStatus = BCryptRemoveContextFunctionProvider(CRYPT_LOCAL,					 // Scope: local machine only
												   NULL,						 // Application context: default
												   NCRYPT_KEY_STORAGE_INTERFACE, // Algorithm class
												   NCRYPT_KEY_STORAGE_ALGORITHM, // Algorithm name
...
///////////////////////////////////////////////////////////////////////////////
int RemoveCmdCertificates(void) {
	...
	while (pCert = CertFindCertificateInStore(hMyStore, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0,
											  CERT_FIND_ISSUER_STR, L"EC de Chave Móvel Digital", pCert)) {
