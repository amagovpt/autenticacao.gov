...
CByteArray ACR83Pinpad::PinCmd(tPinOperation operation, const tPin &pin, unsigned char ucPinType,
	...
	if (operation == PIN_OP_VERIFY) {
		...
		MWLOG(LEV_DEBUG, MOD_CAL, L"[Verify Pin] ACR83 Pinpad sending %d bytes in VERIFY structure", length);
		...
		MWLOG(LEV_DEBUG, MOD_CAL, L"[Modify Pin] ACR83 Pinpad sending %d bytes in MODIFY PIN structure", length);
		...
		MWLOG(LEV_DEBUG, MOD_CAL, L"[Reset Pin] ACR83 Pinpad sending %d bytes in MODIFY PIN structure", length);
