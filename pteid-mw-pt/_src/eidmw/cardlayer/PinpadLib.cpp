...
#if defined WIN32 && defined PTEID_OLD_PINPAD
	m_oPinpadLibOldPteid.UnLoad();
#endif
}
...
{
#if defined WIN32 && defined PTEID_OLD_PINPAD
	if (m_oPinpadLibOldPteid.UseOldLib())
		return m_oPinpadLibOldPteid.PinCmd(hCard, ulControl, oCmd, ucPintype, ucOperation);
#endif
...
{
	// E.g. C:\WINDOWS\System32\pteidpp\pteidpp2*.*
#ifdef WIN32
	char csBuf[20];
...
#ifdef PTEID_OLD_PINPAD
	if (!bPinpadLibFound)
	{
		bPinpadLibFound = m_oPinpadLibOldPteid.Load(hContext, hCard, strReader,
