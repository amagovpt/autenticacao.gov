...
int main(int argc, char **argv) {
	...
	if (!g_bHelp && (strlen(g_csComponent) == 0 || strlen(g_csGuid) == 0 || strlen(g_csFolder) == 0 ||
					 strlen(g_csKeyPath) == 0 || strlen(g_csOutput) == 0))
...
/////////////////////////////////////////////////////////////////////////////
#ifdef WIN32
void scanDir(const char *Dir, FILE *f, const char *idBase, const char *DirRef, const char *ShortcutFile,
			 const char *ShortcutFolder, const char *ShortcutName, const char *ShortcutDescr,
			 void (*callback)(FILE *f, const char *Dir, const char *File, const char *Id, const char *DirRef,
							  const char *ShortcutFile, const char *ShortcutFolder, const char *ShortcutName,
	...
	while (a != ERROR_NO_MORE_FILES) {
		if (strcmp(FindFileData.cFileName, ".") != 0 && strcmp(FindFileData.cFileName, "..") != 0) {
			...
			if (!(FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
				...
				callback(f, Dir, FindFileData.cFileName, id, DirRef, ShortcutFile, ShortcutFolder, ShortcutName,
						 ShortcutDescr);
				count++;
			}
		}
			...
			if (strcmp(pFile->d_name, ".") != 0 && strcmp(pFile->d_name, "..") != 0) {
				...
				if (!stat(path.c_str(), &buffer)) {
					if (!S_ISDIR(buffer.st_mode)) {
						...
						callback(f, Dir, file.c_str(), id, DirRef, ShortcutFile, ShortcutFolder, ShortcutName,
								 ShortcutDescr);
						count++;
					}
				} else {
...
/////////////////////////////////////////////////////////////////////////////
void PrintLine(FILE *f, const char *Dir, const char *File, const char *Id, const char *DirRef, const char *ShortcutFile,
			   const char *ShortcutFolder, const char *ShortcutName, const char *ShortcutDescr) {
	fprintf_s(f, "\t\t\t\t<File Id=\"%s\" Name=\"%s\" KeyPath=\"%s\" Source=\"%s%s\" />\n", Id, File,
			  (!g_bUserProfile && strcmp(g_csKeyPath, File) == 0 ? "yes" : "no"), Dir, File);
	if (strcmp(File, ShortcutFile) == 0) {
		fprintf_s(f,
				  "\t\t\t\t<Shortcut Id=\"%sShortcut\" Target=\"[%s]%s\" Directory=\"%s\" Name=\"%s\" "
				  "WorkingDirectory=\"INSTALLLOCATION\" Description=\"%s\" />\n",
...
static void BrowseFolder() {
	...
	fprintf_s(f, "<Wix xmlns=\"http://schemas.microsoft.com/wix/2006/wi\" "
	...
	if (g_bUserProfile) {
		fprintf_s(f,
				  "\t\t\t\t<RegistryValue Root=\"HKCU\" Key=\"%s\" Name=\"%s\" Type=\"integer\" Value=\"1\" "
				  "KeyPath=\"yes\" />\n",
				  g_csKeyPath, g_csComponent);
		fprintf_s(f, "\t\t\t\t<RemoveFolder Id=\"%s\" On=\"uninstall\"/>\n", g_csDirRef);
	}

	scanDir(g_csFolder, f, g_csComponent, g_csDirRef, g_csShortcutFile, g_csShortcutFolder, g_csShortcutName,
			g_csShortcutDescr, &PrintLine);
