...
PortugalEidPKCS11.prototype = {
  ...
  getPrefs: function() {
    ...
    return prefs.getBranch("extensions.portugaleid.");
  },
  defaultPrefs: {
    moduleLocation: function() {
	  ...
	  if (osString == "Darwin")
	    return "/Library/Frameworks/BeId.framework/Versions/4.0/lib/libpteidpkcs11.dylib;/usr/local/lib/pteid-pkcs11.bundle;/usr/local/lib/pteid-pkcs11.bundle/Contents/MacOS/libpteidpkcs11.dylib";
	  if (osString == "Linux")
	    return "libpteidpkcs11.so;/usr/local/lib/libpteidpkcs11.so";
	}	
  },
  ...
  errorLog: function(aMessage) {
	...
	consoleService.logStringMessage("Portugal eID: " + aMessage);
  },
  ...
  slotFound: function(modulename){
	var found;
	var portugalEidPKCS11Module = this.findModuleByName(modulename)
	try {
	   slot = portugalEidPKCS11Module.listSlots().currentItem();
	   found = true;
	} catch (e)
	{
		found = false;
  ...
  registerModule: function() {
    ...
    if (this.getModuleLocation() == "") {
		this.errorLog("portugaleid.modulelocation is not set");
  ...
  setDefaultPrefs: function() {
    if (this.defaultPrefs.moduleLocation()) {
		var prefs = Components.classes["@mozilla.org/preferences-service;1"]
                    .getService(Components.interfaces.nsIPrefService);
		var defaultPortugaleidPrefs = prefs.getDefaultBranch("extensions.portugaleid.");
		defaultPortugaleidPrefs.setCharPref("modulelocation",this.defaultPrefs.moduleLocation());
	}
  },
  init: function() {
    try {
  ...
  /* Looks in the list of registered PKCS11 modules if a Portugal eID module is installed
...
try {
  PortugalEidPKCS11.prototype.addModule = Components.classes["@mozilla.org/security/pkcs11;1"].getService(Components.interfaces.nsIPKCS11).addModule;
}  
catch (e) {
    // pre-firefox 3.5 interface
  PortugalEidPKCS11.prototype.addModule = Components.classes["@mozilla.org/security/pkcs11;1"].getService(Components.interfaces.nsIDOMPkcs11).addmodule;
};
/**
...
if (XPCOMUtils.generateNSGetFactory)
    var NSGetFactory = XPCOMUtils.generateNSGetFactory([PortugalEidPKCS11]);
else
    var NSGetModule = XPCOMUtils.generateNSGetModule([PortugalEidPKCS11]);
