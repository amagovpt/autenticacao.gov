:: Compiled / external libs
::==============================
@set PTEID_POPPLER_QT5=C:\git\tools\build-scripts\windows\external-libs\poppler\poppler-buildroot\
@set PTEID_XMLSECURITY=C:\git\tools\build-scripts\windows\external-libs\xml-security-c-1.7.3\


:: Compiled / external libs x86
::==============================
@set PTEID_DIR_QML=C:\git\tools\build-scripts\windows\external-libs\qt5.9.1\Win32\VC9\Release\qml-app
@set PTEID_DIR_QT_4=C:\Qt-5.9-win32\
@set PTEID_DIR_OPENSSL_098=C:\git\tools\build-scripts\windows\external-libs\openssl-1.0.2p-win32
@set PTEID_DIR_XERCES_31=C:\git\tools\build-scripts\windows\external-libs\xerces-c-3.1.4-win32
@set PTEID_DIR_CURL=C:\git\tools\build-scripts\windows\external-libs\libcurl-vc9-x86-release-dll-ssl-dll
@set PTEID_DIR_ZLIB=C:\git\tools\build-scripts\windows\external-libs\zlib\Win32
@set PTEID_DIR_LIBZIP=C:\git\tools\build-scripts\windows\external-libs\libzip-1.5.1-build-windows-Release\libzip-1.5.1-win32
@set PTEID_DIR_LIBPNG=C:\git\tools\build-scripts\windows\external-libs\libpng\Win32
@set PTEID_DIR_OPENJPEG=C:\git\tools\build-scripts\windows\external-libs\openjpeg\Win32

:: Compiled / external libs x64
::==============================
@set PTEID_DIR_QML_X64=C:\git\tools\build-scripts\windows\external-libs\qt5.9.1\x64\VC9\Release\qml-app
@set PTEID_DIR_QT_4_X64=C:\Qt-5.9\5.9.1\msvc2013_64
@set PTEID_DIR_OPENSSL_098_X64=C:\git\tools\build-scripts\windows\external-libs\openssl-1.0.2p-win64
@set PTEID_DIR_XERCES_31_X64=C:\git\tools\build-scripts\windows\external-libs\xerces-c-3.1.4-win64
@set PTEID_DIR_CURL_X64=C:\git\tools\build-scripts\windows\external-libs\libcurl-vc12-x64-release-dll-ssl-dll
@set PTEID_DIR_ZLIB_X64=C:\git\tools\build-scripts\windows\external-libs\zlib\x64
@set PTEID_DIR_LIBZIP_X64=C:\git\tools\build-scripts\windows\external-libs\libzip-1.5.1-build-windows-Release\libzip-1.5.1-win64
@set PTEID_DIR_LIBPNG_X64=C:\git\tools\build-scripts\windows\external-libs\libpng\x64
@set PTEID_DIR_OPENJPEG_X64=C:\git\tools\build-scripts\windows\external-libs\openjpeg\x64

:: Tools
::======
@set PTEID_DIR_7ZIP=C:\Program Files\7-Zip
@set PTEID_DIR_PLATFORMSDK=C:\Program Files (x86)\Windows Kits\8.1
@set PTEID_DIR_VS=C:\Program Files (x86)\Microsoft Visual Studio 12.0
@set PTEID_DIR_DOXYGEN=C:\Program Files\doxygen-1.8.15.windows.x64.bin
@set PTEID_DIR_SWIG=C:\swigwin-2.0.12
@set PTEID_DIR_JDK=C:\Program Files\Java\jdk1.8.0_111
@set PTEID_DIR_WIX=C:\Program Files (x86)\WiX Toolset v3.10
@set PTEID_DIR_MSM=C:\Program Files (x86)\Common Files\Merge Modules

:: Credentials
::======
@set PTEID_CREDENTIALS_FILE=C:\git\tools\set_credentials\credentials.h