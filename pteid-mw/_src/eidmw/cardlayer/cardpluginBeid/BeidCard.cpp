	...
	SCARDHANDLE hCard, CContext *poContext, CPinpad *poPinpad)
{
	...
	if (bIsPtgemCard) 
	{
		...
		ulVersion = 1;
		poContext->m_oPCSC.EndTransaction(hCard);
		poCard = BeidCardGetInstance(ulVersion, csReader, hCard, poContext, poPinpad);
	} else {
	...
	SCARDHANDLE hCard, CContext *poContext, CPinpad *poPinpad)
{
	...
	if (ulVersion == 1)
	{
		...
		try {
			...
			unsigned char tucSelectApp[] = {0x00, 0xA4, 0x04, 0x00};
			oCmd.Append(tucSelectApp, sizeof(tucSelectApp));
			cout << "BeidCard.cpp: BeidCardGetInstance - oCmd.Append(tucSelectApp, sizeof(tucSelectApp));" << endl;
			oCmd.Append((unsigned char) sizeof(GEMSAFE_PTEID_APPLET_AID));
			oCmd.Append(GEMSAFE_PTEID_APPLET_AID, sizeof(GEMSAFE_PTEID_APPLET_AID));
			long lRetVal;
			// Don't remove these brackets, CAutoLock dtor must be called!
			{
				...
				{
					cout << "BeidCard.cpp COMM_DATA_LOST " << endl;
		...
		{
			cout << "Exception in cardPluginBeid.CardGetInstance() \(...)" << endl;
		}
        ...
        unsigned long & ulRemaining, const tPrivKey *pKey)
{
	...
	//beidPtrqin.encoding = PIN_ENC_BCD;
	beidPin.encoding = PIN_ENC_ASCII; //PT uses ASCII only for PIN

	if (m_AppletVersion == 1 ) {
		pincheck = CPkiCard::PinCmd(operation, beidPin, csPin1, csPin2, ulRemaining, pKey);
	} else {
		pincheck = CPkiCard::PinCmdIAS(operation, beidPin, csPin1, csPin2, ulRemaining, pKey);
	}
...
void CBeidCard::IasSignatureHelper()
{
