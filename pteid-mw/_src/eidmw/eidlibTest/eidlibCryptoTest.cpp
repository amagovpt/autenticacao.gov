...
TEST(PinCount)
{
	PTEID_ReaderContext &reader=ReaderSet.getReaderByNum(0);	
...
TEST(VerifyChangePin)
{
	...
	PTEID_EIDCard &card=reader.getEIDCard();				
	PTEID_Pin &pin=card.getPins().getPinByNumber(0);
...
TEST(CertifsHierarchy)
{
	...
	PTEID_EIDCard &card=reader.getEIDCard();				
	PTEID_Certificate &root=card.getCertificates().getRoot();

	try
	{
		PTEID_Certificate &issuer=root.getIssuer();
		CHECK(0);
	}
	...
	PTEID_Certificate &ca=root.getChildren(0);
	...
	try
	{
		PTEID_Certificate &child2=root.getChildren(1);
		CHECK(0);
	}
	...
	PTEID_Certificate &child0=ca.getChildren(0);
	...
	PTEID_Certificate &child1=ca.getChildren(1);
	...
	try
	{
		PTEID_Certificate &child2=ca.getChildren(2);
		CHECK(0);
	}
	catch(PTEID_ExParamRange e)
	{
...
TEST(CertifsInfo)
{
	...
	PTEID_Certificate &root=card.getCertificates().getRoot();
	PTEID_Certificate &ca=root.getChildren(0);
	PTEID_Certificate &cert0=ca.getChildren(1);
	...
	CHECK_EQUAL(PTEID_CERTIF_STATUS_VALID,cert0.getStatus(PTEID_VALIDATION_LEVEL_NONE,PTEID_VALIDATION_LEVEL_NONE));
}

TEST(AttributCertificateCRL)
{
	PTEID_Crl crl("http://crl.specimen-eid.portugal.be/eidc0001.crl");
	...
	CHECK_EQUAL("http://crl.specimen-eid.portugal.be/eidc0001.crl",crl.getUri());
}
...
TEST(AttributCertificateOCSP)
{
	...
	PTEID_ByteArray baNameHash(namehash,sizeof(namehash));
	PTEID_ByteArray baKeyHash(keyhash,sizeof(keyhash));
	PTEID_ByteArray baSerialNumber(serialnumber,sizeof(serialnumber));

	PTEID_OcspResponse ocsp("http://ocsp.specimen-eid.portugal.be",PTEID_ALGO_SHA1,baNameHash,baKeyHash,baSerialNumber);

	PTEID_ByteArray baResponse;

	CHECK_EQUAL(PTEID_CERTIF_STATUS_VALID_OCSP,ocsp.getResponse(baResponse));

	CHECK_EQUAL("http://ocsp.specimen-eid.portugal.be",ocsp.getUri());
}

//void FillCertTree(PTEID_Certificate &cert, unsigned long level)
//{
...
//	{
//		PTEID_Certificate &child=cert.getChildren(ulIndex);
