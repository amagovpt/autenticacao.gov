...
DWORD PTeidSignData(PCARD_DATA  pCardData, unsigned int HashAlgo, DWORD cbToBeSigned, PBYTE pbToBeSigned, DWORD *pcbSignature, PBYTE *ppbSignature)
{
   DWORD                   dwReturn = 0;
...
DWORD PTeidReadCert(PCARD_DATA  pCardData, DWORD dwCertSpec, DWORD *pcbCertif, PBYTE *ppbCertif)
{
   ...
   /* Read First 4 bytes */
   cbRead = 4;
   dwReturn = PTeidReadFile(pCardData, 0, &cbRead, bRead);
   if ( dwReturn != SCARD_S_SUCCESS )
   {
      LogTrace(LOGTYPE_ERROR, WHERE, "PTeidReadFile errorcode: [0x%02X]", dwReturn);
      CLEANUP(dwReturn);
