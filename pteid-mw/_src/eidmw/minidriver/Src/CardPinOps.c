	...
	)
{
	...
	dwReturn = PTeidMSE(pCardData, ROLE_DIGSIG);
	...
	dwReturn = PTeidAuthenticate(pCardData, pbPin, cbPin, pcAttemptsRemaining);
	if ( dwReturn != SCARD_S_SUCCESS )
	{
	...
	)
{
	...
	if (pbPinInfo.PinType == ExternalPinType && 
		...
		dwReturn = PTeidAuthenticate(pCardData, pbPinData, cbPinData, pcAttemptsRemaining);
	}
