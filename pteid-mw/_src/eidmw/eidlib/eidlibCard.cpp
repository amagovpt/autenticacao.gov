...
BEID_XMLDoc& BEID_EIDCard::getDocument(BEID_DocumentType type)
{
	...
	case BEID_DOCTYPE_ID:
		return getID();
	case BEID_DOCTYPE_ADDRESS:
		return getAddr();
	case BEID_DOCTYPE_SOD:
		return getSod();
...
BEID_Address& BEID_EIDCard::getAddr()
{
	BEID_Address *out = NULL;

	BEGIN_TRY_CATCH

	APL_EIDCard *pcard=static_cast<APL_EIDCard *>(m_impl);

	out=dynamic_cast<BEID_Address *>(getObject(INCLUDE_OBJECT_DOCEID));
	...
	if(!out)
	{
		...
		//if(!pdoc)
		//{
			out = new BEID_Address(m_context,&pcard->getAddr());
			if(out)
				m_objects[INCLUDE_OBJECT_DOCEID]=out;
			else
				throw BEID_ExUnknown();
...
BEID_Sod& BEID_EIDCard::getSod()
{
	...
	out = dynamic_cast<BEID_Sod *>(getObject(INCLUDE_OBJECT_SODID));
	...
	if(!out)
	{
		...
		//if(!ppicture)
		//{
			out = new BEID_Sod(m_context,&pcard->getSod());
			if(out)
...
const BEID_ByteArray& BEID_EIDCard::getRawData_Trace()
{
	BEID_ByteArray *out = NULL;
	...
	out = dynamic_cast<BEID_ByteArray *>(getObject(INCLUDE_OBJECT_RAWDATA_TRACE));
	...
	if(!out)
	{
		...
		//if(!praw)
		//{
			out = new BEID_ByteArray(m_context,pcard->getRawData_Trace());
			if(out)
...
const BEID_ByteArray& BEID_EIDCard::getRawData_Sod()
{
	BEID_ByteArray *out = NULL;
	...
	out = dynamic_cast<BEID_ByteArray *>(getObject(INCLUDE_OBJECT_RAWDATA_SOD));

	if(!out)
	{
		//CAutoMutex autoMutex(m_mutex);

		//praw=dynamic_cast<BEID_ByteArray *>(getObject(INCLUDE_OBJECT_RAWDATA_SOD));
		//if(!praw)
		//{
			out = new BEID_ByteArray(m_context,pcard->getRawData_Sod());
			if(out)
...
const BEID_ByteArray& BEID_EIDCard::getRawData_PersoData()
{
	BEID_ByteArray *out = NULL;
	...
	out = dynamic_cast<BEID_ByteArray *>(getObject(INCLUDE_OBJECT_RAWDATA_PERSO_DATA));

	if(!out)
	{
		//CAutoMutex autoMutex(m_mutex);

		//praw=dynamic_cast<BEID_ByteArray *>(getObject(INCLUDE_OBJECT_RAWDATA_PERSO_DATA));
		//if(!praw)
		//{
			out = new BEID_ByteArray(m_context,pcard->getRawData_PersoData());
			if(out)
