...
bool APL_EidFile_ID::MapFields()
{
	...
	//IssuingMunicipality
	pteidngidBuffer = m_data.GetBytes(PTEIDNG_FIELD_ID_POS_IssuingMunicipality, PTEIDNG_FIELD_ID_LEN_IssuingMunicipality);
	pteidngidBuffer.TrimRight(' ');
	m_IssuingMunicipality.assign((char*)(pteidngidBuffer.GetBytes()), pteidngidBuffer.Size());
    ...
    ulLen = sizeof(cBuffer);
	memset(cBuffer,0,ulLen);
	oTLVBuffer.FillUTF8Data(PTEID_FIELD_TAG_ID_Nobility, cBuffer, &ulLen);
...
---------------------------------------- APL_EidFile_RRN -----------------------------------------
*****************************************************************************************/
APL_EidFile_RRN::APL_EidFile_RRN(APL_EIDCard *card):APL_CardFile_Certificate(card,PTEID_FILE_RRN,NULL)
{
	cout << "RRN eidfile" << endl;
}
...
tCardFileStatus APL_EidFile_RRN::VerifyFile()
{
	...
	if(issuer==NULL) //If rrn is not issued by the root
	{
		...
		if(tempstatus==CARDFILESTATUS_ERROR_NOFILE)
		{
			...
			for(pissuer=_pteid_rrnca_certs;*pissuer!=NULL;pissuer++)
			{
				CByteArray issuer_data(*pissuer,_pteid_rrnca_certs_size[i++]);
...
---------------------------------------- APL_EidFile_RRNCA -----------------------------------------
*****************************************************************************************/
APL_EidFile_RRNCA::APL_EidFile_RRNCA(APL_EIDCard *card,const CByteArray *data):APL_CardFile_Certificate(card,PTEID_FILE_RRNCA,data)
{
}
